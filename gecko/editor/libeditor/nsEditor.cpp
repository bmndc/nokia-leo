...
nsEditor::FinalizeSelection()
{
  ...
  static bool sOnlyFocusedSelection = Preferences::GetBool(
      "layout.selection.only_focused_selection", false);

  if (sOnlyFocusedSelection) {
    // KaisOS Bug 79985:
    // If we are not focused, hidden the selection simply.
    selCon->SetDisplaySelection(nsISelectionController::SELECTION_HIDDEN);
  } else if (!HasIndependentSelection()) {
