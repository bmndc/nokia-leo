...
nsTextEditRules::WillDoAction(Selection* aSelection,
                              ...
                              bool* aHandled)
{
  return WillDoActionExt(aSelection, aInfo, aCancel, aHandled, 1, 0);
}

NS_IMETHODIMP
nsTextEditRules::WillDoActionExt(Selection* aSelection,
                              nsRulesInfo* aInfo,
                              bool* aCancel,
                              bool* aHandled,
                              int32_t mixLength,
                                ...
                                int32_t aNewlineHandling)
{
  if (aNewlineHandling < 0) {
    int32_t caretStyle;
    bool hideDisabledCommands;
    nsPlaintextEditor::GetDefaultEditorPrefs(aNewlineHandling, caretStyle, hideDisabledCommands);
  }
...
nsTextEditRules::WillInsertText(EditAction aAction,
                                ...
                                int32_t          aMaxLength)
{
  return WillInsertText(aAction, aSelection, aCancel, aHandled, inString, outString, aMaxLength, 1, 0);
}
nsresult
nsTextEditRules::WillInsertText(EditAction aAction,
                                Selection* aSelection,
                                ...
                                int32_t          aMixIndex)
{
  ...
  *aHandled = true;
  ...
  nsresult res = TruncateInsertionIfNeeded(aSelection, inString, outString,
                                           aMaxLength, &truncated, aMixLength,aMixIndex);
  NS_ENSURE_SUCCESS(res, res);
...
nsTextEditRules::TruncateInsertionIfNeeded(Selection* aSelection,
                                           ...
                                           bool *aTruncated)
{
  return TruncateInsertionIfNeeded(aSelection, aInString, aOutString, aMaxLength,
                                   aTruncated, 1, 0);
}
nsresult
nsTextEditRules::TruncateInsertionIfNeeded(Selection* aSelection,
                                           ...
                                           int32_t          aMixIndex)
{
  ...
  if ((-1 != aMaxLength) && IsPlaintextEditor() && !mEditor->IsIMEComposing() )
  {
    ...
    // when meet the mixed char, the first one, need to look left words.
    int32_t lookFowardDocLength = resultingDocLength;
    if (aMixLength > 1 )
    {
        if (aMixIndex == 0) {
          lookFowardDocLength += (aMixLength - 1);
          // Update fail state for the 1st byte.
          mEditor->SetMixCharFail(lookFowardDocLength >= aMaxLength);
        }
        if (mEditor->GetMixCharFail()) {
          // Consider the rest of length
          lookFowardDocLength += (aMixLength - 1);

          // The 2nd, 3rd, .. char's result is not equal to the before, something else goes wrong.
          // The result should be consistent over the whole multibyte char.
          MOZ_ASSERT(mEditor->GetMixCharFail() == (lookFowardDocLength >= aMaxLength));
          if (aMixLength - aMixIndex == 1) {
            // Clear fail state for the last byte.
            mEditor->SetMixCharFail(false);
          }
        }

    }
    if (lookFowardDocLength >= aMaxLength)
    {
