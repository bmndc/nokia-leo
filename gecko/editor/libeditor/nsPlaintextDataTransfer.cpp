...
NS_IMETHODIMP nsPlaintextEditor::Paste(int32_t aSelectionType)
{
  ...
  if (NS_SUCCEEDED(rv) && trans)
  {
    ...
    if (NS_SUCCEEDED(clipboard->GetData(trans, aSelectionType)) && IsModifiable())
    {
      ...
      // KaiOS Bug 82533: 1/2: Temporarily disable EchoPassowrd for password editor.
      uint32_t savedFlags;
      GetFlags(&savedFlags);
      if (IsPasswordEditor()) {
        SetFlags(savedFlags | nsIPlaintextEditor::eEditorDontEchoPassword);
      }

      rv = InsertTextFromTransferable(trans, nullptr, 0, true);

      // KaiOS Bug 82533: 2/2: Restore EchoPassowrd.
      SetFlags(savedFlags);
    }
  }
