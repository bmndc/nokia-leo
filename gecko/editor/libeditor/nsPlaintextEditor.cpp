...
, mCaretStyle(0)
#endif
, mHideDisabledCommands(false)
, mMixCharFail(false)
{
  // check the "single line editor newline handling"
  // and "caret behaviour in selection" prefs
  GetDefaultEditorPrefs(mNewlineHandling, mCaretStyle, mHideDisabledCommands);
}
...
static int32_t sNewlineHandlingPref = -1,
               sCaretStylePref = -1;
static bool sHideDisabledCommandPref = false;
  ...
  if (nsCRT::strcmp(aPrefName, "editor.singleLine.pasteNewlines") == 0) {
    ...
    if (sCaretStylePref == 0)
      sCaretStylePref = 1;
                                                 ...
                                                 0);
#endif
  } else if (nsCRT::strcmp(aPrefName, "layout.selection.hide_disabled_commands") == 0) {
    sHideDisabledCommandPref = Preferences::GetBool(
        "layout.selection.hide_disabled_commands", false);
  }
}
                                         ...
                                         bool &aHideDisabledCommands)
{
  if (sNewlineHandlingPref == -1) {
    ...
    Preferences::RegisterCallback(EditorPrefsChangedCallback,
                                  "layout.selection.hide_disabled_commands");
    EditorPrefsChangedCallback("layout.selection.hide_disabled_commands", nullptr);
  }

  aNewlineHandling = sNewlineHandlingPref;
  aCaretStyle = sCaretStylePref;
  aHideDisabledCommands = sHideDisabledCommandPref;
}
...
nsPlaintextEditor::HandleKeyPressEvent(nsIDOMKeyEvent* aKeyEvent)
{
  ...
  return TypedTextMix(str, eTypedText, nativeKeyEvent->mMixCharLength, nativeKeyEvent->mMixCharIndex);
}
...
{
  return TypedTextMix(aString, aAction, 1, 0);
}

NS_IMETHODIMP
nsPlaintextEditor::TypedTextMix(const nsAString& aString,
                             ETypingAction aAction, int32_t mixLength, int32_t mixIndex)
{
  nsAutoPlaceHolderBatch batch(this, nsGkAtoms::TypingTxnName);

  switch (aAction) {
    case eTypedText:
      return InsertTextExt(aString, mixLength, mixIndex);
    case eTypedBreak:
...
{
  return InsertTextExt(aStringToInsert, 1, 0);
}

NS_IMETHODIMP nsPlaintextEditor::InsertTextExt(const nsAString &aStringToInsert,
                int32_t mixlength, int32_t mixIndex)
{
  if (!mRules) { return NS_ERROR_NOT_INITIALIZED; }
  ...
  bool cancel, handled;
  nsresult res = mixlength > 1 ? mRules->WillDoActionExt(selection, &ruleInfo,
   &cancel, &handled, mixlength, mixIndex):
   mRules->WillDoAction(selection, &ruleInfo, &cancel, &handled);
...
bool
nsPlaintextEditor::SetMixCharFail(bool flag)
{
  mMixCharFail = flag;
  return true;
}
...
bool
nsPlaintextEditor::GetMixCharFail()
{
  return mMixCharFail;
}
...
NS_IMETHODIMP nsPlaintextEditor::CanCut(bool *aCanCut)
{
  ...
  nsCOMPtr<nsIDocument> doc = GetDocument();
  *aCanCut = (!mHideDisabledCommands && doc && doc->IsHTMLOrXHTML()) ||
...
NS_IMETHODIMP nsPlaintextEditor::CanCopy(bool *aCanCopy)
{
  ...
  nsCOMPtr<nsIDocument> doc = GetDocument();
  *aCanCopy = (!mHideDisabledCommands && doc && doc->IsHTMLOrXHTML()) ||
