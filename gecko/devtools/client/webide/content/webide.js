...
var UI = {
  ...
  openInBrowser: function(url) {
    ...
    if (browserWin) {
      // As current UI of webide has changed, it's better to catch exceptions here.
      try {
...
var Cmds = {
  ...
  newApp: function() {
    Services.obs.notifyObservers(null, "CMD_NEWAPP", null);
  },

  importPackagedApp: function() {
    Services.obs.notifyObservers(null, "CMD_IMPORT_PACKAGEDAPP", null);
  },

  importHostedApp: function() {
    Services.obs.notifyObservers(null, "CMD_IMPORT_HOSTEDAPP", null);
  },

  quit: function() {
    if (UI.canCloseProject()) {
      let appStartupService = Cc["@mozilla.org/toolkit/app-startup;1"].getService(Ci.nsIAppStartup);
      appStartupService.quit(Ci.nsIAppStartup.eAttemptQuit);
    }
  },
