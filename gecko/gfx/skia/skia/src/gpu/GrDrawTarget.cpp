...
void GrDrawTarget::recordBatch(GrBatch* batch) {
    ...
    if (maxCandidates) {
        ...
        while (true) {
            ...
            totalBatchCount += candidate->geoDataCount();
            if (totalBatchCount >= gfxPrefs::CanvasSkiaGLMaxBatchCount()) {
                /* Bug-58204 to avoid mess amount of batches to trigger LMK kill APP, */
                /* we limit the batch size to be default 1024, it can also be controlled by CanvasSkiaGLMaxBatchCount. */
                SkDebugf("Batch count for candidate %p is full, need to drop it", candidate);
                return;
            } else if (candidate->combineIfPossible(batch, *this->caps())) {
