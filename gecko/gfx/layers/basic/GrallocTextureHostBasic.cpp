...
HalFormatToSurfaceFormat(int aHalFormat, TextureFlags aFlags)
{
  ...
  case GrallocImage::HAL_PIXEL_FORMAT_I420:
  case GrallocImage::HAL_PIXEL_FORMAT_YUV_PRIVATE:
  case GrallocImage::HAL_PIXEL_FORMAT_YUV_PRIVATE_10BIT:
#endif
  ...
  default:
    if (aHalFormat >= 0x100 && aHalFormat <= 0x1FF) {
      ...
      printf_stderr("GFX: Unhandled HAL pixel format %x\n", aHalFormat);
...
NeedsConvertFromYUVtoRGB565(int aHalFormat)
{
  ...
  default:
    if (aHalFormat >= 0x100 && aHalFormat <= 0x1FF) {
      ...
      printf_stderr("Unhandled HAL pixel format YUV %x\n", aHalFormat);
