...
int32_t GrallocImage::sColorIdMap[] = {
    ...
    HAL_PIXEL_FORMAT_NV12_VED, HAL_PIXEL_FORMAT_NV12_VED,
    HAL_PIXEL_FORMAT_NV12_VEDT, HAL_PIXEL_FORMAT_NV12_VEDT,
     ...
     uint32_t    length)
{
  ...
  if (fp) {
    ...
    NS_WARNING("writeBufferToFile: fopen failed");
  }
}
#endif

/**
 * Converts the format of vendor-specific NV12 block format
 * with the help of GraphicBuffer::lock. In this way, we can convert
 * the YUV color format without awaring actual definition/enumeration
 * of vendor formats.
 */
static status_t
ConvertMTKYUVFormatToRGB565(android::sp<GraphicBuffer>& aBuffer,
                            gfx::DataSourceSurface *aSurface,
                            gfx::DataSourceSurface::MappedSurface *aMappedSurface)
{
  status_t rv;
  uint8_t *buffer;
  buffer_handle_t _handle = aBuffer->getNativeBuffer()->handle;
  ...
  if (GRALLOC_EXTRA_OK != err) {
      NS_WARNING("[ConvertMTKYUVFormatToRGB565]gralloc_extra_query error");
      return BAD_VALUE;
  }

  // get real format within private format
  int format = sf_info.status & GRALLOC_EXTRA_MASK_CM;
  // get alignment
  uint32_t uYAlign = 0;
  uint32_t uCbCrAlign = 0;
  uint32_t uHeightAlign = 0;
  if (sf_info.videobuffer_status & 0x80000000) {
    uYAlign = (sf_info.videobuffer_status & 0x7FFFFFFF) >> 25;
    uYAlign = SYMBOL2ALIGN(uYAlign);

    uCbCrAlign = (sf_info.videobuffer_status & 0x01FFFFFF) >> 19;
    uCbCrAlign = SYMBOL2ALIGN(uCbCrAlign);

    uHeightAlign = (sf_info.videobuffer_status & 0x0007FFFF) >> 13;
    uHeightAlign = SYMBOL2ALIGN(uHeightAlign);
  } else {
    NS_WARNING("[ConvertMTKYUVFormatToRGB565]get alignment fail");
    return BAD_VALUE;
  }
  ...
  /* Use MTK DpFramework for converting MTK YUV format to RGB565. */
  DpBlitStream blitStream;
  uint32_t srcWStride, srcHStride;
  DpRect srcRoi;
  uint8_t *srcPlanar[3];
  uint32_t srcLength[3];
  srcWStride = ALIGN(width, uYAlign);
  srcHStride = ALIGN(height, uHeightAlign);

#if ENABLE_MTK_DUMP
  char filename_mtk_format[256];
  char filename_rgb565[256];
#endif

  if (format == GRALLOC_EXTRA_BIT_CM_YV12) {
    srcRoi.x = 0;
    srcRoi.y = 0;
    srcRoi.w = width;
    srcRoi.h = height;
    ...
    srcLength[1] = srcWStride * srcHStride / 4;
    srcPlanar[2] = srcPlanar[1] + srcLength[1];
    srcLength[2] = srcWStride * srcHStride / 4;

    /* Set source buffer and configuration */
    blitStream.setSrcBuffer((void **)srcPlanar, (unsigned int *)srcLength, 3);
    blitStream.setSrcConfig(srcWStride, srcHStride, srcWStride, ALIGN(srcWStride / 2, uCbCrAlign), eYV12, DP_PROFILE_BT601, eInterlace_None, &srcRoi);

#if ENABLE_MTK_DUMP
    snprintf(filename_mtk_format, sizeof(filename_mtk_format), "/data/kaios_thumbnail_yv12_w%d_h%d_t%d.yuv",
                    srcWStride,srcHStride,gettid());
    writeBufferToFile(filename_mtk_format, srcPlanar[0], srcLength[0]);
    writeBufferToFile(filename_mtk_format, srcPlanar[1], srcLength[1]);
    writeBufferToFile(filename_mtk_format, srcPlanar[2], srcLength[2]);
#endif
  } else if (format == GRALLOC_EXTRA_BIT_CM_NV12_BLK) {
    srcRoi.x = 0;
    srcRoi.y = 0;
    srcRoi.w = width;
    srcRoi.h = height;
    srcPlanar[0] = buffer;
    srcLength[0] = srcWStride * srcHStride;
    srcPlanar[1] = srcPlanar[0] + ALIGN(srcLength[0], 512);
...
#if ENABLE_MTK_DUMP
    snprintf(filename_mtk_format, sizeof(filename_mtk_format), "/data/kaios_thumbnail_nv12_blk_w%d_h%d_t%d.yuv",
                    srcWStride,srcHStride,gettid());
    writeBufferToFile(filename_mtk_format, srcPlanar[0], srcLength[0]);
    writeBufferToFile(filename_mtk_format, srcPlanar[1], srcLength[1]);
#endif
  ...
  DpRect dstRoi;
  uint32_t dstWStride = ALIGN(width * 2, 16);
  uint32_t dstHStride = height;
  uint32_t dstLength = dstWStride * dstHStride;
  dstRoi.x = 0;
  dstRoi.y = 0;
  dstRoi.w = srcRoi.w;
  dstRoi.h = srcRoi.h;
  /* Set destination buffer and configuration */
  blitStream.setDstBuffer((void *)aMappedSurface->mData, dstLength);
  blitStream.setDstConfig(width, height, dstWStride, 0, eRGB565, DP_PROFILE_BT601, eInterlace_None, &dstRoi);

  /* Trigger DpBlitStream for buffer convertion */
  if (blitStream.invalidate() != DP_STATUS_RETURN_SUCCESS) {
    NS_WARNING("MTK DpBlitStream invalidate returns fail");
    rv = FAILED_TRANSACTION;
  }

#if ENABLE_MTK_DUMP
  snprintf(filename_rgb565, sizeof(filename_rgb565), "/data/kaios_thumbnail_rgb565_w%d_h%d_t%d.yuv",
                    dstWStride,dstHStride,gettid());
  writeBufferToFile(filename_rgb565, aMappedSurface->mData, dstLength);
#endif
...
#if defined(PRODUCT_MANUFACTURER_MTK)
  // bug 56529 for converting MTK proprietary format HAL_PIXEL_FORMAT_YUV_PRIVATE into
  // RGB565 format, we need to do this since MTK GPU can't support lockYCbCr
  if (aBuffer->getPixelFormat() == GrallocImage::HAL_PIXEL_FORMAT_YUV_PRIVATE)
  {
    rv = ConvertMTKYUVFormatToRGB565(aBuffer, aSurface, aMappedSurface);
    if (rv != OK) {
      NS_WARNING("Couldn't convert graphic buffer using ConvertMTKYUVFormatToRGB565()");
    }
