...
public:
  ...
  // True if this key event is generated by Keyboard App, and we will not
  // generate mozbrowserbeforeafter* for this kind of event. Furthermore,
  // because the relaying procedure runs on chrome process, if we fail to check
  // this flag and the event target is from chrome process, it'll end up
  // creating an infinite loop.
  bool mGeneratedFromIME : 1;
  // If mHandledByIME is true, it means this event is relayed to Keyboard App
  // via KeyboardAppProxy for further processing.
  bool mHandledByIME : 1;
