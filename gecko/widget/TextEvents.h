...
public:
  ...
  // over long periods.
  uint32_t mUniqueId;
  // Record length of mixing charaters, e.g. emoji input characters has two
  // characters.
  uint32_t mMixCharLength;
  // the index of MixChar words.
  uint32_t mMixCharIndex;
  ...
  void SetMixCharLength(uint32_t length)
  {
      mMixCharLength = length;
  }

  void SetMixCharIndex(uint32_t index)
  {
      mMixCharIndex = index;
  }
