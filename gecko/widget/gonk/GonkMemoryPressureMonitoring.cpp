...
#define B2G_MEMORY_OVERUSED_TRIGGER_CYCLE 10

using namespace mozilla;

namespace {

class B2gMemoryOverusedEvent : public nsRunnable {
public:
  B2gMemoryOverusedEvent()
  {
  }
  ...
  NS_IMETHOD Run()
  {
    ...
    if (obs) {
      obs->NotifyObservers(nullptr, "b2g-memory-overused", nullptr);
    }
    return NS_OK;
  }
};
      ...
      do {
        ...
        if (memoryPressure) {
          ...
          if (XRE_IsParentProcess()) {
            mMemoryPressureCounter++;
            if (mMemoryPressureCounter >= B2G_MEMORY_OVERUSED_TRIGGER_CYCLE &&
                mB2gMemThresholdKB > 0) {
              uint32_t b2gMem = mozilla::hal::GetProcessMemory(getpid());
              if (b2gMem > mB2gMemThresholdKB) {
                LOG("B2g memory is overused (%dMB > %dMB)",
                    b2gMem/1024, mB2gMemThresholdKB/1024);
                NS_DispatchToMainThread(new B2gMemoryOverusedEvent());
              }
            }
          }
          continue;
...
private:
  void ReadPrefs() {
    ...
    Preferences::AddUintVarCache(&mB2gMemThresholdKB,
      "gonk.b2gMemThresholdKB", /* default*/ 0);
  }
  ...
  uint32_t mB2gMemThresholdKB; // Threshold for notifying memory cleaner
  uint32_t mMemoryPressureCounter;
  size_t mPageSize;
  bool mShuttingDown;
