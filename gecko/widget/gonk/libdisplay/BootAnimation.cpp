...
public:
    ...
    {
        LOG("SoundPlay");
        mSourceFd = open("/system/media/poweron-sound.ogg", O_RDONLY);
        if (mSourceFd == -1) {
            LOGW("can't find bootanimation sound resource....");
            return false;
        } 
        LOGW("mSourceFd : %d", mSourceFd);
        
        off_t length = lseek(mSourceFd, 0, SEEK_END);
        if (length == -1) {
            close(mSourceFd);
            mSourceFd = -1;
            LOGW("failed to seek to SEEK_END");
            return false;
        }
        if ((lseek(mSourceFd, 0, SEEK_SET)) == -1) {
            close(mSourceFd);
            mSourceFd = -1;
            LOGW("failed to seek to SEEK_SET");
            return false;
        }

        android::status_t e = android::NO_ERROR;
        mMediaPlayer = new android::MediaPlayer();
        e = mMediaPlayer->setDataSource(mSourceFd, 0, static_cast<int64_t>(length));
        if (e != android::NO_ERROR) {
            LOGW("setDataSource failed.");
            SoundReset();
            return false;
        }
        e = mMediaPlayer->setAudioStreamType(AUDIO_STREAM_SYSTEM);
        if (e != android::NO_ERROR) {
            LOGW("setAudioStreamType failed.");
            SoundReset();
            return false;
        }
        e = mMediaPlayer->prepare();
        if (e != android::NO_ERROR) {
            LOGW("prepare mediaplayer failed.");
            SoundReset();
            return false;
        }
        e = mMediaPlayer->start();
        if (e != android::NO_ERROR) {
            LOGW("start mediaplayer failed.");
            SoundReset();
            return false;
        }
        return true;
    ...
    bool SoundStop() 
    {
        ...
        if (mMediaPlayer != nullptr) {
            error = mMediaPlayer->stop();
            if (error != android::NO_ERROR) {
                LOGW("stop mediaplayer failed.");
                SoundReset();
            }
        }
        return true;
...
static void
SetMemoryCleanerAnimationStatus(const char* aStatus)
{
    if (!property_set("kaios.mcm.animation.status", aStatus)) {
        LOG("AnimationStatus: %s\n", aStatus);
    } else {
        LOGE("Fail to set property, kaios.mcm.animation.status");
    }
}
...
AnimationThread(void *)
{
    ...
    bool startMemoryCleanerAnimation = false;
    property_get("kaios.mcm.animation.status", prop, NULL);

    if (!strcmp(prop, "started")) {
        startMemoryCleanerAnimation = true;
    } else if (strcmp(prop, "stopped")){
        // For other cases, reset the property to stopped
        LOG("Reset animation status from %s to stopped\n", *prop ? prop : "null");
        SetMemoryCleanerAnimationStatus("stopped");
    }
    if (startMemoryCleanerAnimation) {
        SetMemoryCleanerAnimationStatus("stage1");
        if (!primAnimation.LoadAnimations("/system/media/memorycleaneranimation.zip")) {
            LOGW("Failed to load memory cleaner animation file for primary screen");
      ...
      // miaoyj modify for LIO-704:custom bootanimation for AMX start   
      char skuid_prop[PROPERTY_VALUE_MAX];
      property_get("ro.boot.skuid", skuid_prop, NULL);
      bool isLoadSucess=false;
   	  LOGW("load boot animation skuid_prop=%s",skuid_prop);
      if (!strcmp(skuid_prop, "62HMX")) {
        isLoadSucess=primAnimation.LoadAnimations("/system/media/bootanimation_telcel.zip");
      }else if(!strcmp(skuid_prop, "62GMX")){
        isLoadSucess=primAnimation.LoadAnimations("/system/media/bootanimation_claro.zip");
      }else{
        isLoadSucess=primAnimation.LoadAnimations("/system/media/bootanimation.zip");
      }
      if (!isLoadSucess) {
      ...
      // miaoyj modify for LIO-704:custom bootanimation for AMX end   
    }
    ...
    */
    ...
    // MediaPlayer solution
    AnimationSoundSync bootSound;
    // There is no sound when playing animation of memory cleaner

    bool isAMX = false;
    char skuid_prop[PROPERTY_VALUE_MAX];
    property_get("ro.boot.skuid", skuid_prop, NULL);
    LOGW("load boot animation skuid_prop = %s", skuid_prop);
    if (!strcmp(skuid_prop, "62HMX") || !strcmp(skuid_prop, "62GMX")) {
        isAMX = true;
    }

    if (!startMemoryCleanerAnimation && !isAMX) {
        bootSound.SoundPlay();
    }
    ...
    if (!startMemoryCleanerAnimation) {
        ...
        SetMemoryCleanerAnimationStatus("stage1tostage2");
    }
    return nullptr;
}
