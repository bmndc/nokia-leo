...
if (AppConstants.ENABLE_OMADM_REDBEND) {
  Cu.import('resource://gre/modules/OmaService.jsm');
  Cu.import('resource://gre/modules/MdmUtils.jsm');
}
...
var shell = {
  ...
  start: function shell_start() {
    ...
    // Because the system app will call the customization API,
    // So we have to initialize the customization before the system app starts.
    if (isGonk) {
     // DeviceConfig files.
  ...
  notifyEventListenerReady: function shell_notifyEventListenerReady() {
    ...
    // Send reboot reason after receiving 'system-message-listener-ready'
    // which makes sure that system app can receive reboot reason.
    try {
      let reason = Services.prefs.getCharPref("device.rebootReason");
      if (reason) {
        this.sendChromeEvent({type: "rebootreason", value: reason});
        Services.prefs.setCharPref("device.rebootReason", "unknown");
      }
    } catch (e) {  }
...
Services.obs.addObserver(function(subject, topic, data) {
  let filepath = ChromeUtils.saveHeapSnapshot({ runtime: true });
  dump("Dumping heap snapshot file: " + filepath + "\n");
}, 'heap-snapshot', false);
