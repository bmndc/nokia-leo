...
var AlertsHelper = {
  ...
  showNotification: function(iconURL, imageURL, title, text, textClickable,
                             cookie, uid, dir, lang, dataObj, manifestURL,
                             timestamp, behavior, requireInteraction, actions,
                             silent, serviceWorkerRegistrationID) {
    ...
    appsService.getManifestFor(manifestURL).then((manifest) => {
      ...
      let helper = new ManifestHelper(manifest, app.origin, manifestURL, lang);
  ...
  showAlertNotification: function(aMessage) {
    ...
    this.showNotification(data.iconURL, data.imageURL, data.title, data.text,
  ...
  showAppNotification: function(aMessage) {
    ...
    this.showNotification(data.iconURL, data.imageURL, data.title, data.text,
                          ...
                          details.silent, details.serviceWorkerRegistrationID);
  },
