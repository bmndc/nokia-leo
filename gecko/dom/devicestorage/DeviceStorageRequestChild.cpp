  ...
  Recv__delete__(const DeviceStorageResponseValue& aValue)
{
  switch (aValue.type()) {
    ...
    case DeviceStorageResponseValue::TTotalSpaceStorageResponse:
    {
      DS_LOG_INFO("total %u", mRequest->GetId());
      TotalSpaceStorageResponse r = aValue;
      mRequest->Resolve(r.totalBytes());
      break;
    }
