  ...
  const DeviceStorageRequestType aRequestType)
{
  switch(aRequestType) {
    ...
    case DEVICE_STORAGE_REQUEST_TOTAL_SPACE:
...
void
DeviceStorageFile::GetStorageTotalSpace(int64_t* aSoFar)
{
  DeviceStorageTypeChecker* typeChecker
    = DeviceStorageTypeChecker::CreateOrGet();
  if (!typeChecker) {
  ...
  nsresult rv = mFile->GetDiskSpaceTotal(&storageTotal);
  if (NS_SUCCEEDED(rv)) {
    *aSoFar += storageTotal;
  }
}
...
class DeviceStorageTotalSpaceRequest final
  : public DeviceStorageRequest
{
public:
  DeviceStorageTotalSpaceRequest()
  {
    mAccess = DEVICE_STORAGE_ACCESS_READ;
    mUseStreamTransport = true;
    DS_LOG_INFO("");
  ...
  NS_IMETHOD Run() override
  {
    ...
    return Resolve(static_cast<uint64_t>(totalSpace));
  }

protected:
  nsresult CreateSendParams(DeviceStorageParams& aParams) override
  {
    DeviceStorageTotalSpaceParams params(mFile->mStorageType,
                                        mFile->mStorageName);
    aParams = params;
    return NS_OK;
  }
...
already_AddRefed<DOMRequest>
nsDOMDeviceStorage::TotalSpace(ErrorResult& aRv)
{
  MOZ_ASSERT(IsOwningThread());
  ...
  RefPtr<DeviceStorageRequest> request = new DeviceStorageTotalSpaceRequest();
  request->Initialize(mManager, dsf.forget(), id);
...
void
nsDOMDeviceStorage::GetStoragePath(nsAString& aStoragePath)
{
