...
DeviceStorageRequestParent::Dispatch()
{
  ...
  switch (mParams.type()) {
    ...
    case DeviceStorageParams::TDeviceStorageTotalSpaceParams:
    {
      DeviceStorageTotalSpaceParams p = mParams;

      RefPtr<DeviceStorageFile> dsf =
        new DeviceStorageFile(p.type(), p.storageName());
      r = new TotalSpaceFileEvent(this, dsf.forget());
      break;
    }
  ...
  mozilla::dom::ContentParent* aParent)
{
  ...
  switch (mParams.type())
  {
    ...
    case DeviceStorageParams::TDeviceStorageTotalSpaceParams:
    {
      DeviceStorageTotalSpaceParams p = mParams;
      type = p.type();
      requestType = DEVICE_STORAGE_REQUEST_TOTAL_SPACE;
      break;
    }
...
DeviceStorageRequestParent::PostTotalSpaceResultEvent::PostTotalSpaceResultEvent(
  DeviceStorageRequestParent* aParent,
  uint64_t aTotalSpace)
  : CancelableRunnable(aParent)
  , mTotalSpace(aTotalSpace)
{
}

DeviceStorageRequestParent::PostTotalSpaceResultEvent::
  ~PostTotalSpaceResultEvent() {}

nsresult
DeviceStorageRequestParent::PostTotalSpaceResultEvent::CancelableRun() {
  MOZ_ASSERT(NS_IsMainThread());

  TotalSpaceStorageResponse response(mTotalSpace);
  Unused << mParent->Send__delete__(mParent, response);
  return NS_OK;
}
...
nsresult
DeviceStorageRequestParent::TotalSpaceFileEvent::CancelableRun()
{
  MOZ_ASSERT(!NS_IsMainThread());

  int64_t totalSpace = 0;
  if (mFile) {
    mFile->GetStorageTotalSpace(&totalSpace);
  }

  return NS_DispatchToMainThread(
    new PostTotalSpaceResultEvent(mParent, static_cast<uint64_t>(totalSpace)));
}
