...
SEChannelImpl.prototype = {
  ...
  transmit: function transmit(command) {
    ...
    let transmitMgtCmd = false;
    try {
      transmitMgtCmd = Services.prefs.getBoolPref("dom.secureelement.transmit.mgt_cmd")
    } catch (e) {
    }

    if (((command.cla & 0x80) === 0) && ((command.cla & 0x60) !== 0x20)) {
      if ((command.ins === SE.INS_MANAGE_CHANNEL) && (transmitMgtCmd === false)) {
        return PromiseHelpers.rejectWithSEError(SE.ERROR_SECURITY,
               "MANAGE CHANNEL command not permitted");
      }
      if ((command.ins === SE.INS_SELECT) &&
          (command.p1 == 0x04) && (transmitMgtCmd === false)) {
    ...
    // Factory Reset command requires "secureelement-factory-reset" permission.
    if ((command.cla === 0x80) && (command.ins === 0xC3) &&
        (command.p1 === 0x04) && (command.p2 === 0x00) && (command.le === 0x00)) {
      let principal = this._window.document.nodePrincipal;
      let action = Services.perms.testPermissionFromPrincipal(principal,
                                                'secureelement-factory-reset');
      if (action != Ci.nsIPermissionManager.ALLOW_ACTION) {
        return PromiseHelpers.rejectWithSEError(SE.ERROR_SECURITY,
               "Reset command not permitted");
      }
    }
