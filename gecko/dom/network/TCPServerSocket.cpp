...
TCPServerSocket::Init()
{
  ...
  PRNetAddr prAddr;
  if (mAddress.Find(":") != -1) {
    PR_SetNetAddr(PR_IpAddrAny, PR_AF_INET6, mPort, &prAddr);
  } else {
    PR_SetNetAddr(PR_IpAddrAny, PR_AF_INET, mPort, &prAddr);
  }

  PRStatus status = PR_StringToNetAddr(NS_ConvertUTF16toUTF8(mAddress).BeginReading(), &prAddr);
  if (status != PR_SUCCESS) {
    return NS_ERROR_DOM_INVALID_ACCESS_ERR;
  }

  rv = mServerSocket->InitWithAddress(&prAddr, mBacklog);
  NS_ENSURE_SUCCESS(rv, rv);
                             ...
                             mozilla::ErrorResult& aRv)
{
  ...
  RefPtr<TCPServerSocket> socket = new TCPServerSocket(global, aPort, useArrayBuffers,
                                                       aBacklog, aAddress);
