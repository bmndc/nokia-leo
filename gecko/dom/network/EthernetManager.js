...
const DEFAULT_ETHERNET_TCP_BUFFER_SIZE = "4096,87380,524288,4096,16384,110208";
const TCP_BUFFER_SIZE = (function () {
  let tcpBufferSize;
  try {
    tcpBufferSize = Services.prefs.getCharPref("net.tcp.buffersize.default", DEFAULT_ETHERNET_TCP_BUFFER_SIZE);
  } catch (e) {
    tcpBufferSize = DEFAULT_ETHERNET_TCP_BUFFER_SIZE;
  }
...
function EthernetInterface(attr) {
  ...
  this.info.tcpbuffersizes = TCP_BUFFER_SIZES;
  this.httpProxyHost = "";
...
function EthernetManager() {
  ...
  this.ipClient = new IpClient("Ethernet");
...
EthernetManager.prototype = {
  ...
  updateInterfaceConfig: function(ifname, config, callback) {
    ...
    this._ensureIfname(ifname, callback, function(iface) {
      ...
      if ((currentIpMode == IP_MODE_STATIC) && (newIpMode == IP_MODE_DHCP)) {
        this.ipClient.stopDhcp(iface.info.name, function(success) {
          if (success) {
  ...
  _runDhcp: function(ifname, callback) {
    ...
    this.ipClient.dhcpRequest(ifname, function(success, result) {
      if (!success) {
        if (callback) {
