...
UDPSocketParent::RecvBind(const UDPAddressInfo& aAddressInfo,
                          ...
                          const uint32_t& recvBufferSize, const bool& aIsIPv6)
{
  UDPSOCKET_LOG(("%s: %s:%u", __FUNCTION__, aAddressInfo.addr().get(), aAddressInfo.port()));

  if (NS_FAILED(BindInternal(aAddressInfo.addr(), aAddressInfo.port(), aAddressReuse,
                             aLoopback, recvBufferSize, aIsIPv6))) {
                              ...
                              const uint32_t& recvBufferSize, const bool& aIsIPv6)
{
  ...
  if (aHost.IsEmpty()) {
    rv = sock->Init(aPort, false, mPrincipal, aAddressReuse, aIsIPv6,
                    /* optional_argc = */ 2);
  } else {
    PRNetAddr prAddr;
