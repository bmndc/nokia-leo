                           ...
                           mozilla::ErrorResult& aRv)
{
  ...
  GlobalObject globalObj(api.cx(), mGlobal->GetGlobalJSObject());
  return TCPServerSocket::Constructor(globalObj, aPort, aOptions,
                                      aBacklog, aAddress, aRv);
}
...
{
  // Should cut off after parent destroys object,
  // because it is an ASYNC operation with socket thread
  // OnAsyncCopyComplete->nsAsyncStreamCopier->
  // nsRequestObserverProxy->OnStopRequest
  // EnsureCopying->nsAsyncStreamCopier::AsyncCopy

  if(mOwner && mOwner->mActorDestroyed)
  {
    mOwner = nullptr;
