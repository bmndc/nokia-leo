...
HashPages.prototype = {
  registerPage: function registerPage(key, value) {
    let previous = [];
    if (this.hasPage(key)) {
      previous = this.pages[key];
      previous.push(value);
      this.pages[key]=previous;
    } else {
      previous.push(value);
      this.pages[key] = previous;
      this.length++;
  ...
  getPage: function getPage(key) {
    return this.hasPage(key) ? this.pages[key] : [];
  },

  hasPage: function hasPage(key) {
    return this.pages.hasOwnProperty(key);
  },

  unregisterPage: function unregisterPage(key, fn) {
    ...
    this.pages[key] = previous.filter(fn);

    arrlen = previous.length;
    if (!arrlen) {
      this.length--;
      delete this.pages[key];
    }
  ...
  values: function values(fn) {
    var values = [];
    for (var k in this.pages) {
      if (this.hasPage(k)) {
        values.push(this.pages[k]);
        if (fn) {
          fn(this.pages[k]);
        }
      }
...
SystemMessageInternal.prototype = {
  ...
  _findPage: function(aType, aPageURL, aManifestURL) {
    let pages = [];
    pages = this._hashPages.getPage(aType);
    if (!pages.length) {
      return null;
    }
    ...
    for (let i = 0; i < pages.length; i++) {
      if (this._isPageMatched(pages[i], aType, aPageURL, aManifestURL)) {
        page = pages[i];
      }
    }
  ...
  _findCacheForApp: function(aManifestURL) {
    let cache = [];
    this._hashPages.values(function(aValue) {
      for (let i = 0; i < aValue.length; i++) {
        let page = aValue[i];
        if (page.manifestURL === aManifestURL &&
            page.pendingMessages.length != 0) {
          cache.push({ type: page.type,
  ...
  sendMessageInternal: function(aType, aMessage, aPageURI, aManifestURI,
    ...
    if (aPageURI) {
      ...
      // is not specified.
      let pages = this._hashPages.getPage(aType);
      for (let i = 0; i < pages.length; i++) {
        let page = pages[i];
    ...
    pageURLs.forEach(function(aPageURL) {
      ...
      shouldDispatchFunc(manifestURL, aPageURL, aType, aMessage, aExtra)
      ...
      .catch(()=>{
        debug("Configurator type:" + aType + " has something wrong !!!");
      });
    }, this);
  },
  ...
  broadcastMessage: function(aType, aMessage, aExtra) {
    ...
    let hashPage = this._hashPages.getPage(aType);
    if (!hashPage.length) {
      debug("The page isn't registered for " + aType);
      return Promise.resolve();
    }
      ...
      let promise = ((page) => {
        return new Promise((resolve, reject) => {
          ...
          shouldDispatchFunc(page.manifestURL, page.pageURL, aType, aMessage, aExtra)
            .then(aShouldDispatch => {
              if (aShouldDispatch) {
                debug("Dispatch to " + page.manifestURL);
                doDispatch();
              } else {
                debug("Don't dispatch to " + page.manifestURL);
              }
            });
        });
      })(hashPage[i]);
  ...
  registerPage: function(aType, aPageURI, aManifestURI) {
    ...
    this._hashPages.registerPage(aType, {
                       type: aType,
                       pageURL: pageURL,
                       manifestURL: manifestURL,
                       pendingMessages: [] });
  ...
  receiveMessage: function(aMessage) {
    ...
    switch(aMessage.name) {
      ...
      case "SystemMessageManager:GetPendingMessages":
      {
        ...
        if (!page) {
          debug("GetPendingMessages: can't find page");
          return;
        }
        debug("GetPendingMessages: find page:" + JSON.stringify(page));
