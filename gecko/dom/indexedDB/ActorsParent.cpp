...
#define LOGIDB(...) __android_log_print(ANDROID_LOG_INFO, "Gecko::IDB", ## __VA_ARGS__)
#endif
                        ...
                        UsageInfo* aUsageInfo)
{
  ...
  while (NS_SUCCEEDED((rv = entries->HasMoreElements(&hasMore))) &&
    ...
    // files folder is corrupted by accident power failure,
    // that becomes file only. This is unfortuneately
    // prevents all storage from working. Remove the
    // corrupted directory to restore storage.
    nsString recoveryPath;
    rv = file->GetPath(recoveryPath);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }

    if (!isDirectory &&
        StringEndsWith(recoveryPath, filesSuffix)) {
#ifdef MOZ_WIDGET_GONK
      LOGIDB("InitOrigin() remove folder: %s",
             NS_ConvertUTF16toUTF8(recoveryPath).get());
#endif

      rv = file->Remove(false);
      if (NS_WARN_IF(NS_FAILED(rv))) {
#ifdef MOZ_WIDGET_GONK
        LOGIDB("InitOrigin() remove bad files folder failure");
#endif
        return rv;
      }
    }
