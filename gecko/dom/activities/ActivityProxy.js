...
ActivityProxy.prototype = {
  startActivity: function actProxy_startActivity(aActivity, aOptions, aWindow,
    ...
    // Although <input type=file> is triggered by content frame, activity of
    // "file-picker" is created and handled in chrome window. In order to set up
    // caller's manifest URL, override appId if appId is provided in aOptions.
    if ((appId == Ci.nsIScriptSecurityManager.NO_APP_ID ||
         appId == Ci.nsIScriptSecurityManager.UNKNOWN_APP_ID) &&
        (aOptions.data && aOptions.data.appId)) {
      appId = aOptions.data.appId;
    }
    ...
    // Validate the identity of the application.
    if (aOptions.origin) {
      let origin = this._getHashForApp(manifestURL);
      if (origin.toUpperCase() != aOptions.origin.toUpperCase()) {
        Services.DOMRequest.fireErrorAsync(this.activity, "SecurityError");
        Services.obs.notifyObservers(null, "Activity:Error", null);
        return;
      }
  ...
  _getHashForApp: function getHashForApp(manifestURL) {
