...
NS_IMETHODIMP
IccChild::GetGid(nsAString & aGid)
{
  NS_WARNING("GID shall not directly be fetched in child process.");
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
IccRequestChild::Recv__delete__(const IccReply& aResponse)
{
  ...
  switch(aResponse.type()) {
    ...
    case IccReply::TIccReplyOpenChannel: {
      ...
      return NS_SUCCEEDED(
        mChannelRequestReply->NotifyOpenChannelSuccess(channel.channel()));
    }
