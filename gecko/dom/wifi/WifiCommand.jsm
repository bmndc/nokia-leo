...
this.WifiCommand = function(aControlMessage, aInterface, aSdkVersion) {
  ...
  command.addCredential = function (callback) {
    doIntCommand("ADD_CRED", callback);
  };

  command.removeCredential = function (netId, callback) {
    doBooleanCommand("REMOVE_CRED " + netId, "OK", callback);
  };

  command.setCredentialVariable = function (netId, name, value, callback) {
    doBooleanCommand("SET_CRED " + netId + " " + name + " " +
                      value, "OK", callback);
  };
