...
static const char *kPrefNotificationMaxActions = "dom.webnotifications.serviceworker.maxActions";
...
public:
                    ...
                    const nsAString& aServiceWorkerRegistrationID) final
  {
    ...
    mFields.AppendElement(Move(fields));
    return NS_OK;
  }
...
protected:
  ...
  nsTArray<NotificationFields> mFields;
};
...
public:
  ...
  NS_IMETHOD Done() final
  {
    ...
    for (uint32_t i = 0; i < mFields.Length(); ++i) {
      RefPtr<Notification> n =
        Notification::ConstructFromFields(mWindow,
                                           ...
                                           * supported */
                                          mFields[i].mRequireInteraction,
                                          mFields[i].mActions,
                                          mFields[i].mSilent,
                                          mFields[i].mServiceWorkerRegistrationID,
                                          result);
...
{
public:
  enum NotificationTaskAction {
    eShow,
    eClose
  };

  NotificationTask(UniquePtr<NotificationRef> aRef, NotificationTaskAction aAction)
...
protected:
  ...
  NotificationTaskAction mAction;
};
...
Notification::Notification(nsIGlobalObject* aGlobal, const nsAString& aID,
  ...
  : DOMEventTargetHelper(),
    ...
    mTag(aTag), mIconUrl(aIconUrl), mImageUrl(aImageUrl), mBehavior(aBehavior),
    mRequireInteraction(aRequireInteraction), mActions(aActions),
    mSilent(aSilent), mData(JS::NullValue()), mIsClosed(false),
                          ...
                          ErrorResult& aRv)
{
  ...
  if (aOptions.mActions.Length()) {
    aRv.ThrowTypeError<MSG_NOTIFICATION_ACTIONS_NOT_IN_SERVICEWORKER>();
    return nullptr;
  }

  if (aOptions.mSilent) {
    if (aOptions.mMozbehavior.mSoundFile.Length() ||
        aOptions.mMozbehavior.mVibrationPattern.WasPassed()) {
          aRv.ThrowTypeError<MSG_NOTIFICATION_SILENT_DENIED>();
          return nullptr;
        }
  }
  ...
  ErrorResult& aRv)
{
  ...
  if (!actions.EqualsLiteral("[]")) {
    ...
    if (JS_ParseJSON(cx, actions.get(), actions.Length(), &json)) {
      uint32_t length;
      JS::Rooted<JSObject*> obj(cx, json.toObjectOrNull());
      ...
      if (JS_GetArrayLength(cx, obj, &length)) {
        ...
        for (uint32_t i = 0; i < length; ++i) {
          ...
          action.Init(cx, elt, "Value", false);
          nsAutoString t;
          action.ToJSON(t);
          options.mActions.AppendElement(action, mozilla::fallible);
        }
      }
    }
  }
...
nsresult
NotificationActionsToString(const nsTArray<NotificationAction>& aArray,
...
{
  // Also remove corresponding entry from Observer service for the two registered
  // topics before the Notification object is released. The purpose is to save the
  // memory usage of Observer Service as much as possible.
  // Although the observer entries with weak reference can be cleaned out after
  // NotifyObservers() or EnumerateObservers() is called in normal cases, they
  // actually never be removed if these two functions are not called in some abnormal
  // cases.
  if (!mWorkerPrivate) {
    nsCOMPtr<nsIObserverService> obs = mozilla::services::GetObserverService();
    if (obs) {
...
public:
  ...
  ServiceWorkerNotificationObserver(const nsAString& aScope,
    ...
    , mImage(aImage), mData(aData), mBehavior(aBehavior)
    , mRequireInteraction(aRequireInteraction), mActions(aActions)
    , mSilent(aSilent)
  {
    AssertIsOnMainThread();
                                           ...
                                           const char16_t* aData)
{
  ...
  const char16_t* defaultAction = MOZ_UTF16("");
  nsDependentString userAction(aData ? aData : defaultAction);
  ...
  if (!strcmp("alertclickcallback", aTopic)) {
    rv = swm->SendNotificationEvent(NS_LITERAL_STRING("notificationclick"),
                                    originSuffix,
  ...
  if (!strcmp("alertfinished", aTopic)) {
    ...
    rv = swm->SendNotificationEvent(NS_LITERAL_STRING("notificationclose"),
                                    originSuffix,
                                    NS_ConvertUTF16toUTF8(mScope),
                                    mID,
                                    mTitle,
  ...
  if (appNotifier) {
    ...
    if (appId != nsIScriptSecurityManager::UNKNOWN_APP_ID) {
      ...
      if (NS_SUCCEEDED(rv)) {
        ...
        ops.mRequireInteraction = mRequireInteraction;
...
nsresult
Notification::ResolveIconImageAndSoundURL(nsString& iconUrl,
                                          nsString& imageUrl,
                                          nsString& soundUrl)
{
  AssertIsOnMainThread();
  ...
  if (baseUri) {
    ...
    if (mImageUrl.Length() > 0) {
      nsCOMPtr<nsIURI> srcUri;
      rv = NS_NewURI(getter_AddRefs(srcUri), mImageUrl, charset, baseUri);
      if (NS_SUCCEEDED(rv)) {
        nsAutoCString src;
        srcUri->GetSpec(src);
...
/* static */ uint32_t
Notification::MaxActions(const GlobalObject& global)
{
  if (!gDOMMaxActions) {
    gDOMMaxActions = std::min(kMaxNotificationMaxActions,
                              Preferences::GetUint(kPrefNotificationMaxActions));
  }
...
void
Notification::GetActions(nsTArray<NotificationAction>& aRetVal) const
{
  aRetVal = mActions;
}
                                         ...
                                         ErrorResult& aRv)
{
  ...
  if (aOptions.mSilent) {
    if (aOptions.mMozbehavior.mSoundFile.Length() ||
        aOptions.mMozbehavior.mVibrationPattern.WasPassed()) {
          ErrorResult result;
          result.ThrowTypeError<MSG_NOTIFICATION_SILENT_DENIED>();
          p->MaybeReject(result);
          return p.forget();
    }
  }
