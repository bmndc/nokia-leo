...
/* static */
bool
BrowserElementParent::DispatchSpatialNavigationChangedEvent(
  dom::Element* aFrameElement,
  bool aEnabled)
{
  AutoJSAPI jsapi;
  if (!jsapi.Init(aFrameElement->OwnerDoc()->GetScopeObject())) {
    return false;
  }
  JSContext *cx = jsapi.cx();
  JS::Rooted<JS::Value> detail(cx);

  if (!ToJSValue(cx, aEnabled, &detail)) {
    return false;
  }

  nsEventStatus status = nsEventStatus_eIgnore;
  bool dispatchSucceeded =
    DispatchCustomDOMEvent(aFrameElement,
                           NS_LITERAL_STRING("mozbrowserspatialnavigationchanged"),
                           cx, detail, &status);
