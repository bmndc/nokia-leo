...
BrowserElementChild.prototype = {
  ...
  _init: function() {
   ...
   addEventListener('DOMContentLoaded',
    ...
    let mmCalls = {
      ...
      "scroll-to-top": this._recvScrollToTop,
      "scroll-to-bottom": this._recvScrollToBottom,
  ...
  _recvExitFullscreen: function() {
    let fullscreenForceLandscape =
      Services.prefs.getBoolPref('media.video.fullscreen.force-landscape');
    if (fullscreenForceLandscape && this._orientationChanged) {
      let window = content.document.defaultView;
  ...
  _ClickHandler: function(e) {
    ...
    try {
      if (node) {
        ...
        let action = appsService.getDeepLinks(node.href, appId);
        if (action) {
          let window = e.target.ownerDocument.defaultView;
          let openLink = new window.MozActivity({
            name: action,
            data: {
              type: "url",
              url: node.href
          ...
          openLink.onsuccess = function () {
            debug('Launch youtube activity success!');
          };
          openLink.onerror = function (event) {
            debug('Unable to launch youtube activity!' + event.error);
          };
          //prevent continue loading the link.
          e.preventDefault();
        }
      }
    } catch (err) {}
  ...
  _recvScrollToTop: function(data) {
    debug("Received scroll-to-top");
    try {
      let window = content.document.defaultView;
      window.scrollTo({top: 0});
    } catch(e) {
      log("_recvScrollToTop catch " + e);
    }
  },

  _recvScrollToBottom: function(data) {
    debug("Received scroll-to-bottom");
    try {
      let window = content.document.defaultView;
      window.scrollTo({top: window.scrollMaxY});
    } catch(e) {
      log("_recvScrollToBottom catch " + e);
    }
  },
