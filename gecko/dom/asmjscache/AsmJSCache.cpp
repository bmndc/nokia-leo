...
private:
  ...
  DispatchToIOThread()
  {
    ...
    if (NS_WARN_IF(asmjscache::Client::IsShuttingDownOnBackgroundThread()) ||
        IsActorDestroyed()) {
      Fail();
      return;
    }
...
ParentRunnable::Run()
{
  ...
  switch (mState) {
    case eInitial: {
      ...
      if (NS_WARN_IF(asmjscache::Client::IsShuttingDownOnNonBackgroundThread()) ||
          !OperationMayProceed()) {
        FailOnNonOwningThread();
        return NS_OK;
      }
...
private:
  ...
  RecvOnOpenMetadataForRead(const Metadata& aMetadata) override
  {
    ...
    if (FindHashMatch(aMetadata, mReadParams, &moduleIndex)) {
      ...
      SendSelectCacheFileToRead(JS::AsmJSCache_InternalError);
