...
static mozilla::LazyLogModule gVideoOffloadPlayerLog("VideoOffloadPlayer");
#endif

#define LOG(type, msg) MOZ_LOG(gMediaElementLog, type, msg)
#define LOG_EVENT(type, msg) MOZ_LOG(gMediaElementEventsLog, type, msg)
#ifdef MOZ_WIDGET_GONK
#define VIDEO_OFFLOAD_LOG(type, msg) MOZ_LOG(gVideoOffloadPlayerLog, type, msg)
#endif
...
namespace dom {
 #ifdef MOZ_WIDGET_GONK
  class MediaSurfaceControlBack : public IDOMSurfaceControlCallback
  {
  public:
    MediaSurfaceControlBack(HTMLMediaElement* aHTMLMediaElement, int16_t aWidth, int16_t aHeight);
    ~MediaSurfaceControlBack() { }
  ...
  private:
    ...
    HTMLMediaElement* mHTMLMediaElement;
    uint16_t mWidth;
    uint16_t mHeight;
  ...
  MediaSurfaceControlBack::MediaSurfaceControlBack(HTMLMediaElement* aHTMLMediaElement, int16_t aWidth, int16_t aHeight)
    : mProducer(nullptr),
      mHTMLMediaElement(aHTMLMediaElement),
      mWidth(aWidth),
      mHeight(aHeight)
  {
  }
  ...
  void
  MediaSurfaceControlBack::OnProducerCreated(android::sp<android::IGraphicBufferProducer> aProducer)
  {
    VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("OnProducerCreated: %p", aProducer.get()));
    mProducer = aProducer;
    setProducer(mProducer);
  }
  
  void
  MediaSurfaceControlBack::OnProducerDestroyed()
  {
    VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("OnProducerDestroyed"));
    mProducer = nullptr;
    setProducer(nullptr);
  }
  
  void
  MediaSurfaceControlBack::setProducer(android::sp<android::IGraphicBufferProducer> aProducer)
  {
    VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("MediaSurfaceControlBack::setProducer"));
    mHTMLMediaElement->SetProducer(aProducer);
  }
  
  void
  MediaSurfaceControlBack::Shutdown()
  {
  }
#endif
...
void HTMLMediaElement::AbortExistingLoads(bool aClearLoadingSrc)
{
#ifdef MOZ_EME
...
void HTMLMediaElement::DoLoad(bool aClearLoadingSrc)
{
  if (mIsRunningLoadMethod) {
    return;
  }
  ...
  AbortExistingLoads(aClearLoadingSrc);
...
void HTMLMediaElement::SelectResource()
{
  if (!mSrcAttrStream && !HasAttr(kNameSpaceID_None, nsGkAtoms::src) &&
      !HasSourceChildren(this) && !mLoadingSrc) {
  ...
  if (mSrcAttrStream) {
    SetupSrcMediaStreamPlayback(mSrcAttrStream);
             ...
             (!HasSourceChildren(this) && (mLoadingSrc != nullptr))) {
    nsCOMPtr<nsIURI> uri;
    nsresult rv = NewURIFromString(src, getter_AddRefs(uri));
    if (NS_SUCCEEDED(rv) || (mLoadingSrc != nullptr)) {
...
{
#ifdef MOZ_WIDGET_GONK
  VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("%s, aTime:%lf", __func__, aTime));
#endif
...
#ifdef MOZ_WIDGET_GONK
  if (mSurfaceCallback) {
    mSurfaceCallback->Shutdown();
    delete mSurfaceCallback;
    mSurfaceCallback = nullptr;
  }
#endif
  mLoadingSrc = nullptr;
...
#ifdef MOZ_WIDGET_GONK
void HTMLMediaElement::SetProducer(android::sp<android::IGraphicBufferProducer>& aProducer) {
  mProducer = aProducer;
  MediaOmxCommonDecoder* mediaplayerDecoder = static_cast<MediaOmxCommonDecoder*>(mDecoder.get());
  mediaplayerDecoder->SetGraphicBufferProducer(mProducer);
};
#endif
...
#ifdef MOZ_WIDGET_GONK
  //Check if using VideoOffloadPlayer
  if (!IsVideoOffloadDisabled()) {
    if (DecoderTraits::IsVideoOffloadPlayerSupportedType(mimeType) &&
        DoesPricipalAllowVideoOffload()) {
      SetDecoder(decoder);
      if (GetFDFromBlob(decoder)) {
        if (!SetupVideoOffloadSurface()) {
          ShutdownDecoder();
          return NS_ERROR_FAILURE;
        }
      }
...
#ifdef MOZ_WIDGET_GONK
    if (IsVideoOffloadPlayer()) {
      //In VideoOffloadPlayer case, we need to add the VideoFrameContainer to SurfaceControlStream 
                                      ...
                                      nsAutoPtr<const MetadataTags> aTags)
{
  ...
  if (IsVideo() && aInfo->HasVideo()) {
    ...
    NotifyOwnerDocumentActivityChangedInternal();
#ifdef MOZ_WIDGET_GONK
    //Update mSurfaceControl size if there is one.
    if (mSurfaceControl) {
      mSurfaceControl->SetDataSourceSize(mMediaInfo.mVideo.mDisplay.width, 
...
void HTMLMediaElement::Error(uint16_t aErrorCode)
{
  ...
  // Error occurred, reset mPaused to initial value
  mPaused = true;
...
#ifdef MOZ_WIDGET_GONK
  if (IsVideoOffloadPlayer()) {
    //VideoOffloadPlayer case, force HAVE_ENOUGH_DATA as the video data is provided by VideoOffloadPlayer.
...
void HTMLMediaElement::SuspendOrResumeElement(bool aPauseElement, bool aSuspendEvents)
{
  ...
  if (aPauseElement != mPausedForInactiveDocumentOrChannel) {
    ...
    if (aPauseElement) {
      ...
      // to do on shutdown.
      if (mMediaKeys) {
        // adobe prime time may need to shutdown MediaKeys
        // However we don't support adobe right now
        // Just leave this code.
...
HTMLMediaElement::NotifyOwnerDocumentActivityChangedInternal()
{
  ...
  bool pauseElement = !IsActive() || (IsVideo() && HasVideo() && ownerDoc->Hidden());
...
#ifdef MOZ_WIDGET_GONK
nsresult
HTMLMediaElement::OnRemoteFileOpenComplete(nsresult aOpenStatus)
{
  VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("%s", __func__));
  if (NS_SUCCEEDED(aOpenStatus)) {
    PRFileDesc *fd;
    mRemoteFile->OpenNSPRFileDesc(PR_RDONLY, 0, &fd);
    if (!fd) {
      VIDEO_OFFLOAD_LOG(LogLevel::Error, ("OpenNSPRFileDesc failed !!!! \n"));
    } else {
      mRealFd = PR_FileDesc2NativeHandle(fd);  
      VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("real fd is %d \n", mRealFd));
      MediaOmxCommonDecoder* mediaplayerDecoder = static_cast<MediaOmxCommonDecoder*>(mDecoder.get());
      mediaplayerDecoder->SetFD(mRealFd);
...
bool 
HTMLMediaElement::IsVideoOffloadPlayer() const
{
  return (mDecoder && mSrcStream);
}

bool 
HTMLMediaElement::IsVideoOffloadDisabled() const
{
  char property[255];
  property_get("media.disableVideoOffload.on", property, "1");
  return (atoi(property) != 0);
}

bool 
HTMLMediaElement::DoesPricipalAllowVideoOffload() const
{
  nsIPrincipal* principal = NodePrincipal();
                   
  if(principal) {
    uint16_t status = principal->GetAppStatus();
    VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("%s, AppStatus: %d", 
                      __func__, status));
...
bool 
HTMLMediaElement::GetFDFromBlob(MediaDecoder* aDecoder)
{
  DOMString temp;
  GetItemValueText(temp);

  RefPtr<BlobImpl> blob;
  nsAutoCString url_source;
  LossyCopyUTF16toASCII(temp.AsAString(), url_source);
  VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("%s, url_source:%s", __func__, url_source.get()));

  //Only use VideoOffloadPlayer for local video file.
  if (url_source.Find("blob:", true, 0, 5) == -1) {
    VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("%s, Not a local file.", __func__));
    return false;
  }

  NS_GetBlobForBlobURISpec(url_source, getter_AddRefs(blob));

  if (!blob) {
    VIDEO_OFFLOAD_LOG(LogLevel::Warning, ("%s, Cannot get blob.", __func__));
    return false;
  }

  if (XRE_IsParentProcess()) {
    VIDEO_OFFLOAD_LOG(LogLevel::Warning, ("%s, Is ParentProcess", __func__));
    return false;
  }
  ...
  blob->GetMozFullPathInternal(file, ret);

  if (ret.Failed()) {
    VIDEO_OFFLOAD_LOG(LogLevel::Warning, ("%s, Cannot get file path from blob, err:%d", __func__, ret.ErrorCodeAsInt()));
    return false;
  }
  
  RefPtr<RemoteOpenFileChild> remoteFile = new RemoteOpenFileChild();
  nsCOMPtr<nsIURI> uri;
  nsAutoCString realFile;
  realFile.Append((char*)"file://");
  realFile.Append(ToNewUTF8String(file));
  NS_NewURI(getter_AddRefs(uri), realFile);
  uri->SetScheme(NS_LITERAL_CSTRING("remoteopenfile"));
  rv = remoteFile->Init(uri, nullptr);
  if (NS_FAILED(rv)) {
    VIDEO_OFFLOAD_LOG(LogLevel::Error, ("%s, RemoteFile Init failed", __func__));
    return false;
  }

  rv = remoteFile->AsyncRemoteFileOpen(PR_RDONLY, this, nullptr, nullptr);
  if (NS_FAILED(rv)) {
      VIDEO_OFFLOAD_LOG(LogLevel::Error, ("%s, AsyncRemoteFileOpen failed", __func__));
      return false;
  }
  mRemoteFile = remoteFile;
  
  MediaOmxCommonDecoder* mediaplayerDecoder = static_cast<MediaOmxCommonDecoder*>(aDecoder);
  mediaplayerDecoder->SetNoVideoOffloadDueToNoBlobSource(false);
  VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("Try to get FD!!"));

  return true;
}

bool 
HTMLMediaElement::SetupVideoOffloadSurface()
{
  //Get SurfaceControl
  nsCOMPtr<nsIGlobalObject> global =
    do_QueryInterface(OwnerDoc()->GetInnerWindow());
  if (!global) {
    VIDEO_OFFLOAD_LOG(LogLevel::Error, ("Failed to get innerWindow for surface"));
    return false;
  }
  ...
  if (rv.Failed()) {
    VIDEO_OFFLOAD_LOG(LogLevel::Error, ("Failed to create promise for surface"));
    return false;
  }

  HTMLMediaElement* mediaElement = this;
  MediaSurfaceControlBack* callback = new MediaSurfaceControlBack(mediaElement, 1280, 720);
  SurfaceConfiguration options;
  //Default size, will be updated by mDecoder after the size information is retrieved.
  options.mPreviewSize.mWidth = 360;
  options.mPreviewSize.mHeight = 240;
  RefPtr<nsDOMSurfaceControl> control = 
    new nsDOMSurfaceControl(options, promise, OwnerDoc()->GetInnerWindow(), callback);

  mSurfaceCallback = callback;
  mSurfaceControl = control;
  SetupSrcMediaStreamPlayback(mSurfaceControl);
  VIDEO_OFFLOAD_LOG(LogLevel::Debug, ("Set SurfaceControl(%p) to VideoOffloadPlayer",  
                                      mSurfaceControl.get()));
  return true;
}
