...
ImsRegIPCService::GetSingleton()
{
  ...
  RefPtr<ImsRegIPCService> service = gImsRegServiceSingleton;
  ...
  *aHandler = nullptr;
  ...
  if (gImsRegEnabledServiceIds.IndexOf(aServiceId) !=
    ...
    if (!mHandlers[aServiceId]) {
      RefPtr<ImsRegistrationChild> child = new ImsRegistrationChild(aServiceId);
    ...
    RefPtr<nsIImsRegHandler> handler(mHandlers[aServiceId]);
