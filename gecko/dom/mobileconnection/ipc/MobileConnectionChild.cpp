...
{
  return SendRequest(StopNetworkScanRequest(), aCallback) ? NS_OK : NS_ERROR_FAILURE;
}
...
MobileConnectionChild::GetCallForwarding(uint16_t aReason,
                                         ...
                                         nsIMobileConnectionCallback* aCallback)
{
  return SendRequest(GetCallForwardingRequest(aReason, aServiceClass), aCallback)
    ? NS_OK : NS_ERROR_FAILURE;
}
...
bool
MobileConnectionChild::RecvNotifyModemRestart(const nsString& aReason)
{
  for (int32_t i = 0; i < mListeners.Count(); i++) {
