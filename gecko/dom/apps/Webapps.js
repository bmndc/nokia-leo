...
WebappsApplication.prototype = {
  ...
  get role() {
    let value = this._proxy.role;
    return (value === undefined ? "" : value);
  },

  get preinstalled() {
    let value = this._proxy.preinstalled;
...
WebappsApplicationMgmt.prototype = {
  ...
  init: function(aWindow, aHasFullMgmtPrivilege) {
    ...
    this.initDOMRequestHelper(aWindow, ["Webapps:Uninstall:Return:OK",
                                        ...
                                        "Webapps:ClearAllLocalStorage:Return:KO",
                                        "Webapps:ClearAllLocalStorage:Return:OK",
  ...
  clearAllLocalStorage: function() {
    let request = this.createRequest();
    let principal = this._window.document.nodePrincipal;
    cpmm.sendAsyncMessage("Webapps:ClearAllLocalStorage", {
      oid: this._id,
      topId: this._topId,
      requestID: this.getRequestId(request)
    }, null, principal);
  ...
  receiveMessage: function(aMessage) {
    ...
    switch (aMessage.name) {
      ...
      case "Webapps:GetIcon:Return":
        ...
        break;
      case "Webapps:ClearAllLocalStorage:Return:KO":
        Services.DOMRequest.fireError(req, msg.error);
        break;
      case "Webapps:ClearAllLocalStorage:Return:OK":
        Services.DOMRequest.fireSuccess(req, null);
        break;
      case "Activities:Get:OK":
        {
          this.removeMessageListeners(["Activities:Get:OK",
                                       "Activities:Get:KO"]);
          req.resolve(Cu.cloneInto(msg.results.options, this._window));
        }
        break;
