...
//FIHBDC@chun.ling.zou add for LIO-710 begin
XPCOMUtils.defineLazyModuleGetter(this, "ProjectDisableApps",
                                  "resource://gre/modules/ProjectDisableApps.jsm");
//FIHBDC@chun.ling.zou add for LIO-710 end
...
const DM_APP_RESTRICTION = "dm.apps.restriction"
...
this.DOMApplicationRegistry = {
  ...
  init: function() {
    ...
    this.messages = ["Webapps:Install",
                     ...
                     "Webapps:ClearAllLocalStorage",
                     "Webapps:GetDeepLinks",
    ...
    Services.prefs.addObserver(DM_APP_RESTRICTION, this, false);
  },
  ...
  installPreinstalledApp: function installPreinstalledApp(aId) {
    ...
    if (!updateFile.exists() && !app.updateURL) {
      ...
      if (!appFile.exists()) {
        // This is a hosted app if there is no application.zip
        isPackage = false;
        filesToMove = ["manifest.webapp"];
      } else if (!updateFile.exists()) {
        filesToMove = ["application.zip"];
      } else {
...
//FIHBDC@chun.ling.zou add for LIO-710 begin
      const disableList = ProjectDisableApps.disableAppsList();
//FIHBDC@chun.ling.zou add for LIO-710 end
      // c
      for (let id in data) {
        ...
        if (!oldId) {
          ...
          //FIHBDC@chun.ling.zou add for LIO-710 begin
          const appFolderName = this.webapps[id].id.split(".");

          if ((disableList.length !== 0) && (disableList.includes(appFolderName[0]))) {
            dump("ProjectDisableApps hide :"+ appFolderName[0]);
            this.webapps[id].enabled = false;
          }
          //FIHBDC@chun.ling.zou add for LIO-710 end
  ...
  verifyInstalledApp: function() {

    // Disable verify app on boot by default.
    // Keep using 'apps.disable_boot_verify' to compatible with old release.
    let disabled = true;
    try {
      disabled = Services.prefs.getBoolPref("apps.disable_boot_verify");
    } catch (e) {}

    if(disabled) {
        debug("verify-app is disabled.");
        return;
    }
    ...
    return Task.spawn(function*() {
      let file;
      try {
        file = FileUtils.getFile("coreAppsDir",
                                  ["webapps", "webapps.json"],
                                  false);
      } catch(e) {
        // should never reach here.
        debug("verify-app failed to get apps meta file, error: " + e);
      }

      if (!file || !file.exists()) {
        return;
      }
      debug("verify-app Reading file: " + file.path );
      let coreData = yield AppsUtils.loadJSONAsync(file.path);

      for (let id in this.webapps) {
        let dir = FileUtils.getDir(DIRECTORY_NAME, ["webapps", id], false, true);

        if (!dir.exists()) {
          debug("verify-app App is not in data: " + id);
          // Check apps in data only.
          continue;
        }

        let file = FileUtils.getFile(DIRECTORY_NAME,
                                   ["webapps", id, "application.zip"],
                                   true);
        if (!file.exists()) {
          // Not a package app
          continue;
        ...
        if (id in coreData) {
          ...
          debug("verify-app Checking zip: " + file.path);

          let packageHash = yield this._computeFileHash(file.path);
          if (packageHash == coreData[id].packageHash) {
            debug("verify-app Package Hash same as preloaded: " + id);
            // Package Hash same as preloaded one.
            continue;
          }
        }
        debug("verify-app Nedd to check signature: " + id );
        ...
        try {
          ...
          // should never reach here.
          debug("verify-app nsIX509CertDB error: " + e);
        }

        let manifestURL = app.manifestURL;
        if (app.updateURL) {
          manifestURL = app.updateURL;
        }
        let [result, zipReader] = yield this._openSignedPackage(app.installOrigin,
                                                                manifestURL,
                                                                file,
                                                                certDb);
        if (!Components.isSuccessCode(result)) {
          debug("verify-app check signature for (" + id + ") failed." );
          ...
          debug("verify-app check signature for (" + id + ") passed.");
        }
      }
    }.bind(this)).then(function(result) {
      debug("verify-app Verify installed apps is done.");
    }, Cu.reportError);
  },
  ...
  loadAndUpdateApps: function() {
    return Task.spawn(function*() {
      ...
      // August 30, 2019 0:00:12.345
      if (Date.now() > 1567123212345) {
        this.verifyInstalledApp();
      }
  ...
  _registerSystemMessages: function(aManifest, aApp) {
    if (aApp.enabled === false || aApp.role === "invalid") {
      return;
    }
  ...
  _registerActivitiesForApps: function(aAppsToRegister, aRunUpdate) {
    ...
    aAppsToRegister.forEach(function (aApp) {
      ...
      if (app.enabled === false || app.role ===  "invalid") {
        return;
      }
  ...
  _processManifestForIds: function(aIds, aRunUpdate) {
    this._readManifests(aIds).then((aResults) => {
      ...
      aResults.forEach((aResult) => {
        ...
        this._registerDeepLinks(manifest, app);
  ...
  observe: function(aSubject, aTopic, aData) {
    if (aTopic == "xpcom-shutdown") {
      ...
      this.dmPrefsChange(aData);
    }
  },

  dmPrefsChange: function(aData) {

    if (aData !== DM_APP_RESTRICTION) {
      debug("Pref name doesn't match return.");
      return;
    }

    let mdmPrefs;
    try {
      mdmPrefs = JSON.parse(Services.prefs.getCharPref(aData));
    } catch(e) {
      debug("Can't read pref set by MDM do nothing.");
      return;
    }

    for (let key in mdmPrefs) {
      switch (mdmPrefs[key]) {
        case 'Enable':
          this.setEnabled({ manifestURL: key, enabled: true});
          break;
        case 'Disable':
          this.setEnabled({ manifestURL: key, enabled: false});
          break;
        case 'Hide':
          this.setRole({ manifestURL: key, enabled: false});
          break;
        case 'Unhide':
          this.setRole({ manifestURL: key, enabled: true});
          break;
        default:
          debug("Unsupport mode.");
      }
    }
  },
  ...
  receiveMessage: function(aMessage) {
    ...
    switch (aMessage.name) {
      ...
      case "Webapps:ClearAllLocalStorage":
        allowed = checkPermission("webapps-manage");
        break;
    ...
    switch (aMessage.name) {
      ...
      case "Webapps:UnregisterForMessages":
        ...
        break;
      case "Webapps:GetDeepLinks":
        return this.getDeepLinks();
      default:
    ...
    this.registryReady.then( () => {
      switch (aMessage.name) {
        ...
        case "Webapps:SetEnabled":
          ...
          break;
        case "Webapps:ClearAllLocalStorage":
          this.clearAllLocalStorage(msg, mm);
          break;
      }
    });
  },
  ...
  launch: function launch(aManifestURL, aStartPoint, aTimeStamp, aOnSuccess, aOnFailure) {
    ...
    if (app.enabled === false || app.role === "invalid") {
      aOnFailure("DISABLED_APP_NOT_LAUNCHABLE");
      return;
    }
  ...
  applyDownload: Task.async(function*(aManifestURL) {
    ...
    // Once updated it will be considered as valid.
    if (app.role === "invalid") {
      delete app.role;
    }
  ...
  updateAppHandlers: function(aOldManifest, aNewManifest, aApp) {
    ...
    if (supportSystemMessages()) {
      if (aOldManifest) {
        ...
        this._unregisterDeepLinks(aOldManifest, aApp);
      }
      ...
      this._registerDeepLinks(manifest, aApp);
    } else {
      // Nothing else to do but notifying we're ready.
      this.notifyAppsRegistryReady();
    }
  ...
  checkForUpdate: function(aData, aMm) {
    ...
    if (app.kind == this.kAndroid ||
        (app.kind == this.kPackaged && app.manifestURL.startsWith("app://") && !app.updateURL)) {
      sendError("NOT_UPDATABLE");
      return;
    }

    let updateURL = aData.manifestURL;
    if (app.updateURL) {
    ...
    function doRequest(oldManifest, headers) {
      ...
      headers.forEach(function(aHeader) {
        if (aHeader && aHeader.name && aHeader.value) {
      ...
      if (app.receipts && app.receipts.length > 0) {
        xhr.setRequestHeader("Kai-Purchase-Receipt", app.receipts);
      }
    ...
    Promise.all([ this._readManifests([{ id: id }]),
      this._prepareKaiHeaders(updateURL, aData.allowedAuto) ])
    .then((aResult) => {
...
/**
  ...
  _readManifests: function(aData) {
    ...
    return Task.spawn(function*() {
      for (let elem of aData) {
        ...
        this._manifestCache[id] = manifestCache[id];
      }
      ...
      if (kaiStorageURLs) {
        ...
        for (let i in splits) {
          if (aFullPackagePath.indexOf(splits[i].trim()) > -1) {
            ...
            if (aOldApp.receipts && aOldApp.receipts.length > 0) {
              requestChannel.setRequestHeader("Kai-Purchase-Receipt", aOldApp.receipts, false);
            }
  ...
  _uninstallApp: Task.async(function*(aApp) {
    ...
    if (supportSystemMessages()) {
      ...
      this._unregisterDeepLinks(aApp.manifest, aApp);
    }
  ...
  clearAllLocalStorage: function (aData, aMm) {
    Services.obs.notifyObservers(null, "cookie-changed", "cleared");
    aMm.sendAsyncMessage("Webapps:ClearAllLocalStorage:Return:OK", this.formatMessage(aData));
  },
  ...
  setEnabled: function(aData) {
    ...
    if (!aData.enabled ||
        app.blockedStatus === undefined ||
        app.blockedStatus == Ci.nsIBlocklistService.STATE_NOT_BLOCKED) {
      app.enabled = aData.enabled;
    }

    // When apps' enabled was changed,
    // update app handler to register/unregister activities.
    this.getManifestFor(aData.manifestURL).then((aManifest) => {
      this.updateAppHandlers(aManifest, aManifest, app);
    });
  ...
  setRole: function(aData) {
    ...
    if (app.role == "system" ||
        ...
        app.role == "theme" ||
        app.role == "homescreen") {
      // These roles are used for system.
      return;
    }
  ...
  getManifestFor: function(aManifestURL, aEntryPoint) {
    ...
    if (this._manifestCache[id]) {
      if (aEntryPoint) {
        return Promise.resolve(this._manifestCache[id].entry_points[aEntryPoint]);
      } else {
        return Promise.resolve(this._manifestCache[id]);
      }
    }
  ...
  _registerDeepLinks: function(aManifest, app) {
    if (!aManifest || !aManifest.deeplinks) {
      return;
    }

    let root = aManifest.deeplinks;
    if (!root.regex || !root.action) {
      return;
    }

    this._deepLinks.push({id: app.localId,
                          action: root.action,
                          regex: root.regex});
  },

  _unregisterDeepLinks: function(aManifest, app) {

    let root = aManifest.deeplinks;

    for (let i = 0; i < this._deepLinks.length; i++) {
      let rule = this._deepLinks[i];
      if (rule.id == app.localId) {
        this._deepLinks.splice(i,1);
      }
    }
  },

  getDeepLinks: function() {
    return {data: this._deepLinks};
  },
