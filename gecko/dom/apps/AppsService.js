...
AppsService.prototype = {
  ...
  getDeepLinks: function getDeepLinks(url, appId) {
    debug("getDeepLinks");
    let links = DOMApplicationRegistry.getDeepLinks();
    if (links.data) {
      for (let i = 0; i < links.data.length; i++) {
        let link = links.data[i];
        // If the link is clicked from the register app,
        // do no need to take action for it.
        if (appId != Ci.nsIScriptSecurityManager.NO_APP_ID &&
            appId == link.id) {
          continue;
        }
        if (!link.regex || !link.action) {
          continue;
        }
        let linkReg = RegExp(link.regex);
        if (linkReg.exec(url)) {
          return link.action;
        }
      }
