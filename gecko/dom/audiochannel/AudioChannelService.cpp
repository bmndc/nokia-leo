...
AudioChannelService::UnregisterAudioChannelAgent(AudioChannelAgent* aAgent)
{
  ...
  if (winData->mAgents.Contains(aAgent)) {
    ...
    if (winData->mChannels[channel].mNumberOfAgents == 0) {
      // After NotifyChannelActive() is called, system APP will set this audio
      // channel to muted, but there will be a delay. If a new agent with same
      // the channel is registered during this time gap, it will initially be
      // unmuted, and then muted again due to the delay. After that, system APP
      // may unmute this channel because new agent is active. This behavior is
      // really annoying, so here we directly mute the channel when last agent
      // is unregistered, and new agent will be initially muted.
      winData->mChannels[channel].mMuted = true;
...
bool
AudioChannelService::AnyAudioChannelIsActive(bool aCheckChild)
{
  nsTObserverArray<nsAutoPtr<AudioChannelWindow>>::ForwardIterator iter(mWindows);
  while (iter.HasMore()) {
    AudioChannelWindow* next = iter.GetNext();
  ...
  if (IsParentProcess() && aCheckChild) {
