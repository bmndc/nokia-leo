...
MozMtpDatabase::RemoveEntry(MtpObjectHandle aHandle)
{
  ...
  if (removedEntry && (removedEntry->mObjectFormat != MTP_FORMAT_ASSOCIATION)) {
    return;
  }
                             ...
                             MtpObjectHandle aParent)
{
  ...
  PRDirEntry* dirEntry;
  while (GetMtpConnectedStatus() && (dirEntry = PR_ReadDir(dir, PR_SKIP_BOTH))) {
...
// updateSubObjectPath is used to update the mPath of the sub folders or files when a folder was renamed
MtpResponseCode
MozMtpDatabase::updateSubObjectPath(MtpObjectHandle aHandle,
                                    nsCString newFileFullPath)
{
  RefPtr<DbEntry> parentEntry = GetEntry(aHandle);
  if (!parentEntry) {
    MTP_ERR("Invalid Handle: 0x%08x", aHandle);
    return MTP_RESPONSE_INVALID_OBJECT_HANDLE;
  }
  ...
  ProtectedDbArray::index_type tmpIdx;
  ...
  for (entryIdx = 0; entryIdx < numEntries; entryIdx++) {
    ...
    //find the sub entry
    for (tmpIdx = entryIdx, tmpEntry = mDb[entryIdx]; (MTP_PARENT_ROOT != tmpIdx) && mDb[tmpIdx]; tmpIdx = mDb[tmpIdx]->mParent) {
      if (aHandle == mDb[tmpIdx]->mParent) {
        //update the mPath of each sub entry
        MtpWatcherNotify(tmpEntry, "deleted");
        tmpEntry->mPath.Replace(0, parentEntry->mPath.Length(), newFileFullPath.get(), newFileFullPath.Length());
        MtpWatcherNotify(tmpEntry, "modified");
        MTP_DBG("sub element new path: '%s'", tmpEntry->mPath.get());
        break;
      }
    }
  }
                                       ...
                                       MtpDataPacket& aPacket)
{
  ...
  if (MTP_FORMAT_ASSOCIATION == entry->mObjectFormat) {
    MTP_DBG("folder rename operation: sub folders and files path update needed '%s'", newFileFullPath.get());
    updateSubObjectPath(aHandle, newFileFullPath);
  }
