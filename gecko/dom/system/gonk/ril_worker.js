...
RilObject.prototype = {
  ...
  initRILState: function() {
    ...
    /**
     * Weither radio is restarting
     */
    this._waitingModemRestart = false;
  ...
  iccUnlockCardLock: function(options) {
    switch (options.lockType) {
      ...
      case GECKO_CARDLOCK_RSPCK_PUK:
        options.facility = GECKO_CARDLOCK_TO_FACILITY_CODE[options.lockType];
  ...
  iccGetSubsidyLockStatus: function(options) {
  ...
  processGetSubsidyLockStatusResult: function(options) {
    options.serviceClass = this.context.Buf.readInt32List()[0];
    let isLock = false;
    if(options.serviceClass && options.serviceClass != SUBSIDY_LOCK_STATUS_NOT_SUPPORTE_QUERY) {
      isLock = true;
    }

    let option = {
      'result' : isLock,
      'rilMessageClientId' : options.rilMessageClientId,
      'rilMessageToken' : options.rilMessageToken,
      'rilMessageType' : options.rilMessageType
    };
    this.sendChromeMessage(option);
  },
  ...
  getMaxContactCount: function (options) {
    this.context.ICCContactHelper.getMaxContactCount(this.appType,
      options.contactType,
      (aTotalRecords) => {options.totalRecords = aTotalRecords;
      this.sendChromeMessage(options);},
      (aError)=>{options.errorMsg = aError;
      this.sendChromeMessage(options);});
  },
  ...
  dial: function(options) {
    ...
    this.telephonyRequestQueue.push(options.request, () => {
      ...
      //TODO: Enable rttMode for dial
      //Buf.writeInt32(options.rttMode || 0);
   ...
   *        uint32_t call id
   * @param rttMode
   *        One of RTT_MODE_* constants
   */
  sendRttModify: function(options) {
    this.telephonyRequestQueue.push(REQUEST_SEND_RTT_MODIFY, () => {
      let Buf = this.context.Buf;
      Buf.newParcel(REQUEST_SEND_RTT_MODIFY, options);
      Buf.writeInt32(options.callIndex);
      Buf.writeInt32(options.rttMode);
      Buf.sendParcel();
    });
  },
   ...
   *        uint32_t call id
   * @param rttStatus
   *        Indicate to accept rttrequest or not
   */
  sendRttModifyResponse: function(options) {
    this.telephonyRequestQueue.push(REQUEST_SEND_RTT_MODIFY_RESPONSE, () => {
      let Buf = this.context.Buf;
      Buf.newParcel(REQUEST_SEND_RTT_MODIFY_RESPONSE, options);
      Buf.writeInt32(options.callIndex);
      Buf.writeInt32(options.rttStatus);
      Buf.sendParcel();
    });
  },
   ...
   *        uint32_t call id
   * @param message
   *        String of rtt message
   */
  sendRttMessage: function(options) {
    let Buf = this.context.Buf;
    Buf.newParcel(REQUEST_SEND_RTT_MESSAGE, options);
    Buf.writeInt32(options.callIndex);
    Buf.writeString(options.message);
    Buf.sendParcel();
  },
  ...
  setCallForward: function(options) {
    this._callForwardOptions = options;
    let Buf = this.context.Buf;
  ...
  _processICCStatus: function(iccStatus) {
    ...
    // Stop process Icc Status during radio restart
    if (this._waitingRadioTech || this._waitingRadioRestart) {
      return;
    }
  ...
  _processCREG: function(curState, newState) {
    ...
    if (curState.regState === undefined || curState.regState !== regState) {
      ...
      curState.emergencyCallsOnly =
                        ...
                        regState == NETWORK_CREG_STATE_SEARCHING_EMERGENCY_CALLS ||
                        regState == NETWORK_CREG_STATE_DENIED_EMERGENCY_CALLS ||
  ...
  _setCallForwardToSim: function(options) {
    if (DEBUG) {
      this.context.debug("setCallForward to sim, options:" + JSON.stringify(options));
    }

    if (options.serviceClass & ICC_SERVICE_CLASS_VOICE == 0) {
      if (DEBUG) {
        this.context.debug("setCallForwardToSim only support ICC_SERVICE_CLASS_VOICE");
      }
      return;
    }

    // Only unconditional CF needs to set flag.
    if (options.reason != CALL_FORWARD_REASON_UNCONDITIONAL) {
      return;
    }

    let iccInfo = this.context.RIL.iccInfoPrivate;
  ...
  _cancelModemRestartTimeout: function() {
    if (this._exitModemRestartTimeoutID) {
      clearTimeout(this._exitModemRestartTimeoutID);
      this._exitModemRestartTimeoutID = null;
    }
    this._waitingModemRestart = false;
  },

  exitModemRestart: function() {
    this._cancelModemRestartTimeout();

    //To retrive ICCID after modem restart timeout.
    this.context.ICCRecordHelper.readICCID();
  },

  /**
   * Handle Unsolicited Modem Restart
   */
  _handleModemRestart: function(reason) {
    // Exit emergency callback mode when modem restart
    if (this._isInEmergencyCbMode) {
      this.exitEmergencyCbMode();
    }

    // Cancel old status first
    this._cancelModemRestartTimeout();

    // Set waitingModemRestart to avoid some unsolicited report
    this._waitingModemRestart = true;

    this._modemRestartTimeoutID = setTimeout(
      this.exitModemRestart.bind(this), MODEM_RESTART_TIMEOUT_MS);

    this.sendChromeMessage({rilMessageType: "modemrestart", reason: reason});
  },
...
RilObject.prototype[REQUEST_SEND_RTT_MODIFY] = function REQUEST_SEND_RTT_MODIFY(length, options) {
  if (DEBUG) {
    this.context.debug("REQUEST_SEND_RTT_MODIFY " + JSON.stringify(options));
  }
  this.sendChromeMessage(options);
};
RilObject.prototype[REQUEST_SEND_RTT_MODIFY_RESPONSE] = function REQUEST_SEND_RTT_MODIFY_RESPONSE(length, options) {
  if (DEBUG) {
    this.context.debug("REQUEST_SEND_RTT_MODIFY_RESPONSE " + JSON.stringify(options));
  }
  this.sendChromeMessage(options);
};
RilObject.prototype[REQUEST_SEND_RTT_MESSAGE] = function REQUEST_SEND_RTT_MESSAGE(length, options) {
  if (DEBUG) {
    this.context.debug("REQUEST_SEND_RTT_MESSAGE " + JSON.stringify(options));
  }
  this.sendChromeMessage(options);
};
...
RilObject.prototype[REQUEST_QUERY_FACILITY_LOCK] = function REQUEST_QUERY_FACILITY_LOCK(length, options) {
  ...
  if(options.isSbusidyLock) {
    this.processGetSubsidyLockStatusResult(options);
    return;
  }
...
RilObject.prototype[UNSOLICITED_RESPONSE_RADIO_STATE_CHANGED] = function UNSOLICITED_RESPONSE_RADIO_STATE_CHANGED() {
  ...
  if (radioState !== RADIO_STATE_UNAVAILABLE) {
    ...
    if (this._waitingModemRestart)
    {
      // Modem restart finish, do recovery here
      this._cancelModemRestartTimeout();
    }
  }
...
RilObject.prototype[UNSOLICITED_RESPONSE_CDMA_NEW_SMS] = function UNSOLICITED_RESPONSE_CDMA_NEW_SMS(length) {
  ...
  if (message) {
    if (message.teleservice === PDU_CDMA_MSG_TELESERVICE_ID_WAP) {
...
RilObject.prototype[UNSOLICITED_PCO_DATA] = function UNSOLICITED_PCO_DATA() {
  let Buf = this.context.Buf;
  let pco = {};
  pco.cid = Buf.readInt32();
  pco.bearerProto = Buf.readString();
  pco.pcoId = Buf.readInt32();
  pco.contents = Buf.readUint8Array();
  this.sendChromeMessage({rilMessageType: "pcochange", pco: pco});
};

RilObject.prototype[UNSOLICITED_RTT_MODIFY] = function UNSOLICITED_RTT_MODIFY() {
  let Buf = this.context.Buf;
  let info = {rilMessageType:"rttModify"};
  info.callIndex = Buf.readInt32();
  info.rttMode = Buf.readInt32();
  this.sendChromeMessage(info);
};
RilObject.prototype[UNSOLICITED_RTT_MODIFY_RESPONSE] = function UNSOLICITED_RTT_MODIFY_RESPONSE() {
  let Buf = this.context.Buf;
  let info = {rilMessageType:"rttModifyResponse"};
  info.callIndex = Buf.readInt32();
  info.rttStatus = Buf.readInt32();
  this.sendChromeMessage(info);
};
RilObject.prototype[UNSOLICITED_RTT_MESSAGE] = function UNSOLICITED_RTT_MESSAGE() {
  let Buf = this.context.Buf;
  let info = {rilMessageType:"rttMessage"};
  info.callIndex = Buf.readInt32();
  info.message = Buf.readString();
  this.sendChromeMessage(info);
};

RilObject.prototype[UNSOLICITED_MODEM_RESTART] = function UNSOLICITED_MODEM_RESTART() {
  let Buf = this.context.Buf;
  this._handleModemRestart(Buf.readString());
};
...
CdmaPDUHelperObject.prototype = {
  ...
  writeMessage: function(options) {
    ...
    // Common Header
    if (options.segmentMaxSeq > 1) {
      this.writeInt(PDU_CDMA_MSG_TELESERVICE_ID_WEMT);
    } else {
      this.writeInt(PDU_CDMA_MSG_TELESERVICE_ID_SMS);
    }
  ...
  readPBR: function(onsuccess, onerror) {
    ...
    function callback(options) {
      ...
      try {
        ...
        if (DEBUG) {
          this.context.debug("readPBR error :" + e);
        }
      }
...
SimRecordHelperObject.prototype = {
  ...
  readCphsCFF: function() {
    function callback() {
      let RIL = this.context.RIL;
      let strLen = this.context.Buf.readInt32();
      let octetLen = strLen/2;
