...
/**
 * Dispatch netd result on networkutils thread.
 */
...
#ifdef DONGLE
const CommandFunc NetworkUtils::sEnableUsbLocalTetherChain[] = {
  NetworkUtils::setConfig,
  NetworkUtils::tetherInterface,
  NetworkUtils::addInterfaceToLocalNetwork,
  NetworkUtils::addRouteToLocalNetwork,
  NetworkUtils::tetheringStatus,
  NetworkUtils::startTethering,
  NetworkUtils::dongleTetheringSuccess
};

const CommandFunc NetworkUtils::sDisableUsbLocalTetherChain[] = {
  NetworkUtils::untetherInterface,
  NetworkUtils::removeInterfaceFromLocalNetwork,
  NetworkUtils::preTetherInterfaceList,
  NetworkUtils::postTetherInterfaceList,
...
const CommandFunc NetworkUtils::sEnableUsbNatTetherChain[] = {
  NetworkUtils::enableNat,
  NetworkUtils::setIpForwardingEnabled,
  NetworkUtils::setInterfaceForwardingEnabled,
  NetworkUtils::setDnsForwarders,
...
const CommandFunc NetworkUtils::sDisableUsbNatTetherChain[] = {
  NetworkUtils::disableNat,
  NetworkUtils::setIpForwardingEnabled,
  NetworkUtils::setInterfaceForwardingDisabled,
  NetworkUtils::usbTetheringSuccess
                                  ...
                                  NetworkResultOptions& aResult)
{
  ...
  if (SDK_VERSION >= 19) {
    snprintf(command, MAX_COMMAND_SIZE - 1, "softap set %s \"%s\" %s %ld %s \"%s\"",
             GET_CHAR(mIfname),
             ssid.get(),
             GET_CHAR(mHiddenType),
             GET_FIELD(mChannel),
             GET_CHAR(mSecurity),
             key.get());
    ...
    snprintf(command, MAX_COMMAND_SIZE - 1, "softap set %s %s \"%s\" %s \"%s\" %ld 0 8",
             GET_CHAR(mIfname),
...
{
  char command[MAX_COMMAND_SIZE];
  snprintf(command, MAX_COMMAND_SIZE - 1, "network interface remove %d %s",
           GET_FIELD(mNetId), GET_CHAR(mIfname));
...
void NetworkUtils::addDefaultRouteToNetwork(CommandChain* aChain,
                                            ...
                                            NetworkResultOptions& aResult)
{
  NU_DBG("addDefaultRouteToNetwork GET_FIELD(mLoopIndex)= %d , mGateways.Length()= %d" , GET_FIELD(mLoopIndex),GET_FIELD(mGateways).Length());
  if (GET_FIELD(mLoopIndex) >= GET_FIELD(mGateways).Length()) {
    aCallback(aChain, false, aResult);
    NU_DBG("addDefaultRouteToNetwork  return.");
    return;
  }
...
void NetworkUtils::addRouteToSecondaryTable(CommandChain* aChain,
  ...
  if (SDK_VERSION >= 20) {
    ...
    if (getIpType(GET_CHAR(mGateway)) == AF_INET6) {
      NU_DBG("%s : ignore IPv6 for secondary route.", __FUNCTION__);
      next(aChain, false, aResult);
      return;
    }
    snprintf(command, MAX_COMMAND_SIZE - 1,
...
#ifdef DONGLE
void NetworkUtils::dongleTetheringSuccess(CommandChain* aChain,
                                          CommandCallback aCallback,
                                          NetworkResultOptions& aResult)
{
  postMessage(aChain->getParams(), aResult);
 ...
 : mMessageCallback(aCallback)
{
  ...
  nsresult rv = NS_NewNamedThread("NetworkUtils", getter_AddRefs(gNetworkUtilsThread));
...
#ifdef DONGLE
    BUILD_ENTRY(setUSBLocalTethering),
    BUILD_ENTRY(setUSBNatTethering),
#endif
...
CommandResult NetworkUtils::setDefaultNetwork(NetworkParams& aOptions)
{
  ...
  NU_DBG("Calling NetworkUtils::setDefaultNetwork mIfname= %s" , GET_CHAR(mIfname));
...
CommandResult NetworkUtils::setWifiTethering(NetworkParams& aOptions)
{
  ...
  // Skip v6 Dns since it handle by RDNS or DHCPv6 instead of dnsmasq
  ...
  if (enable && (!aOptions.mChannel ||
      aOptions.mChannel < 1 || aOptions.mChannel > 11)) {
...
#ifdef DONGLE
CommandResult NetworkUtils::setUSBLocalTethering(NetworkParams& aOptions)
{
  bool enable = aOptions.mEnable;
  IFProperties interfaceProperties;
  getIFProperties(GET_CHAR(mExternalIfname), interfaceProperties);
  ...
  if (enable) {
    NU_DBG("Starting USB Local Tethering on %s <-> %s",
           GET_CHAR(mInternalIfname), GET_CHAR(mExternalIfname));
    runChain(aOptions, sEnableUsbLocalTetherChain, usbTetheringFail);
  } else {
    NU_DBG("Stopping USB Tethering on %s <-> %s",
           GET_CHAR(mInternalIfname), GET_CHAR(mExternalIfname));
    runChain(aOptions, sDisableUsbLocalTetherChain, usbTetheringFail);
  }
  return CommandResult::Pending();
}

CommandResult NetworkUtils::setUSBNatTethering(NetworkParams& aOptions)
{
  bool enable = aOptions.mEnable;
  IFProperties interfaceProperties;
  getIFProperties(GET_CHAR(mExternalIfname), interfaceProperties);
  ...
  if (enable) {
    NU_DBG("Starting USB Nat Tethering on %s <-> %s",
           GET_CHAR(mInternalIfname), GET_CHAR(mExternalIfname));
    runChain(aOptions, sEnableUsbNatTetherChain, usbTetheringFail);
  } else {
    NU_DBG("Stopping USB Tethering on %s <-> %s",
           GET_CHAR(mInternalIfname), GET_CHAR(mExternalIfname));
    runChain(aOptions, sDisableUsbNatTetherChain, usbTetheringFail);
  }
  return CommandResult::Pending();
}
#endif
...
CommandResult NetworkUtils::enableUsbRndis(NetworkParams& aOptions)
{
  ...
  if (aOptions.mReport) {
    currentResult = cleanState ?
                    waitForUsbState(aOptions.mEnable, USB_FUNCTION_RNDIS) : false;

    // Evaluate current usb configuration state.
    if (aOptions.mEnable && currentResult) {
      currentResult = false;
      for (uint32_t i = 0; i < USB_FUNCTION_RETRY_TIMES; i++) {
        usleep(USB_FUNCTION_RETRY_INTERVAL * 1000);
        if (!IsUsbConfigured()) {
          continue;
        }
...
CommandResult NetworkUtils::addInterfaceToNetwork(NetworkParams& aOptions)
{
  ...
  if (GET_FIELD(mNetId) == -1) {
    ERROR("addInterfaceToNetwork no mNetId");
    return -1;
  }

  if (GET_FIELD(mIfname).IsEmpty()) {
    ERROR("addInterfaceToNetwork no mIfname");
    return -1;
  }
  ...
  ERROR("addInterfaceToNetwork no exist netID");
  return -1;
}
...
CommandResult NetworkUtils::removeInterfaceToNetwork(NetworkParams& aOptions)
{
  ...
  if (GET_FIELD(mNetId) == -1) {
    ERROR("removeInterfaceToNetwork no mNetId");
    return -1;
  }

  if (GET_FIELD(mIfname).IsEmpty()) {
    ERROR("removeInterfaceToNetwork no mIfname");
    return -1;
  }
  ...
  ERROR("removeInterfaceToNetwork no exist mIfname");
  return -1;
}
...
CommandResult NetworkUtils::setTcpBufferSize(NetworkParams& aOptions)
{
  ...
  for (uint32_t i = 0; i < length; i++) {
    ...
    if (!WriteSysFile(nsPrintfCString("%s%s", prefix, suffix[i]).get(),
                      autoTcpBuffer.get())) {
