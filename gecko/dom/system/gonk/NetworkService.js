...
NetworkService.prototype = {
  ...
  */
  setDefaultNetwork: function(aInterfaceName, aCallback) {
  ...
  setDefaultRoute: function(aInterfaceName, aCount, aGateways, aCallback) {
    ...
    debug("Going to change default route for " + aInterfaceName + " , aGateways=" + aGateways);
    let options = {
      cmd: "setDefaultRoute",
      ifname: aInterfaceName,
      gateways: aGateways
  ...
  removeDefaultRoute: function(aInterfaceName, aCount, aGateways, aCallback) {
    ...
    debug("Remove default route for " + aInterfaceName + " , aGateways=" + aGateways);
    let options = {
  ...
  // Enable/disable USB dongle by sending commands to netd.
  setDongleUSBTethering: function(aEnable, aConfig, tetherStage, aCallback) {

    if (tetherStage == "NAT_STAGE") {
      aConfig.cmd = "setUSBNatTethering";
    } else if (tetherStage == "LOCAL_STAGE") {
      aConfig.cmd = "setUSBLocalTethering";
    }
    // The callback function in controlMessage may not be fired immediately.
    this.controlMessage(aConfig, (aData) => {
      let code = aData.resultCode;
      ...
      debug(enableString + " " + aConfig.cmd + " result: Code " + code + " reason " + reason);
  ...
  addInterfaceToNetwork: function(aNetId, aInterfaceName, aCallback) {
    let params = {
  ...
  removeInterfaceToNetwork: function(aNetId, aInterfaceName, aCallback) {
    let params = {
  ...
  setTcpBufferSize: function(aTcpBufferSizes, aCallback) {
