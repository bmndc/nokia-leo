...
/**
 * dm.bluetooth.settings.ui for JIO device control bluetooth by device manager server
 * The value is "gray": can not enable bluetooth by APP
 * The value is "show": can enable bluetooth by APP
 *
 * Please see the bug 70196, for more information
 */
#define DM_BLUETOOTH_SETTING        "dm.bluetooth.settings.ui"
...
class BluetoothService::ReadDmSettingTask final : public nsISettingsServiceCallback
{
public:
  NS_DECL_ISUPPORTS
  ...
  NS_IMETHOD Handle(const nsAString& aName, JS::Handle<JS::Value> aResult)
  {
    ...
    if (!aResult.isString()) {
      BT_WARNING("Setting for '" DM_BLUETOOTH_SETTING "' is not a string!");
      return NS_OK;
    }

    nsAutoJSString valueStr;
    valueStr.init(nsContentUtils::RootingCx(), aResult.toString());

    if (valueStr.EqualsASCII("gray")) {
      BT_LOGR("Setting for '" DM_BLUETOOTH_SETTING "' is gray, the bluetooth can not enable");
      sDmDisable = true;
    } else {
      sDmDisable = false;
    }
  ...
  NS_IMETHOD HandleError(const nsAString& aName)
  {
    BT_WARNING("Unable to get value for '" DM_BLUETOOTH_SETTING "'");
    return NS_OK;
  }
...
NS_IMPL_ISUPPORTS(BluetoothService::ReadDmSettingTask, nsISettingsServiceCallback);
...
nsresult
BluetoothService::ReadDmSetting()
{
  MOZ_ASSERT(NS_IsMainThread());
  ...
  RefPtr<ReadDmSettingTask> callback = new ReadDmSettingTask();
  rv = settingsLock->Get(DM_BLUETOOTH_SETTING, callback);
  NS_ENSURE_SUCCESS(rv, rv);
...
BluetoothService::HandleSettingsChanged(nsISupports* aSubject)
{
  ...
  if (setting.mKey.EqualsASCII(DM_BLUETOOTH_SETTING)) {
    if (!setting.mValue.isString()) {
      MOZ_ASSERT(false, "Expecting a string for 'dm.bluetooth.settings.ui'!");
      return NS_ERROR_UNEXPECTED;
    }

    nsAutoJSString valueStr;
    valueStr.init(nsContentUtils::RootingCx(), setting.mValue.toString());
    if (valueStr.EqualsASCII("gray")) {
      BT_LOGR("Bluetooth can not enable, dm set value is gray");
      sDmDisable = true;
    } else {
      BT_LOGR("Bluetooth dm value: %s", NS_ConvertUTF16toUTF8(valueStr).get());
      sDmDisable = false;
    }
    return NS_OK;
  }
...
BluetoothService::EnableDisable(bool aEnable,
                                BluetoothReplyRunnable* aRunnable)
{
  BT_LOGR("Bluetooth sDmDisable: %d", sDmDisable);
  if (aEnable && sDmDisable) {
    return NS_ERROR_FAILURE;
  }
  sToggleInProgress = true;
