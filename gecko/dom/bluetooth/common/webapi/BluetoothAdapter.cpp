...
BluetoothAdapter::Disable(ErrorResult& aRv)
{
  ...
  BT_ENSURE_TRUE_REJECT(bs, promise, NS_ERROR_NOT_AVAILABLE);
  /**
   * When phone rebooted, the system app init the bluetooth manager(mozBluetooth)
   * during the bluetooth hardware enable. The adapter created and the state changed
   * before the adatper registered. So adapter state would be disabled, but the bluetooth
   * state is enabled actually(bluetoothService). In that case, system app wouldn't be
   * able to disable it.
   * Detail info please refer the bug 72652
   */
...
void
BluetoothAdapter::HandlePairingAborted(const BluetoothValue& aValue)
{
  if (NS_WARN_IF(mState != BluetoothAdapterState::Enabled)) {
    return;
  }
