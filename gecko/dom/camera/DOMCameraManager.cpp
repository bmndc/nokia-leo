...
nsDOMCameraManager::GetCameraSensorAngle(const nsAString& aCamera, ErrorResult& aRv)
{
...
#ifdef MOZ_WIDGET_GONK
  // Firstly, check if aCamera is a valid camera ID (ex: front, back).
  nsTArray<nsString> list;
  aRv = ICameraControl::GetListOfCameras(list);
  if (aRv.Failed() || !list.Contains(aCamera)) {
    aRv.Throw(NS_ERROR_FAILURE);
    return nullptr;
  }
  ...
  CameraInfo info;
  int rv = Camera::getCameraInfo(cameraId, &info);
  if (rv != 0) {
    aRv.Throw(NS_ERROR_FAILURE);
    return nullptr;
  }
  ...
  JS::Rooted<JS::Value> value(nsContentUtils::RootingCx(),
                              JS::Int32Value(info.orientation));
#else
  JS::Rooted<JS::Value> value(nsContentUtils::RootingCx(),
                              JS::Int32Value(0));
#endif
