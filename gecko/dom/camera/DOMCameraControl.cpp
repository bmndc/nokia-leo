  ...
  , mSetInitialConfig(false)
{
  ...
  if (haveInitialConfig) {
    ...
    if (NS_FAILED(rv)) {
      DOM_CAMERA_LOGE("Select preview size failed\n");
...
nsDOMCameraControl::GetFaceAnalysis(const dom::Blob& aLicense, ErrorResult& aRv) {
  RefPtr<dom::Promise> promise = CreatePromise(aRv);
  if (aRv.Failed()) {
    return nullptr;
  }

  if (mFaceAnalysisManager == nullptr) {
    if (mFrService == nullptr) {
      mFrService = std::shared_ptr<FrService>(new FrService());
    }

    RefPtr<BlobImpl> licenseImpl = aLicense.Impl();
    nsCOMPtr<nsIInputStream> stream;
    licenseImpl->GetInternalStream(getter_AddRefs(stream), aRv);
    if (NS_WARN_IF(aRv.Failed())) {
      return nullptr;
    }

    uint64_t blobSize = licenseImpl->GetSize(aRv);
    ...
    FrStatus status = mFrService->Init(blobSize, bufferData.get());
    if (status == FrStatus::STATUS_OK) {
      mFaceAnalysisManager = new DOMFaceAnalysisManager(GetParentObject());
      mFaceAnalysisManager->SetFrService(mFrService);
      mInput->SetFaceAnalysisManager(mFaceAnalysisManager);
    }
  }

  if (mFaceAnalysisManager) {
    promise->MaybeResolve(mFaceAnalysisManager);
  }
