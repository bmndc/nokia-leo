                       ...
                       int16_t *aDecision)
{
  ...
  *aDecision = nsIContentPolicy::ACCEPT;
  ...
  if (status == nsIPrincipal::APP_STATUS_CERTIFIED) {
   ...
   // "default-src * data: blob:; script-src 'self' http://127.0.0.1 http://127.0.0.1:8081 http://local-device.kaiostech.com; object-src 'none'; style-src 'self' app://theme.gaiamobile.org app://shared.gaiamobile.org"
   // That means we can optimize for this case by:
   // - loading same origin scripts and stylesheets, and stylesheets from the
   //   theme url space.
   // - loading scripts and stylesheets from the shared url space.
   // - loading scripts from local http server
      ...
      case nsIContentPolicy::TYPE_STYLESHEET:
        {
          ...
          auto localOrigin = NS_LITERAL_CSTRING("http://127.0.0.1");
          auto localOrigin2 = NS_LITERAL_CSTRING("http://127.0.0.1:8081");
          auto fakeLocalOrigin = NS_LITERAL_CSTRING("http://local-device.kaiostech.com");

          if (!(sourceOrigin.Equals(contentOrigin) ||
                localOrigin.Equals(contentOrigin) ||
                localOrigin2.Equals(contentOrigin) ||
 ...
 // ----- END OF FAST PATH FOR CERTIFIED APPS. -----
