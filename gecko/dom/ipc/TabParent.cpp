...
TabParent::RecvSetSpatialNavigationEnabled(const bool& aEnabled)
{
  ...
  if (NS_SUCCEEDED(rv) && mFrameElement) {
    // Inform BrowserElementParent to dispatch a
    // "mozbrowserspatialnavigationchanged" event.
    Unused << BrowserElementParent::DispatchSpatialNavigationChangedEvent(
      mFrameElement, aEnabled);
  }
...
bool
TabParent::RecvTriggerSpatialNavigationCursorDown()
{
  RefPtr<SpatialNavigationService> service =
    SpatialNavigationService::GetOrCreate();
  service->CursorDown();
...
bool
TabParent::RecvTriggerSpatialNavigationCursorUp()
{
  RefPtr<SpatialNavigationService> service =
    SpatialNavigationService::GetOrCreate();
  service->CursorUp();
                                          ...
                                          bool aMuted)
{
  ...
  while (window) {
    uint32_t appId;
    rv = GetAppId(window->GetCurrentInnerWindow(), &appId);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      break;
    }

    // If this app process is a child process of system App, do not send
    // AudioChannelChangeNotification, that will cause child App to be
    // muted/unmuted incorrectly.
    // See: https://bugzilla.kaiostech.com/show_bug.cgi?id=58670
