...
{
  if (NS_IsMainThread()) {
    // We should not allow main-thread I/O, especially blocking mainthread.
                               ...
                               BlobImpl* aBlobImpl)
{
  ...
  if (nsCOMPtr<nsIRemoteBlob> remoteBlob = do_QueryInterface(aBlobImpl)) {
    actor = MaybeGetActorFromRemoteBlob(remoteBlob, aManager, aBlobImpl);
    if (actor) {
      return actor;
    }
  }
  ...
  actor = new BlobChild(aManager, aBlobImpl);
...
BlobParent::RecvGetFileId(int64_t* aFileId)
{
  ...
  //Let certified APP use FD for VideoOffloadPlayer
...
#ifdef MOZ_WIDGET_GONK
bool
BlobParent::IsCertifiedApp()
{
  ContentParent* contentParent = GetContentManager()->AsContentParent();
  ...
  nsTArray<TabContext> contextArray =
    static_cast<ContentParent*>(contentParent)->GetManagedTabContext();

  if (contextArray.Length() == 0) {
    return false;
  }

  nsCOMPtr<mozIApplication> ownApp = contextArray[0].GetOwnApp();
