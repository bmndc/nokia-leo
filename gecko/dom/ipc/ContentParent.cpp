...
#include "mozilla/dom/TEEParent.h"
#endif
#ifdef ENABLE_RSU
...
#ifdef ENABLE_EMBMS
#include "mozilla/dom/LteBroadcastParent.h"
#endif

#ifdef ENABLE_SOUNDTRIGGER
#include "mozilla/dom/SoundTriggerParent.h"
#endif
...
ContentParent::GetInitialProcessPriority(Element* aFrameElement)
{
  ...
  if (aFrameElement->AttrValueIs(kNameSpaceID_None, nsGkAtoms::mozapptype,
    ...
    return PROCESS_PRIORITY_FOREGROUND_KEYBOARD;
                                        ...
                                        NS_LITERAL_STRING("hipri"), eCaseMatters)) {
...
{
  if (!ProcessPriorityManager::CurrentProcessIsForeground()) {
...
ContentParent::ForkNewProcess(bool aBlocking)
{
  ...
  if (mNuwaParent->ForkNewProcess(pid, mozilla::Move(fds), aBlocking)) {
    // In 256MB devices, if forking a preallocated app is a blocking call
    // (aBlocking=true), it means:
    // - no any preallocated app exists.
    // - system app is launching an app.
    // In order to keep consistency between JS and native gecko, we need
    // to prevent the preallocated app from being killed by LMK before the
    // completion of app transformation.
    OnNewProcessCreated(pid, mozilla::Move(fds), aBlocking);
  }
#else
...
#ifdef ENABLE_EMBMS
PLteBroadcastParent*
ContentParent::AllocPLteBroadcastParent()
{
  RefPtr<LteBroadcastParent> actor = new LteBroadcastParent();
  return actor.forget().take();
}

bool
ContentParent::DeallocPLteBroadcastParent(PLteBroadcastParent* aActor)
{
  RefPtr<LteBroadcastParent> actor = dont_AddRef(static_cast<LteBroadcastParent*>(aActor));
  return true;
}

bool
ContentParent::RecvPLteBroadcastConstructor(PLteBroadcastParent* aActor)
{
  return true;
}
#endif
...
#ifdef ENABLE_TEE_SUI
PTEEParent*
ContentParent::AllocPTEEParent()
{
  RefPtr<TEEParent> actor = new TEEParent();
  return actor.forget().take();
}

bool
ContentParent::DeallocPTEEParent(PTEEParent* aActor)
{
  RefPtr<TEEParent> actor = dont_AddRef(static_cast<TEEParent*>(aActor));
  return true;
}

bool
ContentParent::RecvPTEEConstructor(PTEEParent* aActor)
{
  return static_cast<TEEParent*>(aActor)->Init();
}
#endif
...
bool
ContentParent::RecvPRSUConstructor(PRSUParent* aActor)
{
  return true;/*static_cast<RSUParent*>(aActor)->Init();*/
}
#endif

#ifdef ENABLE_SOUNDTRIGGER
PSoundTriggerParent*
ContentParent::AllocPSoundTriggerParent()
{
  RefPtr<SoundTriggerParent> actor = new SoundTriggerParent();
  return actor.forget().take();
}

bool
ContentParent::DeallocPSoundTriggerParent(PSoundTriggerParent* aActor)
{
  RefPtr<SoundTriggerParent> actor = dont_AddRef(static_cast<SoundTriggerParent*>(aActor));
  return true;
}
#endif
                                        ...
                                        const nsCString& aReason)
{
  ...
  // Notify mtp to update when recieve file modification from child process
  obs->NotifyObservers(dsf, "mtp-watcher-update",
                       NS_ConvertASCIItoUTF16(aReason).get());
  return true;
}

static int32_t
AddGeolocationListener(nsIDOMGeoPositionCallback* aWatcher,
                       nsIDOMGeoPositionErrorCallback* aErrorCallBack,
                       bool aHighAccuracy, bool aE911, uint16_t aGpsMode)
{
  ...
  options->mEnableHighAccuracy = aHighAccuracy;
  options->mEnableE911 = aE911;
#if defined(MOZ_WIDGET_GONK) && \
   !defined(DISABLE_MOZ_RIL_GEOLOC) && !defined(DISABLE_MOZ_GEOLOC)
  options->mGpsMode = aGpsMode;
#endif
  int32_t retval = 1;
  geo->WatchPosition(aWatcher, aErrorCallBack, options, &retval);
  return retval;
}
                                          ...
                                          const uint16_t& aGpsMode)
{
  ...
  RecvRemoveGeolocationListener();
  mGeolocationWatchID = AddGeolocationListener(this, this, aHighAccuracy, aE911,
                                               aGpsMode);
...
bool
ContentParent::RecvSetGeolocationOptions(const bool& aHighAccuracy,
                                         const bool& aE911,
                                         const uint16_t& aGpsMode)
{
                                ...
                                OptionalURIParams* aURI)
{
  ...
  if (NS_FAILED(fixup->KeywordToURI(aKeyword, getter_AddRefs(postData),
                                    getter_AddRefs(info)))) {
    *aPostData = OptionalInputStreamParams(void_t());
    *aURI = OptionalURIParams(void_t());
    return true;
  }
