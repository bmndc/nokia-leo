...
NS_IMPL_ISUPPORTS(PowerManagerService, nsIPowerManagerService, nsITimerCallback)
...
PowerManagerService::Init()
{
  ...
  mFinalPowerOffDone = false;
  mFinalRebootDone = false;
}
...
PowerManagerService::~PowerManagerService()
{
  ...
  if (mRebootTimer) {
    mRebootTimer->Cancel();
    mRebootTimer = nullptr;
  }
  if (mPowerOffTimer) {
    mPowerOffTimer->Cancel();
    mPowerOffTimer = nullptr;
  }
}
...
nsresult
PowerManagerService::Notify(nsITimer* aTimer)
{
  if (aTimer == mPowerOffTimer) {
    PowerOffFinal();
  } else {
    RebootFinal();
  }
...
NS_IMETHODIMP
ShutdownRadioRequest::NotifyGetClirStatusSuccess(uint16_t aN, uint16_t aM)
{
  return NS_OK;
}
...
PowerManagerService::RebootFinal()
{
  ...
  mFinalRebootDone = true;
  LOG_FUNCTION_AND_JS_STACK() // bug 839452
  Preferences::SetCString("device.rebootReason", "normal");
...
{
  if (mFinalPowerOffDone) {
    return;
  }
  mFinalPowerOffDone = true;
  LOG_FUNCTION_AND_JS_STACK() // bug 839452
  Preferences::SetCString("device.rebootReason", "normal");
...
{
  if (Preferences::GetBool("ril.shutdown_radio_request_enabled", false)) {
    LOG_FUNCTION_AND_JS_STACK() // bug 839452
    mShutdownRadioRequest = new ShutdownRadioRequest(this,true);
    if (mShutdownRadioRequest &&
      (NS_OK == mShutdownRadioRequest->ShutdownRadio())) {
      if (!mRebootTimer) {
        mRebootTimer = do_CreateInstance("@mozilla.org/timer;1");
      }
      if (mRebootTimer) {
        int32_t rilRequestTimeoutSecs =
          Preferences::GetInt("shutdownRadioRequest.timeoutSecs", 30) * 1000;
        nsresult rv = mRebootTimer->InitWithCallback(this,
                                                     rilRequestTimeoutSecs,
...
PowerManagerService::PowerOff()
{
  if (Preferences::GetBool("ril.shutdown_radio_request_enabled", false)) {
    ...
    if (mShutdownRadioRequest &&
      (NS_OK == mShutdownRadioRequest->ShutdownRadio())) {
      if (!mPowerOffTimer) {
        mPowerOffTimer = do_CreateInstance("@mozilla.org/timer;1");
      }
      if (mPowerOffTimer) {
        int32_t rilRequestTimeoutSecs =
          Preferences::GetInt("shutdownRadioRequest.timeoutSecs", 30) * 1000;
        nsresult rv = mPowerOffTimer->InitWithCallback(this,
                                                       rilRequestTimeoutSecs,
                                                       nsITimer::TYPE_ONE_SHOT);
        if (NS_SUCCEEDED(rv)) {
          return NS_OK;;
