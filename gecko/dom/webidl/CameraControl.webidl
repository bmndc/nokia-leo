...
callback AnalysisCallback = void (FaceAnalysisManager faceAnalysisManager);
#endif
...
#ifdef ENABLE_FACE_RECOGNITION_SUPPORT
interface FaceAnalysisManager : EventTarget
{
  [Throws]
  Promise<boolean> startFaceAnalysis(long type, optional sequence<FaceAnalysisImage> refImages);

  [Throws]
  Promise<boolean> stopFaceAnalysis();

  /* FaceAnalysisResultsEvent */
  attribute EventHandler onfaceanalysisresults;
};

partial interface CameraControl
{
  [Throws]
  Promise<FaceAnalysisManager>getFaceAnalysis(Blob license);
};
#endif
