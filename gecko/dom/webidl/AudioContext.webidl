...
interface AudioContext : EventTarget {
    ...
    // Immediately grab audio channel for a short duration (500 ms). After
    // this duration is passed, the audio channel will be released if there
    // is no sound playing and forceAudioChannelPlaying() is not called again.
    // If suspendAndAutoResume is true, AudioContext is suspended immediately
    // and automatically resumed after audio channel is unmuted.
    void forceAudioChannelPlaying(optional boolean suspendAndAutoResume = false);
};
