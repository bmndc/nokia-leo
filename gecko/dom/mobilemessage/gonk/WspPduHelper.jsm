...
this.TextString = {
  ...
  decode: function(data) {
    ...
    if (firstCode == 127) {
      ...
      begin = data.offset;
      try {
        let secondCode = Octet.decode(data);
        if (secondCode < 128) {
          if (secondCode == NUL) {
            return "";
          }
...
this.QuotedString = {
  ...
  decode: function(data) {
    ...
    let QuotedStr = null;
    try {
      debug("QuotedString UTF8Code.decode");
      QuotedStr = UTF8Code.decode(data);
    } catch (e) {
      data.offset = begin;
    }
    if (QuotedStr != null) {
      debug("QuotedStr :" + QuotedStr);
      return QuotedStr;
    }
    let value = Octet.decode(data);
    if (value != 34) {
...
/**
 * Internal coder for "china unicode code"
 *
 * encode|decode value for params.name|content-location|content-id parameter
 *
 * As TextString, NullTerminatedTexts supplementary encoding|decoding function
 */
this.UTF8Code = {
  /**
   * @param data
   ...
   * @return Decoded string.
   *
   * @throws CodeError if decoded data array is not chinese string.
   * @throws An exception occurred during decoding process
   *
   */
  decode: function(data) {
    let charset = "utf-8";
    let onlyascii = true;
    debug("decode charset:" + charset);
    let firstnum = Octet.decode(data);
    debug("decode firstnum :" + firstnum);
    let index = data.offset;
    try {
      if ((firstnum == DEL) || (firstnum == DQUOTE)){
        while (data.array[index] != NUL){
          if (data.array[index] > ASCIIS) {
            onlyascii = false;
          }
          index++;
        }
      ...
      if (onlyascii) {
        throw new CodeError("not china unicode.");
      }
    } catch (e if e instanceof CodeError) {
      throw e;
    } catch (e) {
      throw new CodeError("UTF8Code: unexpected error.");
    }
    let end = index;
    let decodeStr = null;
    let octetArray  = null;
    ...
    if (firstnum == DEL) {
      ...
      if (Octet.decode(data) < DEL){
        throw new CodeError("UTF8Code doesn't support mixed Chinese and English.");
      }
      data.offset = begin;
      octetArray = Octet.decodeMultiple(data, end);
      decodeStr = PduHelper.decodeStringContent(octetArray, charset);
      if (decodeStr == null){
        throw new CodeError("UTF8Code decodeStr is null.");
      }
      data.offset = end + 1;
    } else if (firstnum == DQUOTE) {
      octetArray = Octet.decodeMultiple(data, end);
      decodeStr = PduHelper.decodeStringContent(octetArray, charset);
      if (decodeStr == null){
        throw new CodeError("UTF8Code decodeStr is null.");
      ...
      throw new CodeError("UTF8Code has exception.");
    }
    debug("UTF8Code decodeStr:" + decodeStr);
    return decodeStr;
  },
   ...
   * @param data
   *        A wrapped object to store encoded raw data.
   * @param A String to be encoded.
   * @throws An exception occurred during Chinese string encoding
   */
  encode: function(data, unicodeStr) {
    let charset = "utf-8";
    debug("charset:" + charset +" unicodeStr:" + unicodeStr);
    debug("unicodeStr typeof is:" + typeof(unicodeStr));
    let code = unicodeStr.charCodeAt(0);
    if (code == 60) {
      Octet.encode(data, 0x22);
    } else if (code > DEL) {
      //If the first character in the TEXT is in the range of 128-255, a Quote character must precede it.
      //Quote = <Octet 127>
      //End-of-string = <Octet 0>
      Octet.encode(data, 0x7F);
    } else {
      throw new CodeError("encode invalid string");
    }
    let encodeStr = null;
    try {
      encodeStr = PduHelper.encodeStringContent(unicodeStr, charset);
    } catch (e) {
      throw new CodeError("UTF8Code.encode has exception");
    }
    if (encodeStr == null){
      throw new CodeError("UTF8Code encodeStr is null.");
    }
    debug("UTF8Code encodeStr:" + encodeStr);
    for (let i = 0; i < encodeStr.length; i++){
      Octet.encode(data, encodeStr[i]);
    }
    Octet.encode(data, 0);
  },
};
...
this.Parameter = {
  ...
  decodeTypedParameter: function(data) {
    ...
    try {
      ...
      value = decodeAlternatives(data, null,
                                 param.coder, TextValue, UTF8Code, TextString);
      debug("decodeTypedParameter param.value:" + value);
    } catch (e) {
      data.offset = begin;
  ...
  encodeTypedParameter: function(data, param) {
    ...
    encodeAlternatives(data, param.value, null,
                       entry.coder, TextValue, TextString, UTF8Code);
  },
...
this.WellKnownHeader = {
  ...
  decode: function(data) {
    ...
    let begin = data.offset, value;
    try {
      value = decodeAlternatives(data, null, entry.coder, TextValue, UTF8Code);
      debug("WellKnownHeader.decode name:" + entry.name + " value:" + value);
    } catch (e) {
      data.offset = begin;
  ...
  encode: function(data, header) {
    ...
    encodeAlternatives(data, header.value, null, entry.coder, TextValue, UTF8Code);
  },
};
...
this.OMNA_PUSH_APPLICATION_IDS = (function() {
  ...
  add("x-oma-application:ulp.ua", 0x10);
