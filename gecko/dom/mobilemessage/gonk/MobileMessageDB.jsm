...
const MSG_ATTACHMENT_STATE_NONE   = 0;
const MSG_ATTACHMENT_STATE_NOT_DOWNLOAD  = 1;
const MSG_ATTACHMENT_STATE_HAS_ATTACHMENT   = 2;
...
MobileMessageDB.prototype = {
  ...
  getAttachmentStatus : function(aMessageRecord) {
    if (!aMessageRecord) {
      return MSG_ATTACHMENT_STATE_NONE;
    }
    if (aMessageRecord.type == "mms") {
      if (aMessageRecord.delivery == DELIVERY_NOT_DOWNLOADED) {
        return MSG_ATTACHMENT_STATE_NOT_DOWNLOAD;
      }
      ...
      for (let i=0; i< parts.length; i++) {
        ...
        if (partHeaders["content-type"]["media"] != "application/smil" &&
          ...
          return MSG_ATTACHMENT_STATE_HAS_ATTACHMENT;
        }
      }
    }
    return MSG_ATTACHMENT_STATE_NONE;
        ...
        threadMessageCursorReq.onsuccess = function(aEvent) {
          ...
          self.findThreadRecordByTypedAddresses(threadStore, participantStore,
            ...
            if (aThreadRecord.lastTimestamp <= messageRecord.timestamp) {
              ...
              aThreadRecord.lastAttachmentStatus = self.getAttachmentStatus(messageRecord);
  ...
  saveRecord: function(aMessageRecord, aThreadParticipants, aCallback) {
    ...
    this.newTxn(READ_WRITE, function(error, txn, stores) {
      ...
      txn.oncomplete = function(event) {
        ...
        // If thread deleted, the message id should be null, it is not necessary.
    ...
    this.findThreadRecordByTypedAddresses(aThreadStore, aParticipantStore,
        ...
        aMessageStore.get(aMessageRecord.id).onsuccess = function(event) {
          ...
          if (oldMessageRecord) {
            self.updateThreadByMessageChange(aMessageStore,
                                             ...
                                             aDeletedInfo);
            if (DEBUG) debug("Notify download replace action to update UI type, old thread is " +
                             oldMessageRecord.threadId + ", new thread is " + aMessageRecord.threadId);
      ...
      if (threadRecord) {
        ...
        if (threadRecord.lastTimestamp <= timestamp) {
          ...
          threadRecord.lastAttachmentStatus = self.getAttachmentStatus(aMessageRecord);
          threadRecord.lastTimestamp = timestamp;
      ...
      let lastAttachmentStatus = self.getAttachmentStatus(aMessageRecord);
      ...
      threadRecord = {
        ...
        lastAttachmentStatus: lastAttachmentStatus,
  ...
  fillReceivedMmsThreadParticipants: function(aMessage, threadParticipants) {
    ...
    if (aMessage.phoneNumber) {
      for (var i = 0; i < slicedReceivers.length; i++) {
        if (PhoneNumberUtils.fuzzyMatch(slicedReceivers[i], aMessage.phoneNumber)) {
          isSuccess = true;
          slicedReceivers.splice(i, 1);
          break;
        }
      }
    }
    ...
    threadStore.get(threadId).onsuccess = function(event) {
      ...
      if (removedMsgIds.indexOf(threadRecord.lastMessageId) >= 0) {
        ...
        request.onsuccess = function(event) {
          ...
          threadRecord.lastMessageSubject = lastMessageSubject || null;
          threadRecord.lastAttachmentStatus = self.getAttachmentStatus(nextMsg);
  ...
  saveReceivedMessage: function(aMessage, aCallback) {
    ...
    if (aMessage.type == "mms" && aMessage.receivers.length > 1) {
  ...
  saveSendingMessage: function(aMessage, aCallback) {
    ...
    if (aMessage.type == "sms") {
      ...
      if (aMessage.headers.bcc) {
        for (var i = 0; i < aMessage.headers.bcc.length; i++) {
    ...
    getRequest.onsuccess = function(event) {
      ...
      let thread =
        gMobileMessageService.createThread(threadRecord.id,
                                           ...
                                           threadRecord.lastAttachmentStatus ||
                                           MSG_ATTACHMENT_STATE_NONE);
