...
/* << [BTS-135]: BDC kanxj 20190503 add to support MMS When Mobile Data disabled >> */
...
//BDC kanxj 20190325 modify to true for debug
var DEBUG = true;
function debug(s) {
  dump("-@- MmsService: " + s + "\n");
};
...
const kDevicePhoneNumberSim1             = "ril.mms.phoneNumber.sim1";
const kDevicePhoneNumberSim2             = "ril.mms.phoneNumber.sim2";
const kRilDataEnabled                     = 'ril.data.enabled';

const kPrefMmsDebuggingEnabled           = "mms.debugging.enabled";
const kPrefMmsGroupSupported             = "device.capability.group-message";
...
/**
 * Return wifi calling available state.
 */
function isWifiCallingAvailable(aServiceId) {
  let connection;
  try {
    connection = gImsRegService &&
                 gImsRegService.getHandlerByServiceId(aServiceId);
  } catch (e) {
    return false;
  }
  let capability = connection && connection.capability;
...
/* << [BTS-135]: BDC kanxj 20190325 porting to support MMS When Mobile Data disabled */
...
function isDataConnected(aServiceId) {
  ...
  try {
    ...
    debug(" read prop failed:" + e);
  }

  debug(" @dataEnabled :" + dataEnabled);
  debug(" @isDataConnected allowMmsWhenDataOff:" + allowMmsWhenDataOff);
  return dataEnabled || allowMmsWhenDataOff;
}
/* >> [BTS-135] */
  ...
  */
  ...
  acquire: function(callback) {
    ...
    if (!this.connected) {
      ...
      /* << [BTS-135]: BDC kanxj 20190325 porting to support MMS When Mobile Data disabled */
      /* << [BTS-2386]: BDC kanxj 20190723 Unable to send MMS when registered on VoWifi >> */
      } else if (!isDataConnected(this.serviceId) && !isWifiCallingAvailable(this.serviceId)) {
        debug(" MMS service not allowed when data is off");
  ...
  let helper = {
    ...
    sendHttpRequest: function(mmsConnection, method, url, istream, proxyFilter,
      ...
      try {
        ...
        /* << BDC: kanxj 20190506 add the uaprofile for Beatles >> */
        let uaProfProp = libcutils.property_get("ro.build.mms.ua.profile", "");
        if(uaProfProp !== "") {
          uaProfUrl ="http://useragentprofile.hmdglobal.com/uaprof/" + uaProfProp;
        }
        ...
        /* << BDC: kanxj 20190506 add the User Agent for Beatles */
        /* << [CNT-1374] : BDC kanxj 201901108 MT MMS does not work */
        let user_agent_name = libcutils.property_get("ro.build.mms.ua", "");
        let iccInfo = mmsConnection.getIccInfo();
        let mccmnc = "";
        if (iccInfo && iccInfo.mcc && iccInfo.mnc) {
          mccmnc = iccInfo.mcc + iccInfo.mnc;
        }
        if (DEBUG) debug("mccmnc = "+ mccmnc);
        if (((mccmnc === '24601') || (mccmnc === '23820') || (mccmnc === '23830'))
                 && user_agent_name) {
          user_agent_name = user_agent_name + "-MMS/1.1;KaiOS 2.5.4";
        } else {
          user_agent_name = libcutils.property_get("ro.build.product", "") + "-MMS/1.1;KaiOS 2.5.4";
        }
        if (DEBUG) debug("user_agent_name = "+ user_agent_name);
        /* >> [CNT-1374] */
 ...
 */
function MmsService() {
  // First get the phone number when mms service start.
  gSettingsService.createLock().get(kDevicePhoneNumberSim1, this);
  gSettingsService.createLock().get(kDevicePhoneNumberSim2, this);

  try {
    this.deviceGroupSupport = Services.prefs.getBoolPref(kPrefMmsGroupSupported);
  } catch (e) {
    this.deviceGroupSupport = false;
  }
  ...
  dataEnabled = Services.prefs.getBoolPref("dom.data.enabled");
...
MmsService.prototype = {
  ...
  convertIntermediateToSavable: function(mmsConnection, intermediate,
    ...
    if (!intermediate.phoneNumber) {
      if (serviceId === 0) {
        intermediate.phoneNumber = this.sim1Number;
      } else {
        intermediate.phoneNumber = this.sim2Number;
      }
    }
  ...
  handleNotificationIndication: function(serviceId, notification) {
    ...
    gMobileMessageDatabaseService
      ...
      let handleSettingResult = {
        handle: function(aName, aResult) {
          if (aName === kRilDataEnabled) {
            this.dataEnabled = aResult;
            this.retrieveNotificationIndication(serviceId, notification, aResult);
          }
        }.bind(this),

        handleError: function(aErrorMessage) {
          // If get settings fail, we think the data is off default.
          this.retrieveNotificationIndication(serviceId, notification, false);
        }.bind(this)
      };

      if (this.dataEnabled === null) {
        // Only get data switch value once, use observer value later.
        gSettingsService.createLock().get(kRilDataEnabled, handleSettingResult);
      } else {
        this.retrieveNotificationIndication(serviceId, notification, this.dataEnabled);
      }
    });
  },

  retrieveNotificationIndication: function(serviceId, notification, dataEnable) {
    let retrievalMode = RETRIEVAL_MODE_MANUAL;
    try {
      retrievalMode = Services.prefs.getCharPref(kPrefRetrievalMode);
      /* << BDC kanxj 20190402 add to support mms default retrival mode start */
      if (retrievalMode == '1') {
        retrievalMode = RETRIEVAL_MODE_MANUAL;
      } else if (retrievalMode == '0') {
        retrievalMode = RETRIEVAL_MODE_AUTOMATIC_HOME;
      }
      /* << BDC kanxj 20190402 add to support mms default retrival mode end */
    } catch (e) {}
    ...
    // When device support group but not have SIM Msisdn/Mdn
    // and not have fake number, need make MMS switch to manual download
    // then let user input fake number at UI.
    let noMdnForGroup = false;
    if (this.deviceGroupSupport) {
      let fakeNumber = serviceId === 0 ? this.sim1Number : this.sim2Number;
      if (!fakeNumber) {
        noMdnForGroup = true;
      }
    }
    ...
    if ((retrievalMode == RETRIEVAL_MODE_AUTOMATIC ||
        ...
        (serviceId != this.mmsDefaultServiceId || lowDataSpace ||
         noMdnForGroup || !isDataConnected(serviceId))) {
      if (DEBUG) {
  ...
  createSavableFromParams: function(aMmsConnection, aParams, aMessage) {
    ...
    // If group message, the cc field is necessary.
    let headersCc;
    if (aParams.isGroup) {
      headersCc = headers["cc"] = [];
    }
    // If device support group but not send group,
    // should use BCC field according to ATT case LTE-BTR-5-3076.
    let headerBcc;
    if (this.deviceGroupSupport) {
      headerBcc = headers["bcc"] = [];
    }
    if (receivers.length != 0) {
      for (let i = 0; i < receivers.length; i++) {
        ...
        // For the device which support group, if send group MMS, use to + cc field,
        // if send normal MMS, use bcc field.
        // For the device which not support group, send normal MMS, use to field.
        if (aParams.isGroup) {
          if (i > 0) {
            headersCc.push({"address": address, "type": type});
          } else {
            headersTo.push({"address": address, "type": type});
          }
        } else {
          if (this.deviceGroupSupport) {
            headerBcc.push({"address": address, "type": type});
          } else {
            headersTo.push({"address": address, "type": type});
          }
        }
  ...
  sim1Number: null,
  sim2Number: null,
  deviceGroupSupport: false,
  /* << [LIO-319]: BDC kanxj 20200713 modify for MMS with Mobile Data disabled */
  //dataEnabled: null,
  ...
  send: function(aServiceId, aParams, aRequest) {
    ...
    // Check if |isGroup| is valid.
    if (aParams.isGroup === null || typeof aParams.isGroup !== "boolean") {
      if (DEBUG) debug("Error! 'isGroup' should be a boolean if passed.");
      throw Cr.NS_ERROR_INVALID_ARG;
      return;
    }
    ...
    gMobileMessageDatabaseService
      ...
      /* << [BTS-135]: BDC kanxj 20190325 porting to support MMS When Mobile Data disabled */
      /* << [BTS-2386]: BDC kanxj 20190723 Unable to send MMS when registered on VoWifi >> */
      if (!isDataConnected(aServiceId) && !isWifiCallingAvailable(aServiceId)) {
  ...
  retrieve: function(aMessageId, aRequest) {
    ...
    gMobileMessageDatabaseService
      ...
      /* << [BTS-135]: BDC kanxj 20190325 porting to support MMS When Mobile Data disabled */
      /* << [BTS-2386]: BDC kanxj 20190723 Unable to send MMS when registered on VoWifi >> */
      if (!isDataAllowed && !isWifiCallingAvailable(aServiceId)) {
        debug("My Tag Error! MMS APN not allowed when data is off.");
        aRequest.notifyGetMessageFailed(
          Ci.nsIMobileMessageCallback.SERVICE_NOT_AVAILABLE_ERROR);
        return;
      }
  ...
  handle: function(name, result) {
    if (name === kDevicePhoneNumberSim1) {
      this.sim1Number = result;
    } else if (name === kDevicePhoneNumberSim2) {
      this.sim2Number = result;
    } else if (name === kRilDataEnabled) {
      /* << [LIO-319]: BDC kanxj 20200713 MMS with Mobile Data disabled */
      dataEnabled = result;
    }
  },
