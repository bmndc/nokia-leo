...
SmsService.prototype = {
  ...
  _processReceivedSmsSegment: function(aSegment) {
    ...
    if (aSegment.teleservice === RIL.PDU_CDMA_MSG_TELESERVICE_ID_WAP
        && seq === 1) {
   ...
   */
  _handleSmsSuplInitMessage: function(aMessage, aServiceId) {
    aMessage.type = "sms";
    aMessage.receiver = this._getPhoneNumber(aServiceId);
    aMessage.body = aMessage.fullBody = aMessage.fullBody || null;
    if (DEBUG) {
      debug("handleSmsSuplInit, receiver = " + aMessage.receiver + ", Message = " + aMessage.body);
    }
    if (aMessage.encoding == Ci.nsIGonkSmsService.SMS_MESSAGE_ENCODING_8BITS_ALPHABET) {
      Services.obs.notifyObservers(aMessage, kSmsSuplInitObserverTopic, aMessage.fullData || []);
    } else {
  ...
  _handleSmsReceived: function(aMessage, aServiceId) {
    ...
    if (aMessage.destinationPort !== Ci.nsIGonkSmsService.SMS_APPLICATION_PORT_INVALID) {
      if (DEBUG) debug("Message destinationPort = " + aMessage.destinationPort);
...
SmsSendingScheduler.prototype = {
  ...
  _ensureMoboConnObserverRegistration: function() {
    ...
    let imsReg;
    try {
      imsReg = gImsRegService &&
               gImsRegService.getHandlerByServiceId(this._serviceId);
    } catch (e) {
      imsReg = null;
    }
  ...
  isIMSConnected: function() {
    let imsRegService;
    try {
      imsRegService = gImsRegService &&
                      gImsRegService.getHandlerByServiceId(this._serviceId);
    } catch (e) {
      return false;
    }

    return imsRegService &&
           imsRegService.capability !== Ci.nsIImsRegHandler.IMS_CAPABILITY_UNKNOWN;
  },
  ...
  send: function() {
    ...
    // If the voice connection and IMS connection are temporarily unavailable, pend the request.
    let voiceInfo = connection && connection.voice;
    let voiceConnected = voiceInfo && voiceInfo.connected;
    if (!voiceConnected && !this.isIMSConnected()) {
      if (DEBUG) {
        debug("Voice connection and IMS connection are temporarily unavailable. Skip sending.");
      }
      return;
    }
  ...
  // Unused nsIImsRegListener methods.
  notifyRttEnabledStateChanged: function(aEnabled) {}
};
