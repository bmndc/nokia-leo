                           ...
                           ErrorResult& aRv)
{
  ...
  uint32_t serviceId;
  nsresult rv = GetSmsServiceAndId(smsService, serviceId, aSendParams);
  if (NS_FAILED(rv)) {
    aRv.Throw(rv);
    return nullptr;
  }
...
bool
MobileMessageManager::IsCalledBySystem()
{
  ...
  return (appId == systemAppId);
}

nsresult
MobileMessageManager::GetSmsServiceAndId(nsCOMPtr<nsISmsService>& aSmsService,
                                         uint32_t &aServiceId,
                                         const SmsSendParameters& aSendParams)
{
  aSmsService = do_GetService(SMS_SERVICE_CONTRACTID);
  if (!aSmsService) {
    return NS_ERROR_FAILURE;
  }

  if (aSendParams.mServiceId.WasPassed()) {
      aServiceId = aSendParams.mServiceId.Value();
  } else {
    nsresult rv = aSmsService->GetSmsDefaultServiceId(&aServiceId);
    if (NS_FAILED(rv)) {
      return NS_ERROR_FAILURE;
    }
  }
...
already_AddRefed<DOMRequest>
MobileMessageManager::SendSilently(const nsAString& aNumber,
                                   const nsAString& aText,
                                   const SmsSendParameters& aSendParams,
                                   ErrorResult& aRv)
{
  // only system can send silent SMS
  if (!IsCalledBySystem()) {
    aRv.Throw(NS_ERROR_FAILURE);
    return nullptr;
  }
  ...
  return Send(smsService, serviceId, aNumber, aText, aRv, true);
}
