...
public:
  MediaEngineWebRTCMicrophoneSource(nsIThread* aThread,
    ...
    , mPlayoutDelay(0)
    , mAudioSource(dom::AudioSourceEnum::Mic)
    ...
    if (mDeviceName.EqualsLiteral("mic")) {
      ...
      mAudioSource = dom::AudioSourceEnum::Voicecall;
    } else if (mDeviceName.EqualsLiteral("fm")) {
      mAudioSource = dom::AudioSourceEnum::Fm;
    } else if (mDeviceName.EqualsLiteral("voicerecognition")) {
...
private:
  ...
  dom::AudioSourceEnum mAudioSource;
