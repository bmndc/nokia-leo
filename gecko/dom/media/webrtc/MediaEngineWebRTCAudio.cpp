    ...
    const nsString& aDeviceId)
{
  ...
  if (mAudioSource == dom::AudioSourceEnum::Voicecall ||
      mAudioSource == dom::AudioSourceEnum::Fm ||
      mAudioSource == dom::AudioSourceEnum::Voicerecognition) {
    exactMatching = true;
  }

  nsString sourceString(NS_ConvertUTF8toUTF16(
      dom::AudioSourceEnumValues::strings[uint32_t(mAudioSource)].value));
  uint64_t distance = 0;

  for (const MediaTrackConstraintSet* cs : aConstraintSets) {
    // When |exactMatching| is true, rule us out if our audio source is not
    // specified in the constraint.
    if (exactMatching) {
      if (!cs->mAudioSource.IsString() ||
          cs->mAudioSource.GetAsString() != sourceString) {
        return UINT32_MAX;
      }
    }

    distance = uint64_t(GetMinimumFitnessDistance(*cs, false, aDeviceId)) + 
               uint64_t(FitnessDistance(sourceString, cs->mAudioSource, true));
...
MediaEngineWebRTCMicrophoneSource::Restart(const dom::MediaTrackConstraints& aConstraints,
                                           ...
                                           const nsString& aDeviceId)
{
  // Disable audio processing for FM source.
  if (mAudioSource == dom::AudioSourceEnum::Fm) {
    LOG(("Disable audio processing"));
    MOZ_ASSERT(gFarendObserver);
    gFarendObserver->SetSkipping(true);
    return NS_OK;
  }
