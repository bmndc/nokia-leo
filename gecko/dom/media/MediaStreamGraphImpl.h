...
public:
   ...
   */
  void RunMessagesInQueue(bool aAfterProcessing);
  /**
  ...
  /*
   * Same as SuspendOrResumeStreams(), but let the streams be suspended or resumed
   * after processing. This makes sure that any AudioContext operation takes effect
   * in the next iteration, instead of the current one.
   */
  void SuspendOrResumeAfterProcessing(dom::AudioContextOperation aAudioContextOperation,
                                      const nsTArray<MediaStream*>& aStreamSet);
