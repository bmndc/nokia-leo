...
LazyLogModule gAudioContextLog("AudioContext");
...
AudioContext::AudioContext(nsPIDOMWindowInner* aWindow,
  ...
  , mAutoResume(false)
{
  CTX_LOG(this, "AudioContext(), window %p, offline %d, audio channel %d, "
          "number of channels %u, length %u, sample rate %f",
          aWindow, aIsOffline, (int)aChannel, aNumberOfChannels, aLength, aSampleRate);
...
AudioContext::Init()
{
  // We skip calling SetIsOnlyNodeForContext during
  // mDestination's constructor, because we can only
  // call it after mDestination has been set up.
  if (!mIsOffline) {
...
AudioContext::~AudioContext()
{
  ...
  CTX_LOG(this, "AudioContext destroyed");
}
...
AudioContext::OnStateChanged(void* aPromise, AudioContextState aNewState)
{
  ...
  CTX_LOG(this, "State changed to %d", (int)aNewState);
  mAudioContextState = aNewState;
}
...
AudioContext::Suspend(ErrorResult& aRv)
{
  ...
  CTX_LOG(this, "Suspending context...");
...
void
AudioContext::AudioChannelStateChanged(bool aPlaying)
{
  MOZ_ASSERT(NS_IsMainThread());
  if (mAutoResume && !aPlaying) {
    CTX_LOG(this, "Failed to wait audio channel unmuted. Resume anyway.");
    mAutoResume = false;
    ErrorResult rv;
    RefPtr<Promise> unused = Resume(rv);
  }
}

void
AudioContext::AudioChannelMuted(bool aMuted)
{
  MOZ_ASSERT(NS_IsMainThread());
  if (mAutoResume && !aMuted) {
    CTX_LOG(this, "Audio channel unmuted. Resume now.");
    mAutoResume = false;
    ErrorResult rv;
    RefPtr<Promise> unused = Resume(rv);
  }
...
void
AudioContext::ForceAudioChannelPlaying(bool aSuspendAndAutoResume)
{
  if (mIsOffline || mCloseCalled) {
    return;
  }

  CTX_LOG(this, "Force audio channel to start playing, aSuspendAndAutoResume %d",
          aSuspendAndAutoResume);

  // Don't suspend if audio channel is already playing.
  if (aSuspendAndAutoResume && !mDestination->IsAudioChannelPlaying()) {
    if (!mSuspendCalled) {
      ErrorResult rv;
      RefPtr<Promise> unused = Suspend(rv);
    }
    mAutoResume = aSuspendAndAutoResume;
  }

  // The duration should be long enough to cover the delay before
  // AudioDestinationNode::InputMuted() notifies AudioChannelHelper
  // that input is unmuted.
  static const uint32_t durationMs = 500;
  mDestination->ForceAudioChannelPlaying(durationMs);
}
