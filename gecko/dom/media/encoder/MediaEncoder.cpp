                             ...
                             nsAString& aMIMEType)
{
  ...
  while (reloop) {
    ...
    case ENCODE_TRACK: {
      ...
      // check whether the time exceeds the "timeslice" in MediaRecorder. If it is, force the writer to
      // output the results.
      bool timeout = (TimeStamp::Now() - startTime).ToMilliseconds() > mTimeout;
      int32_t flags = isAudioCompleted && isVideoCompleted ? ContainerWriter::FLUSH_NEEDED : 0;

      if (timeout) {
        // FLUSH_NEEDED will stop the encoder, we need to use FORCE_OUTPUT
        flags |= ContainerWriter::FORCE_OUTPUT;
      }

      rv = mWriter->GetContainerData(aOutputBufs, flags);
