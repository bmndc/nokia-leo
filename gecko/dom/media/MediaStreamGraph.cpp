...
MediaStreamGraphImpl::AudioTrackPresent(bool& aNeedsAEC)
{
  ...
  for (uint32_t i = 0; i < mStreams.Length() && audioTrackPresent == false; ++i) {
    ...
    for (StreamBuffer::TrackIter tracks(stream->GetStreamBuffer(), MediaSegment::AUDIO);
         !tracks.IsEnded() && !audioTrackPresent; tracks.Next()) {
      // To prevent AudioContext from holding cubeb stream even if it's not producing sound, use
      // AudioCallbackDriver only if AudioDestinationNode's audio track is enabled. For now we
      // only apply this check on AudioNodeStream. Maybe we should apply it on all MediaStreams.
      if (stream->AsAudioNodeStream()) {
        audioTrackPresent = stream->IsTrackEnabled(tracks->GetID());
      } else {
...
static void
RunMessages(nsTArray<MessageBlock>& aQueue)
{
  // Calculate independent action times for each batch of messages (each
  // batch corresponding to an event loop task). This isolates the performance
  // of different scripts to some extent.
  for (auto& block : aQueue) {
    for (auto& message : block.mMessages) {
      message->Run();
    }
  }
  aQueue.Clear();
...
void
MediaStreamGraphImpl::RunMessagesInQueue(bool aAfterProcessing)
{
  if (aAfterProcessing) {
    RunMessages(mFrontMessageQueue);
  } else {
    // When we run messages in mFrontMessageQueue, new messages may be appended
    // to the queue through RunMessageAfterProcessing(). We don't want to run
    // those new messages now.
    nsTArray<MessageBlock> messageQueue;
    messageQueue.SwapElements(mFrontMessageQueue);
    RunMessages(messageQueue);
  }
}
                                             ...
                                             const nsTArray<MediaStream*>& aStreamSet)
{
  ...
  for (MediaStream* stream : aStreamSet) {
    // SuspendOrResumeStreams() may be postponed until processing is done, and
    // these streams may have been destroyed by earlier messages of the same
    // iteration, so we need to check if they still exist.
    if (!mStreams.Contains(stream) && !mSuspendedStreams.Contains(stream)) {
      continue;
    }
...
void
MediaStreamGraphImpl::SuspendOrResumeAfterProcessing(AudioContextOperation aAudioContextOperation,
                                                     const nsTArray<MediaStream*>& aStreamSet)
{
  class SuspendOrResumeMessage : public ControlMessage
  {
  public:
    SuspendOrResumeMessage(MediaStreamGraphImpl* aGraphImpl,
    ...
    void Run() override
    {
      mGraphImpl->SuspendOrResumeStreams(mAudioContextOperation, mStreams);
    }
  ...
  RunMessageAfterProcessing(MakeUnique<SuspendOrResumeMessage>(
      this, aStreamSet, aAudioContextOperation));
}
    ...
    AudioContextOperation aOperation, void* aPromise)
{
  ...
  SuspendOrResumeAfterProcessing(aOperation, aStreams);
