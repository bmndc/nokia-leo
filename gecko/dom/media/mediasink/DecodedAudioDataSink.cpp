...
{
  //set mConverter until init. Make sure no one use it before that
}
...
void
DecodedAudioDataSink::SetConverter()
{
...
{
  MOZ_ASSERT(mAudioStream, "Must call after audio stream init");
...
// Update audio info before sink init
// bug 40098, metadata sample rate is different from decoder sample rate
// update decoder sample rate before init sink
void
DecodedAudioDataSink::UpdateInfo(const AudioInfo& aInfo)
{
  MOZ_ASSERT(!mAudioStream, "Must call before audio stream init");
  mInfo = aInfo;
}
