...
//MTK decoder has fewer output buffers,
//thus we need to lower the lowerbound of video queue.
//see https://bugzilla.kaiostech.com/show_bug.cgi?id=58543
static const uint32_t MIN_VIDEO_QUEUE_SIZE = 2;
#else
...
void MediaDecoderStateMachine::MaybeStartPlayback()
{
  ...
  if (!playStatePermits || mIsAudioPrerolling ||
      mIsVideoPrerolling || mMediaOffloading) {
    DECODER_LOG("Not starting playback [playStatePermits: %d, "
                ...
                (int)mIsVideoPrerolling, (int)mMediaOffloading);
    return;
  }
...
MediaDecoderStateMachine::StartMediaSink()
{
  ...
  if (!mMediaSink->IsStarted()) {
    ...
    //make sure updated mInfo is used to start media sink
