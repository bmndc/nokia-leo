...
MediaFormatReader::OnDemuxerInitDone(nsresult)
{
  ...
  mAudio.mIsEncrypted = mIsEncrypted;
  mVideo.mIsEncrypted = mIsEncrypted;
...
MediaFormatReader::EnsureDecoderCreated(TrackType aTrack)
{
  ...
  auto& platform = decoder.mIsEncrypted ? mEncryptedPlatform : mPlatform;

  if (!platform) {
    platform = new PDMFactory();
    NS_ENSURE_TRUE(platform, false);
    if (decoder.mIsEncrypted) {
#ifdef MOZ_EME
      MOZ_ASSERT(mCDMProxy);
      platform->SetCDMProxy(mCDMProxy);
#else
      // EME not supported.
      return false;
#endif
  ...
  switch (aTrack) {
    case TrackType::kAudioTrack:
      decoder.mDecoder =
        platform->CreateDecoder(decoder.mInfo ?
    ...
    case TrackType::kVideoTrack:
      ...
      decoder.mDecoder =
        platform->CreateDecoder(mVideo.mInfo ?
                                        ...
                                        AbstractMediaDecoder::AutoNotifyDecoded& aA)
{
  ...
  // decode one sample at a time instead of decoding all mQueuedSamples
  // Bug60548 shows that each sample may have different encrypt value,
  // such sample will require different decoder.
  LOGV("Giving %s input to decoder", TrackTypeToStr(aTrack));

  RefPtr<MediaRawData> sample = decoder.mQueuedSamples[0];
  RefPtr<SharedTrackInfo> info = sample->mTrackInfo;

  if ((info && decoder.mLastStreamSourceID != info->GetID()) || sample->mCrypto.mValid != decoder.mIsEncrypted) {
    ...
    decoder.mIsEncrypted = sample->mCrypto.mValid;
