  ...
  if (aOuterWindow) {
    ...
    if (mObserver) {
      ...
      if (os) {
        ...
        os->AddObserver(mObserver, "dom-storage2-full", false);
        os->AddObserver(mObserver, "dom-storage2-free", false);
      }
...
nsGlobalWindow::CleanUp()
{
  ...
  if (mObserver) {
    ...
    if (os) {
      ...
      os->RemoveObserver(mObserver, "dom-storage2-full");
      os->RemoveObserver(mObserver, "dom-storage2-free");
    }
...
nsGlobalWindow::MayResolve(jsid aId)
{
  ...
  if (aId == XPCJSRuntime::Get()->GetStringID(XPCJSRuntime::IDX_CONTROLLERS) ||
      aId == XPCJSRuntime::Get()->GetStringID(XPCJSRuntime::IDX_CONTROLLERS_CLASS)) {
...
bool
nsGlobalWindow::DispatchCustomEventOpenWindow(nsIURI* aURI, const nsAString& aName,
                        const nsACString& aFeatures)
{
  ...
  if (url.IsEmpty()) {
    ...
    detail.mUrl = NS_ConvertUTF8toUTF16(url);
  }
  detail.mName = name;
  detail.mFeatures = NS_ConvertUTF8toUTF16(features);
  detail.mFrameElement = nullptr;
  ...
  domEvent->SetTrusted(true);
  domEvent->WidgetEventPtr()->mFlags.mOnlyChromeDispatch = false;

  nsCOMPtr<EventTarget> target = do_QueryInterface(ToSupports(this));
  domEvent->SetTarget(target);
                        ...
                        const char16_t* aData)
{
  ...
  if (!nsCRT::strcmp(aTopic, "dom-storage2-full")) {
    RefPtr<Event> event = NS_NewDOMEvent(this, nullptr, nullptr);
    event->InitEvent(NS_LITERAL_STRING("storagefull"), false, false);
    event->SetTrusted(true);
  ...
  if (!nsCRT::strcmp(aTopic, "dom-storage2-free")) {
    RefPtr<Event> event = NS_NewDOMEvent(this, nullptr, nullptr);
    event->InitEvent(NS_LITERAL_STRING("storagefree"), false, false);
    event->SetTrusted(true);
