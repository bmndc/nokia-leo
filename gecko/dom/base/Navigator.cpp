...
#include "mozilla/dom/MemoryCleanerManager.h"
...
#include "mozilla/dom/SoundTriggerManager.h"
#endif
#endif
...
#ifdef ENABLE_EMBMS
#include "mozilla/dom/LteBroadcastManager.h"
#endif

#ifdef ENABLE_TEE_SUI
#include "mozilla/dom/TEEReader.h"
#endif
...
#ifdef HAS_KOOST_MODULES
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mMemoryCleanerManager)
...
#endif
#ifdef ENABLE_EMBMS
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mLteBroadcastManager)
#endif
...
#ifdef ENABLE_TEE_SUI
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mTEEReader)
#endif
...
#ifdef HAS_KOOST_MODULES
  if (mMemoryCleanerManager) {
    mMemoryCleanerManager->Shutdown();
    mMemoryCleanerManager = nullptr;
  }
...
#ifdef ENABLE_EMBMS
  if (mLteBroadcastManager) {
    mLteBroadcastManager->Shutdown();
    mLteBroadcastManager = nullptr;
  }
#endif
...
#ifdef ENABLE_RSU
RemoteSimUnlock*
Navigator::GetRsu(ErrorResult& aRv)
{
  if(mRSU){
    return mRSU;
  }

  mRSU = new RemoteSimUnlock(mWindow);
  return mRSU;
}
#endif
...
Navigator::GetMemoryCleanerManager(ErrorResult& aRv)
{
  if (!mMemoryCleanerManager) {
    if (!mWindow) {
      aRv.Throw(NS_ERROR_UNEXPECTED);
    ...
    mMemoryCleanerManager = new MemoryCleanerManager(mWindow);
    mMemoryCleanerManager->Init();
  }

  return mMemoryCleanerManager;
}
...
/* static */
bool
Navigator::HasTEEReaderSupport(JSContext* /* unused */, JSObject* /* unused */)
{
  // Do not support TEE Reader if it's disabled on this device.
  char value[PROPERTY_VALUE_MAX];
  uint32_t len = property_get("ro.teereader.enabled", value, nullptr);
  if (len > 0) {
    return strncmp(value, "true", PROPERTY_VALUE_MAX) == 0;
  }
...
#ifdef ENABLE_SOUNDTRIGGER
SoundTriggerManager*
Navigator::GetSoundTriggerManager(ErrorResult& aRv)
{
  if (!mSoundTriggerManager) {
    if (!mWindow) {
      aRv.Throw(NS_ERROR_UNEXPECTED);
      return nullptr;
    }
    mSoundTriggerManager = SoundTriggerManager::GetInstance(mWindow);
  }

  return mSoundTriggerManager;
}
#endif //ENABLE_SOUNDTRIGGER
#endif // HAS_KOOST_MODULES

#ifdef ENABLE_EMBMS
LteBroadcastManager*
Navigator::GetLteBroadcastManager(ErrorResult& aRv)
{
  if (!mLteBroadcastManager) {
    if (!mWindow) {
      aRv.Throw(NS_ERROR_UNEXPECTED);
    ...
    mLteBroadcastManager = LteBroadcastManager::GetInstance(mWindow);
  }

  return mLteBroadcastManager;
}
#endif

#ifdef ENABLE_TEE_SUI
TEEReader*
Navigator::GetTeeReader(ErrorResult& aRv)
{
  if (!mTEEReader) {
    if (!mWindow) {
      aRv.Throw(NS_ERROR_UNEXPECTED);
    ...
    mTEEReader = TEEReader::GetInstance(mWindow);
  }
