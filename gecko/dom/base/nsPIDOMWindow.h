...
public:
  ...
  /**
   * Dispatch a custom event with openwindow detail targeted at this window.
   * Returns whether the default action should be performed.
   *
   * Outer windows only.
   */
  virtual bool DispatchCustomEventOpenWindow(nsIURI* aURI, const nsAString& aName,
                        const nsACString& aFeatures) = 0;
