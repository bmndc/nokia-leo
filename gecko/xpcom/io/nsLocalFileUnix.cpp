...
NS_IMETHODIMP
nsLocalFile::GetDiskSpaceTotal(int64_t* aDiskSpaceTotal)
{
  if (NS_WARN_IF(!aDiskSpaceTotal)) {
    return NS_ERROR_INVALID_ARG;
  }
  ...
  if (STATFS(mPath.get(), &fs_buf) < 0) {
    // The call to STATFS failed.
#ifdef DEBUG
    printf("ERROR: GetDiskSpaceTotal: STATFS call FAILED. \n");
#endif
    return NS_ERROR_FAILURE;
  }

  *aDiskSpaceTotal = (int64_t)fs_buf.F_BSIZE * fs_buf.f_blocks;

#ifdef DEBUG_DISK_SPACE
  printf("DiskSpaceTotal: %lu bytes\n",
         *aDiskSpaceTotal);
#endif
  ...
  if (!FillStatCache()) {
    // Return aspace total size from statfs
    return NS_OK;
  }
  ...
  if (!quotactl(QCMD(Q_GETQUOTA, USRQUOTA), deviceName.get(),
      ...
      && dq.dqb_bhardlimit) {
    // dqb_bhardlimit is count of BLOCK_SIZE blocks, dqb_curspace is bytes
    *aDiskSpaceTotal = int64_t(BLOCK_SIZE * dq.dqb_bhardlimit);
  }
#endif
   ...
   */
#ifdef DEBUG
  printf("ERROR: GetDiskSpaceTotal: Not implemented for plaforms without statfs.\n");
#endif
  return NS_ERROR_NOT_IMPLEMENTED;
