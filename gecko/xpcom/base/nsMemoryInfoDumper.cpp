...
class SnapshotRunnable final
  : public nsRunnable
  , public nsISnapshotCallback
{
public:
  NS_DECL_ISUPPORTS_INHERITED
  ...
  NS_IMETHOD Run() override
  {
    ...
    if (os) {
      return os->NotifyObservers(nullptr, "heap-snapshot", nullptr);
    }
      return NS_OK;
  }

  NS_IMETHOD OnDump() override
  {
    return NS_OK;
  }
...
NS_IMPL_ISUPPORTS_INHERITED(SnapshotRunnable, nsRunnable,
                            nsISnapshotCallback)
...
static uint8_t sSnapshotSignum;                // SIGRTMIN + 3
...
void doSnapshot(const uint8_t aRecvSig)
{
  LOG("SignalWatcher(sig %d) dispatching snapshot log runnable.", aRecvSig);
  // Dump snapshot logs (from the main thread).

  RefPtr<SnapshotRunnable> runnable = new SnapshotRunnable();
  NS_DispatchToMainThread(runnable);
}
} // namespace
...
void
doSnapshot(const nsCString& aInputStr)
{
  LOG("FifoWatcher(command:%s) dispatching snapshot runnable.", aInputStr.get());
  RefPtr<SnapshotRunnable> runnable = new SnapshotRunnable();
  NS_DispatchToMainThread(runnable);
}
...
SetupFifo()
{
  ...
  fw->RegisterCallback(NS_LITERAL_CSTRING("abbreviated gc log"),
                       doGCCCDump);
  fw->RegisterCallback(NS_LITERAL_CSTRING("snapshot"),
                       doSnapshot);
...
nsMemoryInfoDumper::Initialize()
{
  ...
  // Dump snapshot
  sSnapshotSignum = SIGRTMIN + 3;
  sw->RegisterCallback(sGCAndCCDumpSignum, doSnapshot);
#endif
