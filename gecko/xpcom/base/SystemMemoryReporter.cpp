...
#define REPORT(_path, _amount, _desc) \
                            ...
                            nsISupports* aData, bool aAnonymize) override
  {
    ...
    int64_t totalMem = 0;
    rv = CollectProcessReports(aHandleReport, aData, &totalMem);
    NS_ENSURE_SUCCESS(rv, rv);

    // Report the non-process numbers.
    int64_t other = memTotal - memFree - totalMem;
...
private:
                         ...
                         int64_t* aTotalMem)
  {
    ...
    while (fgets(line, sizeof(line), aFile)) {
      ...
      // Now that we have a name and other metadata, scan for the PSS and SWAP.
      sscanf(line, "Pss: %zu", &pss_kb);
      sscanf(line, "Swap: %zu", &swap_kb);

      // Compute PSS+SWAP only when both have been scanned.
      if (pss_kb == -1 || swap_kb == -1) {
       continue;
      }

      size_t mem = (pss_kb + swap_kb) * 1024;
      if (mem > 0) {
        ...
        REPORT(processMemPath, mem, description);

        // Increment the appropriate aProcessSizes values, and the total.
        aProcessSizes->Add(tag, mem);
        *aTotalMem += mem;
      }

      // Now that we've seen the PSS and SWAP, we're done with this entry.
      currentPath.SetIsVoid(true);
