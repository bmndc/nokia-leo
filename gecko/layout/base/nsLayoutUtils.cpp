...
nsLayoutUtils::ClampRectToScrollFrames(nsIFrame* aFrame, const nsRect& aRect)
{
  ...
  float zoomLevel = [&]() {
    float fullZoom = aFrame->PresContext()->GetFullZoom();
    float resolution = aFrame->PresContext()->GetPresShell()->GetCumulativeResolution();
  ...
  while (closestScrollFrame) {
    ...
    // Consider the user-zoom, clamp the scrollPortRect properly.
    scrollPortRect.width /= zoomLevel;
    scrollPortRect.height /= zoomLevel;
...
Element*
nsLayoutUtils::FindElementAncestor(nsIContent* aContent)
{
