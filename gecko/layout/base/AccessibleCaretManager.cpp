...
/* static */ bool
AccessibleCaretManager::sSelectionBarCursorEnabled = false;
/* static */ bool
  ...
  : mPresShell(aPresShell)
{
  ...
  if (!addedPrefs) {
    ...
    Preferences::AddBoolVarCache(&sSelectionBarCursorEnabled,
                                 "layout.accessiblecaret.bar.cursor_enabled", false);
...
void
AccessibleCaretManager::UpdateCarets(const UpdateCaretsHintSet& aHints)
{
  if (!FlushLayout()) {
    return;
  }
  ...
  case CaretMode::Cursor:
    UpdateCaretsForCursorMode(aHints);
    break;
  case CaretMode::Selection:
    UpdateCaretsForSelectionMode(aHints);
    break;
  }
}
...
AccessibleCaretManager::UpdateCaretsForCursorMode(const UpdateCaretsHintSet& aHints)
{
  ...
  switch (result) {
    ...
    case PositionChangedResult::Changed:
      if (!aHints.contains(UpdateCaretsHint::RespectOldAppearance)
        || mFirstCaret->GetAppearance() == Appearance::NormalNotShown) {
  ...
  mFirstCaret->SetSelectionBarEnabled(sSelectionBarCursorEnabled);
  mSecondCaret->SetAppearance(Appearance::None);
  ...
  if (sSelectionBarCursorEnabled && !mCaretHide) {
    RefPtr<nsCaret> caret = mPresShell->GetCaret();
    caret->AddForceHide();
    mCaretHide = true;
  }
...
AccessibleCaretManager::UpdateCaretsForSelectionMode(const UpdateCaretsHintSet& aHints)
{
                                   ...
                                   int32_t aOffset) -> PositionChangedResult
  {
    ...
    switch (result) {
      ...
      case PositionChangedResult::Changed:
        if (!aHints.contains(UpdateCaretsHint::RespectOldAppearance)
          || aCaret->GetAppearance() == Appearance::NormalNotShown) {
...
AccessibleCaretManager::PressCaret(const nsPoint& aPoint)
{
  ...
  if (mActiveCaret) {
    // Since the caret visual is under the text line, User may drag
    // to the next line by accident. We move the "control" position
    // slightly higher (AccessibleCaret::sHeight/2) than the finger
    // position to match user's feel better.
    mOffsetYToCaretLogicalPosition =
      mActiveCaret->LogicalPosition().y - aPoint.y
      - nsPresContext::CSSPixelsToAppUnits(AccessibleCaret::sHeight / 2);
...
bool
AccessibleCaretManager::MaybeRedispatchSelectWordToOuterMost(
    const nsPoint& aPoint, nsresult* rv)
{
  AutoRestore<bool> redispatching(mIsRedispatched);
  mIsRedispatched = true;

  nsCOMPtr<nsPIDOMWindowOuter> topWin =
      mPresShell->GetDocument()->GetWindow()->GetTop();
  nsCOMPtr<nsIDocument> doc = topWin.get() ? topWin->GetDoc() : nullptr;
  nsCOMPtr<nsIPresShell> presShell = doc.get() ? doc->GetShell() : nullptr;
  if (!presShell.get() || presShell.get() == mPresShell) {
    return false;
  }

  nsPoint ptInFrame = aPoint;
  nsLayoutUtils::TransformPoint(
      mPresShell->GetRootFrame(), presShell->GetRootFrame(), ptInFrame);

  AC_LOG("redispatch SelectWord to TOP window");
  RefPtr<AccessibleCaretEventHub> eventHub =
    presShell->GetAccessibleCaretEventHub();
  *rv = eventHub->mManager->SelectWordOrShortcut(ptInFrame);
  return true;
}

bool
AccessibleCaretManager::MaybeReDispatchSelectWordToInner(
    nsIFrame* aFrame, const nsPoint& aPoint, nsresult* rv)
{
  AutoRestore<bool> redispatching(mIsRedispatched);
  mIsRedispatched = true;

  nsIContent* node = aFrame->GetContent();
  nsCOMPtr<nsIDocument> doc = node->GetComposedDoc();

  nsCOMPtr<nsIDocument> subDoc = do_QueryInterface(doc->GetSubDocumentFor(node));
  nsCOMPtr<nsIPresShell> subPresShell = subDoc.get() ? subDoc->GetShell() : nullptr;
  if (!subPresShell.get() || mPresShell == subPresShell.get()) {
    return false;
  }

  AC_LOG("redispatch SelectWord to inner window");
  RefPtr<AccessibleCaretEventHub> eventHub =
      subPresShell->GetAccessibleCaretEventHub();
  *rv = eventHub->mManager->SelectWordOrShortcut(aPoint);
  return true;
}
...
AccessibleCaretManager::SelectWordOrShortcut(const nsPoint& aPoint)
{
  ...
  if (!ptFrame.IsAlive()) {
    // User want to select a word in <iframe> without focus it.
    // Check if ptFrame is invliad, re-dispatch to the outter-most document.
    nsresult rv;
    if (MaybeRedispatchSelectWordToOuterMost(aPoint, &rv)) {
      return rv;
    }
    return NS_ERROR_FAILURE;
  }
  ...
  // User want to select a word in <iframe> without focus it.
  // Check if ptFrame is a nsIDocument, re-dispatch to the inner document.
  nsresult rv;
  if (MaybeReDispatchSelectWordToInner(ptFrame, ptInFrame, &rv)) {
    return rv;
  }
  ...
  rv = SelectWord(aPoint, ptFrame, ptInFrame, &isOnEmpty);

  UpdateCaretsWithHapticFeedback();
  if (isOnEmpty && focusableFrame) {
    DispatchCaretStateChangedEvent(CaretChangedReason::Longpressonemptycontent);
  }
...
void
AccessibleCaretManager::OnFocus()
{
  if (mLastUpdateCaretMode != GetCaretMode()) {
    AC_LOG("%s: do nothing", __FUNCTION__);
    return;
  }

  AC_LOG("%s: show", __FUNCTION__);
  UpdateCarets();
}
...
nsresult
AccessibleCaretManager::SelectWord(const nsPoint& aRootPoint,
                                   nsIFrame* aFrame,
                                   const nsPoint& aPoint,
                                   bool *outIsOnEmpty) const
{
  SetSelectionDragState(true);
  ...
  // KaiOS Bug 79002:
  // For KaiOS's UX requirments, we want to select a word if and only if
  // the user touchs on a word. Since the SelectByTypeAtPoint() will try
  // to select the closet word, we need to check again if the result met
  // our expectation or not.
  if (!GetSelection()->ContainsPoint(aRootPoint)) {
    // Do a single click instead if we does not touch on a word.
    nsIFrame::ContentOffsets offsets =
      frame->GetContentOffsetsFromPoint(aPoint);
    if (!offsets.IsNull()) {
      AC_LOG("Touch on empty space => do a 'click'.");
      RefPtr<nsFrameSelection> fs = GetFrameSelection();
      fs->HandleClick(offsets.content,
...
// Retrun a frame if it is visible.
//   IsHTMLElement() & GetPrimaryFrame(), to skip <script> <br /> .. etc.
//   TextIsOnlyWhitespace(), to skip empty text node.
static
nsIFrame* RetriveVisibleFrame(nsIContent* node) {
  if (node->IsHTMLElement() || !node->TextIsOnlyWhitespace())
    return node->GetPrimaryFrame();
  return nullptr;
}

// Trim the node in DOM tree to find the innermost valid node by RetriveVisibleFrame().
// The dom::TreeWalker is not suitable here, since the API is difficult to know which
// we are stay in the deepest node or not.
static
nsIFrame* FindNearestFrameForNodeOffset(
  nsIContent* aParentNode, int32_t aStartOffset,
  nsDirection aDirection, int32_t* aOutOffset)
{
  int32_t outterOffset = 0;
  nsIFrame* outterFrame = nullptr;
  nsIContent* parentNode = aParentNode;
  int32_t startOffset = aStartOffset;

  // Find the target by greedy search:
  // each loop we do a linear scan and stop at the fist valid node we met.
  while (true) {
    const int32_t lastIndex = parentNode->GetChildCount() - 1;
    nsIFrame* frame = nullptr;
    if (aDirection == eDirNext) {
      // Range: max(startOffset, 0) up to lastIndex
      for (int32_t i = std::max(startOffset, 0); !frame && i <= lastIndex; ++i) {
        frame = RetriveVisibleFrame(parentNode->GetChildAt(i));
      }
    } else {
      // Range: min(startOffset, lastIndex) down to 0
      for (int32_t i = std::min(startOffset, lastIndex); !frame && i >= 0; --i) {
        frame = RetriveVisibleFrame(parentNode->GetChildAt(i));
      }
    }
    ...
    parentNode = frame->GetContent();

    if (aDirection == eDirNext) {
      startOffset = 0;
    } else {
      startOffset = parentNode->IsHTMLElement()
             ? parentNode->GetChildCount()
             : parentNode->TextLength();
    }

    outterOffset = startOffset;
    outterFrame = frame;
  }

  if (outterFrame) {
    if (aOutOffset) {
      *aOutOffset = outterOffset;
    }
  }
  ...
  int32_t* aOutNodeOffset) const
{
  ...
  if (!startFrame) {
    // The selected node does not have a frame (invisible),
    // we try to find the nearest one instead.
    startFrame = FindNearestFrameForNodeOffset(
        startContent, nodeOffset, aDirection, aOutOffset);
  }

  if (startFrame) {
    if (aOutNode) {
      *aOutNode = startFrame->GetContent();
    }
    if (aOutNodeOffset) {
