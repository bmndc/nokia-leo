...
public:
  ...
  // Handle focus event from nsFocusManager.
...
protected:
  ...
  nsresult SelectWord(const nsPoint& aRootPoint, nsIFrame* aFrame,
    const nsPoint& aPoint, bool *outIsOnEmpty) const;
  ...
  bool MaybeRedispatchSelectWordToOuterMost(
      const nsPoint& aPoint, nsresult* rv);
  bool MaybeReDispatchSelectWordToInner(
  ...
  // Set to true to return immediately to avoid recursive.
  bool mIsRedispatched = false;
  ...
  // Preference to show selection bars at the two ends in selection mode.
  static bool sSelectionBarEnabled;

  // Preference to show selection bars at the two ends in cursor mode.
  static bool sSelectionBarCursorEnabled;
