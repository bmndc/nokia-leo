...
public:
  ...
  friend class AccessibleCaretManager;

protected:

  // This enumeration representing the coordinate reference used by
  // AccessibleCaret.
  //
  // Use SetPositionMode() to change the mode, and use
  // GetPositionMode() to get the current mode.
  enum class PositionMode : uint8_t {
    // relative to CustomContentContainerFrame().
    Absolute,

    // relative to RootFrame().
    Fixed,
  };

  // Set PositionMode, return true if changed.
  // return false, otherwise.
  bool SetPositionMode(PositionMode aMode);
  PositionMode GetPositionMode();

  // Argument aRect should be
  // relative to CustomContentContainerFrame() if GetPositionMode() is Absolute.
  // relative to RootFrame(), if GetPositionMode() is Fixed.
  ...
  Appearance mLastAppearnce = Appearance::Normal;
  PositionMode mPositionMode = PositionMode::Absolute;
  ...
  // mImaginaryCaretRect is relative to container frame.
  ...
  nsIFrame* mImaginaryCaretReferenceFrame;

  // Cache current rootRect to determine whether viewport is changed.
  nsRect mRootRect;

  // Cache current zoom level to determine whether position is changed.
  float mZoomLevel = 0.0f;

  // Force the apperance to "in" the border
  // if layout.accessiblecaret.always_in_border is set.
  AccessibleCaret::Appearance mForceAppearance = AccessibleCaret::Appearance::None;
  ...
  static bool sNormalAsRight;
  static bool sAlwaysInBorder;
  static float sBorderBottom;
