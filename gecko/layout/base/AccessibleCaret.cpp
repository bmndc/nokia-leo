...
bool AccessibleCaret::sNormalAsRight = false;
bool AccessibleCaret::sAlwaysInBorder = false;
float AccessibleCaret::sBorderBottom = 0.0f;
...
static nscolor
GetBackgroundColorFromFrameForMozSelection(nsIPresShell* aPresShell, nsIFrame* aFrame)
{
  nsIFrame* nonGeneratedAncestor = nsLayoutUtils::GetNonGeneratedAncestor(aFrame);
  Element* selectionElement =
    nsLayoutUtils::FindElementAncestor(nonGeneratedAncestor->GetContent());

  if (selectionElement) {
    RefPtr<nsStyleContext> sc =
      aPresShell->GetPresContext()->StyleSet()->
      ProbePseudoElementStyle(selectionElement,
                              CSSPseudoElementType::mozSelection,
                              aFrame->StyleContext());

    if (sc) {
      return sc->GetVisitedDependentColor(eCSSProperty_background_color);
    }
  }

  return NS_DONT_CHANGE_COLOR;
}

static bool
IsFixedToViewport(const nsIFrame* aFrame) {
  auto CheckIsFixed = [](const nsIFrame* aFrame) {
    if (aFrame->StyleDisplay()->IsPositionForcingStackingContext()) {
  ...
  : mPresShell(aPresShell)
{
  ...
  if (!prefsAdded) {
    ...
    Preferences::AddBoolVarCache(&sNormalAsRight, "layout.accessiblecaret.normal_as_right");
    Preferences::AddBoolVarCache(&sAlwaysInBorder, "layout.accessiblecaret.always_in_border");
    Preferences::AddFloatVarCache(&sBorderBottom, "layout.accessiblecaret.border-bottom");
    prefsAdded = true;
  }
}
...
{
  if (aAppearance == Appearance::NormalNotShown
      && mAppearance != Appearance::None
      && mAppearance != Appearance::NormalNotShown) {
    // Remember the appearance when set to "NotShown".
    // Only if the appearance is not "None".
    mLastAppearnce = mAppearance;
  } else if (aAppearance == Appearance::Normal) {
    // If set to Normal, overwrite to the LastAppearnce.
    aAppearance = mLastAppearnce;
  }

  Appearance appearance = aAppearance;

  if (mForceAppearance != Appearance::None
      && appearance != Appearance::None
      && appearance != Appearance::NormalNotShown) {
    appearance = mForceAppearance;
  } else if (sNormalAsRight && aAppearance == Appearance::Normal) {
    appearance = Appearance::Right;
  }
...
AccessibleCaret::SetPosition(nsIFrame* aFrame, int32_t aOffset)
{
  ...
  nsRect imaginaryCaretIdeaRectInFrame =
    nsCaret::GetGeometryForFrame(aFrame, aOffset, nullptr);

  nsRect imaginaryCaretRectInFrame =
    nsLayoutUtils::ClampRectToScrollFrames(aFrame, imaginaryCaretIdeaRectInFrame);
  ...
  // If zoom and rootRect is not changed, we may skip to update.
  if (FuzzyEqualsMultiplicative(zoomLevel, mZoomLevel) && rootRect.IsEqualEdges(mRootRect)) {
    bool posInRootEqual = imaginaryCaretRect.IsEqualEdges(mImaginaryCaretRect);
    bool posInCanvasEqual = imaginaryCaretRectInContainerFrame
                            .IsEqualEdges(mImaginaryCaretRectInContainerFrame);

    // Skip if
    // 1. RootRect and CanvasRect are both not changed.
    // 2. Position is in Fixed mode and RootRect is not changed.
    // 3. Position is in Absolute mode and CanvasRect is not changed.
    //
    // For 3, the root is moved relative to the canvas and the feature::AlwaysInBorder may
    // update Appearance when caret is close to the boundary, so we cannot skip in that case.
    if ( (posInRootEqual && posInCanvasEqual)
      || (posInRootEqual && !SetPositionMode(PositionMode::Fixed))
      || (posInCanvasEqual && !SetPositionMode(PositionMode::Absolute) && !sAlwaysInBorder)
      ) {
      return PositionChangedResult::NotChanged;
    }
  }
  ...
  // To reduce scroll jank, we set the css:position to the same mode as the
  // attached aFrame. It will reduce #repaint and remove the jank totally
  // when moving around the container frame, e.g. <html> and <iframe>.
  //
  // For other cases, to scroll a overflow:auto cotent in a element, the
  // position of caret is still one frame behind to follow.
  SetPositionMode(
    IsFixedToViewport(aFrame) ?
    PositionMode::Fixed : PositionMode::Absolute
  );

  // Select the location by the setting of PositionMode
  nsRect caretRect = (GetPositionMode() == PositionMode::Absolute)
      ? mImaginaryCaretRectInContainerFrame
      : mImaginaryCaretRect;

  // Shrink height to let caret align to the boundary, the root frame,
  // the maximal offset to be moved is the height of caret.
  if (sAlwaysInBorder) {
    nsRect originRect = imaginaryCaretIdeaRectInFrame;
    nsLayoutUtils::TransformRect(aFrame, rootFrame, originRect);

    // To shrink the caret's height if it touch to the bottom
    nscoord appUnitHeight = nsPresContext::CSSPixelsToAppUnits(sHeight - sBorderBottom);
    originRect.height += appUnitHeight;

    if (originRect.YMost() > rootRect.YMost()) {
      caretRect.height = imaginaryCaretIdeaRectInFrame.height - appUnitHeight;
    }

    // To limit the appearance if caret is nearby the left or right border.
    nscoord appUnitWidth = nsPresContext::CSSPixelsToAppUnits(sWidth);
    if (originRect.X() < appUnitWidth) {
      mForceAppearance = Appearance::Right;
    } else if (originRect.XMost() + appUnitWidth > rootRect.XMost()) {
      mForceAppearance = Appearance::Left;
    } else {
      mForceAppearance = Appearance::None;
    }
  }

  SetCaretElementStyle(caretRect, mZoomLevel);
  SetSelectionBarElementStyle(caretRect, mZoomLevel);
...
AccessibleCaret::SetCaretElementStyle(const nsRect& aRect, float aZoomLevel)
{
  ...
  styleStr.AppendPrintf(" --caret-width: %.2fpx; --caret-height: %.2fpx;",
                        sWidth, sHeight);

  if (GetPositionMode() == PositionMode::Fixed) {
    styleStr.AppendPrintf("position: fixed;");
  }

  nscolor color = GetBackgroundColorFromFrameForMozSelection(
      mPresShell, mImaginaryCaretReferenceFrame);

  if (color != NS_DONT_CHANGE_COLOR) {
...
bool
AccessibleCaret::SetPositionMode(PositionMode aMode)
{
  if (mPositionMode == aMode) {
    return false;
  }
...
AccessibleCaret::PositionMode
AccessibleCaret::GetPositionMode()
{
  return mPositionMode;
}

void
AccessibleCaret::ResetCache()
{
