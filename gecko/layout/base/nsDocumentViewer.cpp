...
{
  // Ensure AccessibleCarets are dismissed before closing the document.
  // XXX: Move code into AccessibleCaretEventHub::Terminate() will not work.
  // The document is invalid wheh Terminate() is called,
  // we will fail to send events to chrome process.
  RefPtr<AccessibleCaretEventHub> eventHub =
    mPresShell ? mPresShell->GetAccessibleCaretEventHub() : nullptr;
  if (eventHub) {
    // To dismiss the caret (and action-menu implicitly).
    eventHub->NotifyBlur(true);
  }
...
NS_IMETHODIMP nsDocumentViewer::CopyLinkLocation()
{
  ...
  // KaiOS Bug 82891: SPEC: remove specific protocals' prefix
  const char* protocolToRemove[] = {
    "mailto:", "tel:", "callto:",
  };
  for (auto protocol: protocolToRemove) {
    nsAutoCString prefix(protocol);
    if (locationText.EqualsIgnoreCase(prefix.get(), prefix.Length())) {
      locationText = Substring(locationText, prefix.Length(), -1);
      locationText.Trim(" \t\n\f\r");
      break;
    }
  }
