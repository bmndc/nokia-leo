...
void
TouchManager::MaybeInitForTouchLocation()
{
  if (mTouchLocation) {
    return;
  }
  ...
  // Only for the outermost frame.
  nsCOMPtr<nsPIDOMWindowOuter> window
      = mPresShell->GetDocument()->GetWindow();
  nsCOMPtr<nsPIDOMWindowOuter> windowParent
      = window ? window->GetParent() : nullptr;
  if (windowParent && window != windowParent) {
    return;
  }
