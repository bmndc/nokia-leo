...
nsIFrame::PeekOffset(nsPeekOffsetStruct* aPos)
{
  ...
  switch (aPos->mAmount) {
    ...
    case eSelectWord:
    {
      ...
      // Treat the line break as a word boundary.
      bool wordSelectStopAtLine = Preferences::GetBool("layout.word_select.stop_at_line", true); 
      ...
      while (!done) {
        ...
        if (!done) {
          ...
          if (NS_FAILED(result) ||
              (wordSelectStopAtLine && jumpedLine && !wordSelectEatSpace && state.mSawBeforeType)) {
            done = true;
            ...
            if (wordSelectStopAtLine && jumpedLine) {
