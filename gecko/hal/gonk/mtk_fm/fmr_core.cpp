...
int FMR_pwr_up(int idx, int freq)
{
	...
	//following code is temporary, yht

	LOGI("%s,[freq=%d],[low_band=%d] [high_band=%d]\n", __func__, freq,fmr_data.cfg_data.low_band,fmr_data.cfg_data.high_band);
...
{
    fm_s32 i = 0;//, j = 0;//for unused error
...
int FMR_seek(int idx, int start_freq, int dir, int *ret_freq)
{
    ...
    LOGI("Tune to %d", *ret_freq);
    ret = FMR_tune(idx, *ret_freq);
    return ret;
}
...
int FMR_scan(int idx, uint16_t *scan_tbl, int *max_cnt)
{
    ...
    fm_u8 NF_Space = 41;
	LOGD("%s, [seek_space=%d] [band=%d]\n", __func__, fmr_data.cfg_data.seek_space, fmr_data.cfg_data.band);
	...
	LOGD("%s, [Start_Freq=%d] [NF_Space=%d],band_channel_no %d\n", __func__, Start_Freq, NF_Space,band_channel_no);
