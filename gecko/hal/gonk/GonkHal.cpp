...
#if defined(SPEAKER_VIBRATE_SUPPORT)
      if ((mPattern.Length() == 1) && (duration == 0)) {
        AudioSystem::setParameters(0, String8("vibrate_enabled=false"));
      } else if (mIndex % 2 == 0) {
        String8 cmd("vibrate_enabled=true;");
        cmd.appendFormat("vibrate_duration=%d", duration);
...
SetScreenEnabled(bool aEnabled)
{
  ...
  bool state = GetGonkDisplay()->GetSecureUIState();
  if (state == true) {
    printf_stderr("SecureUI state=%d, skip SetScreenEnabled(%d)", state, aEnabled);
    return;
  }
#endif
...
{
#ifdef ENABLE_TEE_SUI
  bool sui_state = GetGonkDisplay()->GetSecureUIState();
   if (sui_state == true) {
     HAL_LOG("SecureUI state=%d, skip SetScreenLight", sui_state);
     return;
   }
#endif
...
EnableAlarm()
{
  ...
  if (strcmp(useTimerfd, "true") == 0) {
    ...
    if (alarmFd < 0) {
      HAL_ERR("Failed to create timerfd for CLOCK_POWEROFF_ALARM: %s.", strerror(errno));

      // In case kernel doesn't support CLOCK_POWEROFF_ALARM, we use CLOCK_REALTIME_ALARM
      alarmFd = timerfd_create(CLOCK_REALTIME_ALARM, 0);
      if (alarmFd < 0) {
        HAL_ERR("Failed to create timerfd for CLOCK_REALTIME_ALARM: %s.", strerror(errno));
        return false;
      }
    }
  } else {
    ...
    if (alarmFd < 0) {
      HAL_ERR("Failed to open alarm device: %s.", strerror(errno));
      return false;
    }
  }
  ...
  if (sigaction(SIGUSR1, &actions, nullptr)) {
    HAL_ERR("Failed to set SIGUSR1 signal for alarm-watcher thread.");
    return false;
  }
  ...
  if (status) {
    alarmData.reset();
    HAL_ERR("Failed to create alarm-watcher thread. Status: %d.", status);
    return false;
  }
...
public:
      ...
      mRegexes(nullptr)
  {
    ...
    NS_NewNamedThread("OomLogger", getter_AddRefs(mIOThread));
  }
  ...
  const char16_t* aData)
{
  ...
  nsCOMPtr<nsIRunnable> runnable =
    NS_NewRunnableMethod(this, &OomVictimLogger::ParseLog);
