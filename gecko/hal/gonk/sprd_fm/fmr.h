...
#define HEADSET_STATE_PATH_9820E "/sys/kernel/headset/state"
...
struct fm_cbk_tbl {
    ...
    int (*ana_switch_inner)(int fd, int antenna);
	...
	int (*set_dcc_bw)(int fd, int bw);
	int (*set_vol)(int fd, int vol);
	int (*get_vol)(int fd, int *vol);
};
...
int FMR_scan(int idx, uint16_t *tbl, int *num, int startFreq);
