  ...
  NS_IMETHOD Run()
  {
    ...
    if (obs) {
      obs->NotifyObservers(nullptr, "almost-low-disk-space",
                          mLowDiskSpace ? MOZ_UTF16("true") : MOZ_UTF16("false"));
    } else {
...
void
GonkDiskSpaceWatcher::NotifyAlmostLowDiskSpace(bool aLowDiskSpace) {
  nsCOMPtr<nsIRunnable> runnable = new AlmostLowDiskSpaceNotifier(aLowDiskSpace);
  NS_DispatchToMainThread(runnable);
}
