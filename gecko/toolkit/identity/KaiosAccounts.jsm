...
this.EXPORTED_SYMBOLS = ["KaiosAccounts"];
...
#else
log.warn("The KaiAccountsManager is only functional in B2G at this time.");
var KaiAccountsManager = null;
var ONVERIFIED_NOTIFICATION = null;
var ONLOGIN_NOTIFICATION = null;
var ONLOGOUT_NOTIFICATION = null;
#endif

function KaiAccountsService() {
  ...
  this.kaiAccountsManager = KaiAccountsManager;
}

KaiAccountsService.prototype = {
  ...
  watch: function watch(aRpCaller) {
    ...
    let runnable = {
      run: () => {
        this.kaiAccountsManager.getAssertion(aRpCaller.audience,
                                             aRpCaller.principal,
                                             { silent:true }).then(
          data => {
            if (data) {
  ...
  request: function request(aRPId, aOptions) {
    ...
    this.kaiAccountsManager.getAssertion(rp.audience, rp.principal, options)
    .then(
      data => {
  ...
  doLogout: function doLogout(aRpCallerId, aErrorReason) {
    let rp = this._rpFlows.get(aRpCallerId);
    if (!rp) {
...
this.KaiosAccounts = new KaiAccountsService();
