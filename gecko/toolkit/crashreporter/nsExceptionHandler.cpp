                      ...
                      bool succeeded)
{
  ...
  if (prefs) {
    // Report reboot reason type only.
    (void) prefs->SetCharPref("device.rebootReason", "b2g crash");
    // Flush pref file immediately.
    nsCOMPtr<nsIPrefService> prefServ = do_QueryInterface(prefs);
    nsresult rv = prefServ->SavePrefFile(nullptr);
    if (NS_FAILED(rv)) {
      NS_WARNING("Could not write rebootReason into pref file.");
    }
  }
