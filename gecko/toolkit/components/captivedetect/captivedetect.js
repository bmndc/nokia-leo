...
function CaptivePortalDetector() {
  ...
  try {
    this.isTestingModeEnabled =
      Services.prefs.getBoolPref('captivedetect.test.enabled');
  } catch(e) {
    this.isTestingModeEnabled = false;
    debug('captivedetect.test.enabled not set, default set to disabled.');
  }
  ...
  debug('Load Prefs {site=' + this._canonicalSiteURL
        ...
        + ' ,test mode enabled=' + this.isTestingModeEnabled
...
CaptivePortalDetector.prototype = {
  ...
  checkCaptivePortal: function checkCaptivePortal(aInterfaceName, aCallback) {
    if (this.isTestingModeEnabled) {
      return aCallback.complete(true);
    }
