...
#if defined(WEBRTC_GONK) && defined(ENABLE_SOUNDTRIGGER)
...
int32_t OpenSlesInput::RecordingDeviceName(uint16_t index,
  ...
  } else if (index == kAudioSourceIndexFM) {
    strcpy (name, "fm");
  } else if (index == kAudioSourceIndexVoiceRecognition) {
    strcpy (name, "voicerecognition");
  } else {
    assert(false);
  }
...
int32_t OpenSlesInput::SetRecordingDevice(uint16_t index) {
  if (index >= kAudioSourceIndexCount) {
    return -1;
  }
  audio_source_ = static_cast<AudioSourceEnum>(index);
  return 0;
}
...
// some soundtrigger vendors need specific sample rate for better
// result.
#if defined(WEBRTC_GONK) && defined(ENABLE_SOUNDTRIGGER)
  if (audio_source_ == kAudioSourceIndexVoiceRecognition) {
    char value[PROPERTY_VALUE_MAX];
    // 44100 is the default input sample rate on Android
    property_get("audio.input.recognition.freq", value, "44100");

    int samplerate = atoi(value);
    if (samplerate != rec_sampling_rate_) {
      rec_sampling_rate_ = samplerate;
...
bool OpenSlesInput::SetupRecordingPreset() {
  SLuint32 preset = SL_ANDROID_RECORDING_PRESET_NONE;
  SLuint32 presetSize = sizeof(preset);

  switch (audio_source_) {
    case kAudioSourceIndexMIC:
...
#if defined(WEBRTC_GONK)
    case kAudioSourceIndexVoiceCall:
      #if defined(CALL_RECORDING_SUPPORT) || defined(PRODUCT_MANUFACTURER_QUALCOMM)
      preset = SL_ANDROID_RECORDING_PRESET_VOICE_CALL;;
      #else
      preset = SL_ANDROID_RECORDING_PRESET_GENERIC;
      #endif
      break;
    ...
    case kAudioSourceIndexFM:
      preset = SL_ANDROID_RECORDING_PRESET_FM;
      break;
#endif
#endif
    case kAudioSourceIndexVoiceRecognition:
  ...
  res = (*configItf)->SetConfiguration(configItf,
                                       ...
                                       &preset, presetSize);
  WEBRTC_TRACE(kTraceError, kTraceAudioDevice, 0, "OpenSL Set recording preset res: %d", res);
  if (res != SL_RESULT_SUCCESS) {
    return false;
  }
  return true;
...
void OpenSlesInput::SetupAECAndNS() {
  if (audio_source_ == kAudioSourceIndexFM) {
    return;
  }
