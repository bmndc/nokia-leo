                                         ...
                                         uint32_t      redirectFlags)
{
  ...
  // Copy User-Agent to the new channel - see bug 61274.
  {
    nsCString ua;
    nsresult hasHeader = mRequestHead.GetHeader(nsHttp::User_Agent, ua);
    if (NS_SUCCEEDED(hasHeader)) {
      httpChannel->SetRequestHeader(NS_LITERAL_CSTRING("User-Agent"), ua, false);
    }
  }
