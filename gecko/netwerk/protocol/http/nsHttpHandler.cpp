...
nsHttpHandler::BuildUserAgent()
{
    ...
    char product_name_prop[PROPERTY_VALUE_MAX];
    property_get("ro.product.name", product_name_prop, NULL);
    LOG(("miao product_name_prop:%s",product_name_prop));
    //string product_name=product_name_prop;
   // mProductModel.AssignLiteral(product_name_prop);
    ...
    if (sizeof(product_name_prop)>0) {
        mUserAgent += product_name_prop;
        mUserAgent.AppendLiteral("; ");
...
#ifdef KAIOS_256MB_SUPPORT
    mUserAgent.AppendLiteral("; ");
    mUserAgent.AppendLiteral("K-Lite");
#endif
    ...
    if (!mUiVersion.IsEmpty()) {
        mUserAgent.AppendLiteral(" ");
        mUserAgent += mUiVersion;
    }
}
...
nsHttpHandler::InitUserAgentComponents()
{
    ...
    nsCString uiVersion;
    rv = Preferences::GetCString("gaia.ui.version", &uiVersion);
    if (NS_SUCCEEDED(rv)) {
        mUiVersion = uiVersion;
    }
#endif
