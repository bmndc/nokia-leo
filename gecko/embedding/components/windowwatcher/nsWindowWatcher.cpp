  ...
  if (!newDocShellItem) {
    ...
    if (!aDialog && !chromeWin &&
      ...
      if (provider) {
        ...
        nsCOMPtr<nsPIDOMWindowOuter> itemWin;
        if (parentTreeOwner) {
          parentTreeOwner->GetPrimaryContentShell(getter_AddRefs(item));
        }
        if (item) {
          itemWin = item->GetWindow();
        }
        if (itemWin) {
          itemWin->DispatchCustomEventOpenWindow(uriToLoad, name, features);
        }
