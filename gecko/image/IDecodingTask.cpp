...
/* static */ void
IDecodingTask::NotifyProgress(Decoder* aDecoder)
{
...
static void
NotifyDecodeComplete(Decoder* aDecoder)
{
  // Synchronously notify if we can.
  if (NS_IsMainThread() &&
      !(aDecoder->GetDecoderFlags() & DecoderFlags::ASYNC_NOTIFY)) {
...
DecodingTask::DecodingTask(Decoder* aDecoder)
  : mDecoder(aDecoder)
{
...
DecodingTask::Run()
{
  nsresult rv = mDecoder->Decode(this);
...
MetadataDecodingTask::MetadataDecodingTask(Decoder* aDecoder)
  : mDecoder(aDecoder)
{
  MOZ_ASSERT(mDecoder->IsMetadataDecode(),
...
AnonymousDecodingTask::AnonymousDecodingTask(Decoder* aDecoder)
  : mDecoder(aDecoder)
{ }
