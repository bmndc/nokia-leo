                                                   ...
                                                   uint32_t aFlags)
{
              ...
              targetIsViewSource))
    {
        ...
        if (hasFlags) {
            ...
            // Allow apps to load scripts from this perticular domain, they
            // don't need special permission, but has to be certified type
            // (checked in CSPService).
            auto sharedOrigin = NS_LITERAL_CSTRING("app://shared.gaiamobile.org");
            if (targetOrigin.Equals(sharedOrigin)) {
              return NS_OK;
            }
