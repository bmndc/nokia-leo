...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		...
		if (tasksize <= 0)
			continue;
		if (strcmp(p->comm, "Launcher") == 0 && min_score_adj > 400) {
			lowmem_print(2, "not kill launcher if min_score_adj > 400\n");
			continue;
		} else if (strcmp(p->comm, "Music") == 0 && 400 == oom_score_adj && min_score_adj >= 400) {
			lowmem_print(2, "not kill Music if oom_score_adj = 400, min_score_adj >= 400\n");
			continue;
		}
		if (selected) {
