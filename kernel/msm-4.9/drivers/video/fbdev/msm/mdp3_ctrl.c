					...
					u32 cmd, void __user *argp)
{
	...
	if (!mdp3_session)
    {
        pr_err("mdp3_ctrl_ioctl_handler, session is null\n");
		return -ENODEV;
    }
	...
	//pr_debug("mdp3_ctrl_ioctl_handler,cmd=0x%x\n",cmd);
...
int mdp3_ctrl_init(struct msm_fb_data_type *mfd)
{
	...
	/* Enable PM runtime */
#if 0
      pr_info("mdp3_ctrl_init: dev->power.usage_count=%d\n",mdp3_res->pdev->dev.power.usage_count.counter);
	pm_runtime_set_suspended(&mdp3_res->pdev->dev);

      pr_info("mdp3_ctrl_init: dev->power.usage_count=%d\n",mdp3_res->pdev->dev.power.usage_count.counter);
	pm_runtime_enable(&mdp3_res->pdev->dev);

       pr_info("mdp3_ctrl_init: dev->power.usage_count=%lu\n",mdp3_res->pdev->dev.power.usage_count);
      pr_info("mdp3_ctrl_init: dev->power.usage_count=%d\n",mdp3_res->pdev->dev.power.usage_count.counter);
#endif
