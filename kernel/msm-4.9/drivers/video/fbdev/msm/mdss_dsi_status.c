...
static void check_dsi_ctrl_status(struct work_struct *work)
{
	...
	pr_info("%s: call check_dis_status,pdsi_status=0x%p\n", __func__,pdsi_status);

	pr_info("%s: check_dsi_status=0x%p\n", __func__,&pdsi_status->mfd->mdp.check_dsi_status);
				...
				unsigned long event, void *data)
{
	...
	pr_info("%s:mdss_dsi_status  panel name:%s,event=%lu\n", __func__, pinfo->panel_name,event);

	if(strstr(pinfo->panel_name, "ext panel")){
		pr_info("%s: ext panel not do esd check, return\n", __func__);
	        return NOTIFY_DONE;
	}
	...
	if (event == FB_EVENT_BLANK) {
		...
		case FB_BLANK_UNBLANK:
			pr_info("%s : schedual esd work.pdata=0x%p\n", __func__,pdata);
		...
		case FB_BLANK_HSYNC_SUSPEND:
			pr_info("%s : cancel esd work.pdata=0x%p\n", __func__,pdata);
			cancel_delayed_work(&pdata->check_status);
			break;
		default:
...
int __init mdss_dsi_status_init(void)
{
	...
	pr_info("%s: DSI status check interval:%d,pstatus_data=0x%p\n", __func__,interval,pstatus_data);
