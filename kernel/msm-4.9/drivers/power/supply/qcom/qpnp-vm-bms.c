...
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	bool charging;
	static bool charge_warm_enable = true;
	union power_supply_propval ret = {0,};
	...
	if (chip->batt_psy){
                power_supply_get_property(chip->batt_psy,POWER_SUPPLY_PROP_HEALTH, &ret);
#ifdef CONFIG_FIH_BEATLES
		if(ret.intval == POWER_SUPPLY_HEALTH_WARM || ret.intval == POWER_SUPPLY_HEALTH_COOL){
			if(chip->last_ocv_uv / 1000 > 4180 && charge_warm_enable){
#else
		 if(ret.intval == POWER_SUPPLY_HEALTH_WARM){
                       if(chip->last_ocv_uv / 1000 > 4080 && charge_warm_enable){
#endif
				ret.intval = 0;
				power_supply_set_property(chip->batt_psy,POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
				charge_warm_enable = false;
#ifdef CONFIG_FIH_BEATLES
			}else if(chip->last_ocv_uv / 1000 < 4050 && !charge_warm_enable){
#else
			}else if(chip->last_ocv_uv / 1000 < 3950 && !charge_warm_enable){
#endif
				ret.intval = 1;
				power_supply_set_property(chip->batt_psy,POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
				charge_warm_enable = true;
			}
		}else if(!charge_warm_enable){
			ret.intval = 1;
			power_supply_set_property(chip->batt_psy,POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
			charge_warm_enable = true;
		}
	...
	pr_debug("last_soc=%d calculated_soc=%d soc=%d time_since_last_change=%d  charge_warm_enable=%d chip->last_ocv_uv=%d\n",
			chip->last_soc, chip->calculated_soc,
			soc, time_since_last_change_sec, charge_warm_enable,chip->last_ocv_uv);
...
static int qpnp_vm_bms_probe(struct platform_device *pdev)
{
	...
	schedule_delayed_work(&chip->monitor_soc_work, 30 * HZ);
