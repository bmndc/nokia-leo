...
struct qpnp_lbc_chip {
	...
	struct delayed_work		charger_log_work;
...
#define QPNP_LBC_IBATSAFE_MAX_MA	550
...
#define QPNP_LBC_IBATMAX_MAX	550
/*
...
static int64_t qpnp_lbc_get_battery_id(struct qpnp_lbc_chip *chip)
{
	...
	if (rc) {
		...
		pr_debug("batt id channel = %d, batt_id = %lld\n",
...
static int get_prop_batt_temp(struct qpnp_lbc_chip *chip)
{
	...
	if (chip->fih_battery_temp > -500)
                return chip->fih_battery_temp;
	pr_err("get_prop_batt_present=%d\n",get_prop_batt_present(chip));
	if (chip->cfg_use_fake_battery || !get_prop_batt_present(chip))
		return DEFAULT_TEMP;
...
{
	unsigned int chg_current = 550;  //chip->usb_psy_ma;
	...
	pr_info("%s: current %d mA. therm = %d mA. \n", __func__, chg_current, chip->thermal_mitigation[chip->therm_lvl_sel]);
}
...
#else
			pr_err("beck delay eoc!\n");
			chip->chg_done = true;
			alarm_start_relative(&chip->eoc_delay_work_alarm, ns_to_ktime(1LL * NSEC_PER_SEC));
...
static void qpnp_lbc_jeita_adc_notification(enum qpnp_tm_state state, void *ctx)
{
...
#ifdef CONFIG_FIH_BEATLES
	chip->cfg_overheat_bat_decidegc = 470;
#else
	chip->cfg_overheat_bat_decidegc = 500;
#endif
	...
	if (state == ADC_TM_WARM_STATE) {
		if (temp >= (chip->cfg_overheat_bat_decidegc)) {
			/* Warm to overheat */
			bat_overheat = true;
			chip->adc_param.low_temp = chip->cfg_overheat_bat_decidegc - 20;
			chip->adc_param.state_request = ADC_TM_COOL_THR_ENABLE;
		} else if (temp >= chip->cfg_warm_bat_decidegc) {
			/* Normal to warm */
			bat_warm = true;
			chip->adc_param.low_temp = chip->cfg_warm_bat_decidegc - 60;
			...
			chip->adc_param.high_temp = chip->cfg_cool_bat_decidegc + 30;
			chip->adc_param.state_request = ADC_TM_HIGH_LOW_THR_ENABLE;
		} else {
	...
	pr_err("beck chip->therm_lvl_sel %d overheat %d, warm %d, cold %d, cool %d, low = %d deciDegC, high = %d deciDegC\n",
			chip->therm_lvl_sel, chip->bat_is_overheat, chip->bat_is_warm, chip->bat_is_cold, chip->bat_is_cool,
...
#define CHG_LOG_DLY_MS       10000
static void qpnp_charger_log_work(struct work_struct *work)
{
        struct delayed_work *dwork = to_delayed_work(work);
        struct qpnp_lbc_chip *chip = container_of(dwork,
                        struct qpnp_lbc_chip,
	...
	pr_err("ui_soc=%d current=%dmA real_status=%d temp=%d vol=%dmv chip->therm_lvl_sel=%d charger_type=%d chip->chg_done=%d\n",ui_soc,get_prop_current_now(chip)/1000,real_status,get_prop_batt_temp(chip),get_prop_battery_voltage_now(chip)/1000,chip->therm_lvl_sel,ret.intval,chip->chg_done == true ? 1 :0);
	schedule_delayed_work(&chip->charger_log_work,
                                msecs_to_jiffies(CHG_LOG_DLY_MS));
}
...
static int qpnp_lbc_main_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&chip->charger_log_work,
                        qpnp_charger_log_work);
	...
	if (temp >= chip->cfg_overheat_bat_decidegc) {
			...
			chip->adc_param.low_temp = chip->cfg_overheat_bat_decidegc - 50;//50
			...
			chip->adc_param.low_temp = chip->cfg_warm_bat_decidegc - 60;//39
			...
			chip->bat_is_overheat = false;
			chip->adc_param.low_temp = chip->cfg_cold_bat_decidegc;
			chip->adc_param.high_temp = chip->cfg_cool_bat_decidegc + 30;//18
			chip->adc_param.state_request = ADC_TM_HIGH_LOW_THR_ENABLE;
		} else {
			chip->bat_is_warm = false;
...
#ifndef CONFIG_FIH_BEATLES
	if((result.physical>550000) && (result.physical<650000))		//battery id resisteris 100k
	{
		check_batt_id_flag = true;
		pr_err("battery id is right,  battery id:  %lu\n", (unsigned long)(result.physical));
	}
#else
	if((result.physical>900000) && (result.physical<999000))		//battery id resisteris 220k
	{
		check_batt_id_flag = true;
	...
	//schedule_delayed_work(&chip->charger_log_work,
        //                        msecs_to_jiffies(CHG_LOG_DLY_MS*2));
