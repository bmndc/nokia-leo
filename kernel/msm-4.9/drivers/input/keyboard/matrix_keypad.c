...
#ifdef CONFIG_FIH_LEO
static ssize_t keypad_wakeup_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct matrix_keypad *keypad = dev_get_drvdata(dev);

	return sprintf(buf, "%d", keypad->gpio_all_wakeup_enabled);
}

static ssize_t keypad_wakeup_store(struct device *dev,
			...
			const char *buf, size_t count)
{
	...
	/* '1' to enable irq wakeup; '0' to disable irq wakeup */
	if (state == 1) {
#ifdef CONFIG_PM_SLEEP
		matrix_keypad_enable_wakeup(keypad);
		keypad->gpio_all_wakeup_enabled = true;
#endif
	} else {
#ifdef CONFIG_PM_SLEEP
		matrix_keypad_disable_wakeup(keypad);
		keypad->gpio_all_wakeup_enabled = false;
#endif
	}
	return count;
}

static DEVICE_ATTR(keypad_wakeup, 0444, keypad_wakeup_show, keypad_wakeup_store);
#endif
...
static int matrix_keypad_probe(struct platform_device *pdev)
{
	...
	dev_warn(&pdev->dev, "keypad type = %d\n", type);
	if (0 == type)
	{
		err = matrix_keypad_build_keymap(pdata->keymap_data, "linux,keymap_0",
						 pdata->num_row_gpios,
						 pdata->num_col_gpios,
						 NULL, input_dev);
	} else if (1 == type) {
		err = matrix_keypad_build_keymap(pdata->keymap_data, "linux,keymap_1",
						 pdata->num_row_gpios,
						 pdata->num_col_gpios,
						 NULL, input_dev);
	} else {
...
#ifdef CONFIG_FIH_LEO
	err = device_create_file(&pdev->dev, &dev_attr_keypad_wakeup);
	if (err) {
		dev_err(&pdev->dev, "sys create keypad irq wakeup file failed!\n");
...
module_param(type, int, 0600); /*0: 2.4 panel; 1: 2.8 panel*/
