...
struct sugov_tunables {
	...
	bool pl;
#ifdef CONFIG_CPU_FREQ_SCHEDUTIL_BOOST
	bool disable_down_freq;
#endif
};
...
#ifdef CONFIG_CPU_FREQ_SCHEDUTIL_BOOST
	if (next_freq < sg_policy->next_freq && (unlikely(sg_policy->tunables->disable_down_freq) ||
...
#ifdef CONFIG_CPU_FREQ_SCHEDUTIL_BOOST
bool get_disable_down_cpufreq(void)
{
	if (NULL != global_tunables) {
		return global_tunables->disable_down_freq;
	}
	return false;
}

void set_disable_down_cpufreq(bool disable)
{
	if (NULL == global_tunables) return;

	global_tunables->disable_down_freq = disable;
}
EXPORT_SYMBOL_GPL(get_disable_down_cpufreq);
EXPORT_SYMBOL_GPL(set_disable_down_cpufreq);

static ssize_t disable_down_cpufreq_show(struct gov_attr_set *attr_set, char *buf)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);

	return scnprintf(buf, PAGE_SIZE, "%u\n", tunables->disable_down_freq);
}

static ssize_t disable_down_cpufreq_store(struct gov_attr_set *attr_set, const char *buf,
				   size_t count)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);

	if (kstrtobool(buf, &tunables->disable_down_freq))
		return -EINVAL;

	return count;
}

static struct governor_attr disable_down_cpufreq = __ATTR_RW(disable_down_cpufreq);
#endif
